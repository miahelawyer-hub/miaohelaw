import { SITE_URL } from '../consts';

export interface FaqItem {
  slug: string;
  capsule: string;
  question: string;
  summary: string;
  answers: string[];
  details: string[];
  keywords: string;
  featured?: boolean;
  /** English FAQ path for hreflang, e.g. `/en/faq/toronto-divorce-cost-how-much/` */
  pairedFaqPath?: string;
  /** When set, listing / search / related links use this path (page must exist separately, not `/faq/[slug]`). */
  detailPath?: string;
  /** Overrides `<title>` / og:title when set */
  metaTitle?: string;
  /** Overrides meta description when set (plain text, no HTML) */
  metaDescription?: string;
}

/** FAQ list / search URL path for an item (respects `detailPath` for standalone hubs). */
export function getFaqDetailPath(item: FaqItem): string {
  return item.detailPath ?? `/faq/${item.slug}/`;
}

export function getFaqDetailAbsoluteUrl(item: FaqItem): string {
  return `${SITE_URL}${getFaqDetailPath(item)}`;
}

export const faqItems: FaqItem[] = [

  {
    slug: 'laogong-rrsp-lihun-fen',
    capsule: '老公名下有RRSP，离婚能分到吗？',
    question: '我老公名下有RRSP，离婚的时候能分到吗？',
    summary:
      '可以。RRSP写在一方名下也要纳入财产披露与计价；婚姻存续期间账户有增长的，增长部分通常体现在分居日的资产统计里，通过净家庭财产均等分割结算。最终能拿到多少取决于双方整体资产负债与差额，不是只看RRSP这一项。',
    answers: [
      '如果他的RRSP在婚姻存续期间内是有增长的，你是可以分到的。离婚时看的是分居日双方名下的财产，减去结婚日已经存在的财产，这段期间里增长出来的价值双方都可以参与分配，这就是家庭净资产（净家庭财产）的计算思路。',
      '所以在加拿大安省，分居时一方名下有RRSP的，另一方往往可以通过均等分割主张到相应价值。根据何律师的经验，具体金额要结合双方全部资产与债务一起测算，建议带上RRSP对账单和财产披露材料咨询律师。',
    ],
    details: [
      '安省《家庭法》实行的是净家庭财产均等分割（Equalization of Net Family Property），不是把RRSP或某一套房单独拿出来"一刀切"平分。计算方式是：各自统计分居日名下财产总值，减去结婚日财产总值，再依法扣除债务与可排除项目，得出各自的净家庭财产；双方净财产有差额时，净资产较高的一方通常向较低的一方支付差额的一部分。',
      'RRSP登记在丈夫或妻子一方名下，并不改变其需要披露、需要按分居日计价并纳入净资产表的性质。婚姻期间市值的增长，一般会体现在分居日的账户价值里；若存在婚前余额、婚前供款或依法可剔除的部分，则可能影响计入净值的数额，需要个案核对。',
      '根据何律师的经验，不少客户误以为"在他名下就没我的份"，这在安省并不是这样理解的。您实际能主张多少，要看Equalization的整体结果；履行方式也可能是现金结算、双方协商方案，或在符合条件时讨论是否采用RRSP定向划转等安排，涉及税务与银行规则，不宜自行想当然操作。',
      '建议您尽早整理账户月结单、供款记录与分居日前后的市值材料，便于律师评估。欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '老公名下的RRSP，离婚时能分吗？| 安省家庭法律师 何淼',
    metaDescription:
      '可以。RRSP须纳入财产披露与计价，婚姻存续期间的增长部分通过净家庭财产均等分割结算。最终能拿多少取决于双方整体资产负债差额，不只看RRSP一项。647-930-6688。',
    keywords: 'RRSP离婚分割,老公RRSP离婚,安省净家庭财产,Equalization,多伦多离婚财产律师,何淼律师',
    featured: true,
  },
  {
    slug: 'haizi-yijian-jianhu-fayuan',
    capsule: '法院会听取孩子的意见，但孩子没有决定权。法官综合考量多项因素，六七岁以下孩子的声音在判决中占比较小。',
    question: '孩子说想跟爸爸住——离婚后孩子的意见法院会听吗？',
    summary:
      '法院会考虑孩子的意愿，但孩子没有决定权；意愿只是综合因素之一。法官以子女最大利益为首要标准，尤其看重婚姻存续及分居后的实际照顾现状（parenting status quo）。六七岁以下孩子的意见通常权重较低，年龄越大、表达越成熟，分量越重。',
    answers: [
      '法院会考虑孩子的意愿，但不由孩子做主。孩子的声音只是法院综合考量的因素之一。',
      '法院最核心的判断标准是：婚姻存续期间及分居后，双方实际照顾孩子的状况（parenting status quo）是怎样的。',
    ],
    details: [
      '在安省家庭法中，孩子的意愿（Voice of the Child）是法院考量监护安排的因素之一，但绝不是唯一因素，也不是决定性因素。法院的首要原则始终是孩子的最大利益（Best Interests of the Child）。',
      '法院最看重的是"现状"（Status Quo）——也就是在婚姻存续期间以及分居之后，孩子实际上主要由谁照顾、谁接送上学、谁处理日常医疗和学校事务。这个照顾现状是最有分量的证据。',
      '孩子的年龄对其意见的权重影响很大。六七岁以下的孩子，法院认为其尚不具备充分理解"最大利益"的认知能力，其意愿的参考比重相对较小。年龄越大、表达越成熟，孩子的意见在判决中的分量越重。',
      '如果孩子的意见需要正式呈堂，通常由法院委任的独立儿童律师（OCL，Office of the Children\'s Lawyer）来代表孩子的立场，而不是让孩子直接出庭作证。',
      '何淼律师提示：分居后尽早建立稳定的育儿现状，是保护自身监护权的最有效方式。如需了解您的具体情况，请致电647-930-6688预约咨询。',
    ],
    metaTitle: '孩子想跟爸爸住，法院会听吗？| 多伦多华人家庭法律师',
    metaDescription:
      '法院会考虑孩子意愿，但孩子没有决定权。法官以子女最大利益为首要标准，年龄越大表达越成熟，意见分量越重。六七岁以下孩子的意见通常权重较低。647-930-6688。',
    keywords:
      '孩子意见监护权安省,孩子说想跟爸爸住,离婚孩子选择权,子女最大利益安省,Voice of the Child安省,何淼律师',
    featured: true,
  },
  {
    slug: 'dai-haizi-qianju-wengehua',
    capsule: '不能直接搬。对方不同意的情况下，必须走正式的迁居（Relocation）程序，获得法院命令后才可以搬。',
    question: '离婚后我想带孩子搬去温哥华，对方不同意，我能直接搬吗？',
    summary:
      '不能直接搬。带孩子迁往另一城市或省份须遵守迁居规则：一般须书面通知对方（Notice of Relocation），对方可在法定期限内反对；若无法协商一致，须由法院以子女最大利益为准裁决。未取得对方同意或法院命令擅自带孩子搬离，可能带来严重法律后果。',
    answers: [
      '不能。如果对方不同意，你不可以直接带孩子搬去温哥华。你需要先走法定的迁居通知程序，如果双方无法达成一致，必须申请法院命令。',
      '简单来说：没有对方同意或法院命令，擅自带孩子搬离是违法的，可能严重影响你的监护权。',
    ],
    details: [
      '根据安省<a href="https://www.ontario.ca/laws/statute/90c12" target="_blank" rel="noopener noreferrer">《儿童法改革法》</a>及《离婚法》下的迁居规则，如果你计划带孩子搬到另一城市或省份，通常须提前以书面形式向对方发出迁居通知（Notice of Relocation）。对方收到通知后须在法定期限内回复，说明是否同意以及理由。',
      '如果对方在规定时间内没有回复，在符合法定条件的情形下，有时可能被推定为同意；但若对方明确反对，则无法自行解决，必须向法院提出申请，由法官判断是否允许迁居。',
      '法庭审理迁居申请时，核心考量仍是子女最大利益。法官通常会综合评估：迁居对孩子生活、学习、社交的影响；迁居后另一方与孩子保持有意义联系是否现实可行；申请方理由是否充分合理（例如工作机会、家庭支持等）。',
      '在未取得对方同意或法院命令的情况下擅自带孩子搬离，可能被认定为严重不当行为，并可能被视为违反现有法院命令或引发紧急救济程序，极端情形下对您的育儿安排产生重大不利影响。',
      '何淼律师提示：计划迁居前请尽早咨询律师，了解通知期限与申请流程，保护自身法律权益。致电647-930-6688预约咨询。',
    ],
    metaTitle: '离婚后带孩子搬去温哥华，对方不同意能直接搬吗？| 何淼律师',
    metaDescription:
      '不能直接搬。须书面通知对方，对方可在法定期限内反对；无法协商须由法院裁决。擅自带孩子搬离可能带来严重法律后果。647-930-6688。',
    keywords:
      '离婚后带孩子搬家安省,relocation子女监护安省,带孩子搬去温哥华离婚,迁居通知程序安省,华人离婚律师多伦多,中文家庭法律师安省,华人家庭法律师万锦,华人离婚律师列治文山,中文离婚咨询大多伦多,何淼律师Markham,Miao He律师安省',
    featured: true,
  },
  {
    slug: 'hun-hou-zhai-wu-li-hun-cheng-dan-ontario',
    capsule:
      '不一定。关键看债务的性质——是否用于家庭共同生活、是否只在对方个人名下。债权人无法单纯因为你们是夫妻就向你追索。',
    question: '老公婚后欠了很多信用卡债，离婚后这些债我也要还吗？',
    summary:
      '不一定。关键看债务性质与资金实际用途：是否用于家庭共同生活、是否仅为对方个人举债且登记在其个人名下。债权人通常不能仅因婚姻关系向你追索；若进入诉讼，法院会审查款项最终用途以判断责任范围。',
    answers: [
      '要看债务的性质。如果这笔债是用于家庭共同生活，你有可能承担还款义务。如果完全是对方个人行为、你没有任何受益，债权人通常无法向你追索。',
      '信用卡只在对方名下，你没有签字担保，债权人一般不能直接向你索偿。但如果对方被起诉，法院会追查这笔钱的最终用途。',
    ],
    details: [
      '在安省家庭法框架下，婚姻期间的债务并不自动由双方共同承担。债务归属的核心问题是：这笔钱最终用在了哪里？',
      '如果债务用于家庭共同开支——例如家庭日常生活、子女教育、共同购置资产——那么这笔债务可能被认定为夫妻共同债务，另一方有一定的偿还义务。',
      '如果债务完全属于个人行为——例如赌债、个人私贷、私下借款、网贷等——另一方配偶对此毫无受益，信用卡也只在对方个人名下，债权人在法律上无权向你追索。',
      '如果债权人同时起诉夫妻双方，法院会审查这笔钱的实际用途。如果证明资金完全用于个人挥霍、赌博或其他与家庭无关的事项，另一方配偶在很大程度上不需要承担责任。',
      '何淼律师提示：离婚时债务的分配和归属是财产分割的重要组成部分，建议尽早咨询律师，厘清自己的法律责任边界。致电647-930-6688或微信372656035预约咨询。',
    ],
    metaTitle: '老公婚后欠的信用卡债，离婚后我也要还吗？| 安省家庭法律师',
    metaDescription:
      '不一定。关键看债务是否用于家庭共同生活、是否仅为对方个人举债。债权人通常不能仅因婚姻关系向你追索。647-930-6688。',
    keywords:
      '婚后债务离婚安省,老公信用卡债离婚我要还吗,夫妻债务分担安省,赌债离婚安省,个人债务配偶责任安省,华人离婚律师多伦多,中文家庭法律师安省,华人家庭法律师万锦,华人离婚律师列治文山,中文离婚咨询大多伦多,何淼律师Markham,Miao He律师安省',
    featured: true,
  },
  {
    slug: 'quanzhizhifu-shinian-peioushanyangfei',
    capsule:
      '可以申请。配偶赡养费的金额和年限没有强制标准，法院综合考量双方收入差距、婚姻年限、牺牲程度等多项因素来决定。',
    question: '结婚10年全职带孩子，离婚后我能拿到赡养费吗？能拿多久？',
    summary:
      '可以申请配偶赡养费（Spousal Support）。全职照顾家庭、牺牲职业发展，是法院重点考量的因素之一。金额与年限没有固定公式，法院会参考联邦《配偶赡养费指引》（SSAG）但可裁量调整；婚姻越长、收入差距与依赖越大，越可能获得较高或较长期的赡养安排，具体因案而异。',
    answers: [
      '可以申请配偶赡养费（Spousal Support）。全职在家照顾孩子、放弃职业发展，是法院支持赡养费申请的重要依据。',
      '赡养费能拿多少、拿多久，没有固定答案。法院会参考联邦赡养费指引（SSAG），但这只是参考范围，最终判决取决于双方的具体情况。',
    ],
    details: [
      '在安省，配偶赡养费的核心考量是婚姻期间的经济依赖关系。你全职带孩子10年，放弃了职业发展和收入积累，这本身就构成了对赡养费的强力依据。',
      '赡养费的金额主要看双方的收入差距。你目前没有工作，法院会参考对方的年收入来确定合理的支付金额。收入差距越大，赡养费通常越高。',
      '赡养费的年限与婚姻存续时间密切相关。一般经验是，婚姻年限越长，赡养费支付的时间越长。10年婚姻通常意味着相对较长的赡养期，有些案例甚至获得长期或不定期赡养费。',
      '联邦《配偶赡养费指引》（SSAG）提供了一个金额和年限的参考范围，但安省法院并不强制适用，法官会根据实际情况上下调整。',
      '如果一方存在残疾或健康问题影响其工作能力，法院也会将此纳入考量，可能进一步延长赡养期或提高金额。',
      '何淼律师提示：全职照顾家庭的一方往往低估了自己对赡养费的申请权利。建议尽早咨询，了解自己的实际权益范围。致电647-930-6688或微信372656035预约咨询。',
    ],
    metaTitle: '结婚10年全职带孩子，离婚后能拿到赡养费吗？| 何淼律师',
    metaDescription:
      '可以申请配偶赡养费。全职照顾家庭、牺牲职业发展是法院重点考量因素。婚姻越长、收入差距越大，越可能获得较高或较长期的赡养安排。647-930-6688。',
    keywords:
      '全职主妇离婚赡养费安省,配偶抚养费能拿多久安省,结婚十年离婚赡养费,Spousal Support安省,SSAG配偶赡养费指引,华人离婚律师多伦多,中文家庭法律师安省,华人家庭法律师万锦,华人离婚律师列治文山,中文离婚咨询大多伦多,何淼律师Markham,Miao He律师安省',
    featured: true,
  },
  {
    slug: 'gongqian-peiou-pr-lihun-yimin',
    capsule:
      '通常不会因为离婚失去已有身份。但如果你还没拿到PR、正依赖对方担保申请中，分居后担保关系可能终止，需要尽快评估移民影响。',
    question: '我有工签，对方是PR，现在要离婚，我的身份会不会有问题？',
    summary:
      '已有工签或已获批的PR，一般不会仅因离婚而自动失效；但若PR申请系配偶担保且尚未获批，分居后婚姻关系变化可能导致担保无法继续，须尽快同时评估家庭法与移民法路径。',
    answers: [
      '如果你已经拿到PR卡，离婚通常不会导致PR被撤销——前提是当初申请时提交的材料是真实的。',
      '如果你目前持工签，还没有PR，希望对方作为配偶帮你担保申请PR，那么分居之后你们的配偶关系在法律上已经改变，对方很可能无法继续作为你的担保人。',
    ],
    details: [
      '已获批的PR身份是独立的移民身份，不会因为婚姻关系破裂而自动被撤销。离婚本身不影响已经成立的PR卡。',
      '但有一个重要前提：如果移民局在审查中发现当初的婚姻申请材料存在虚假或欺诈成分，PR是有可能被撤销的。这与离婚本身无关，而与材料的真实性有关。',
      '如果你目前持工签、尚未获得PR，而你的PR申请是以对方为担保人（配偶担保）进行的，那么一旦你们正式分居，你们的配偶身份在法律上发生了实质性变化。',
      '在这种情况下，对方很可能无法继续作为你的担保申请人，已提交的担保申请可能面临被终止的风险。你需要尽快评估是否有其他移民途径，例如工作类别申请或其他签证类别。',
      '何淼律师提示：离婚涉及移民身份的情况较为复杂，家庭法和移民法需要同步考量。建议同时咨询家庭法律师和移民律师，避免因处理离婚而影响自己的移民进程。致电647-930-6688或微信372656035预约咨询。',
    ],
    metaTitle: '我有工签对方是PR，离婚会影响移民身份吗？| 安省家庭法律师',
    metaDescription:
      '已有工签或已获批PR一般不因离婚自动失效。高风险情形是配偶担保PR尚未获批，须同时评估家庭法与移民法路径。647-930-6688。',
    keywords:
      '工签离婚身份问题安省,配偶担保PR离婚安省,离婚影响移民身份加拿大,工签持有人离婚安省,PR离婚会被撤销吗,华人离婚律师多伦多,中文家庭法律师安省,华人家庭法律师万锦,华人离婚律师列治文山,中文离婚咨询大多伦多,何淼律师Markham,Miao He律师安省',
    featured: true,
  },
  {
    slug: 'duifang-zhongguo-yanglao-shebao-fen',
    capsule:
      '可以计入分割。安省财产分割计算的是全球净资产，中国的养老金和社保也必须披露，婚姻期间增长的部分要纳入对方的家庭净资产进行均分计算。',
    question: '对方在中国有养老金和社保，分居时在安省能分吗？',
    summary:
      '可以纳入计算。安省净家庭财产均等分割以全球资产为基础，对方在中国参加的养老金、企业年金及城镇职工基本养老保险等，原则上均需披露现金价值；婚姻存续期间对应权益的增长一般计入其净家庭财产参与差额结算。拒不披露可通过披露令等程序处理。',
    answers: [
      '可以。安省财产分割的计算范围是全球净资产，不论资产在哪个国家，都必须纳入计算。',
      '对方在中国的养老金和社保需要完整披露现金价值，婚姻存续期间增长的部分要计入对方的家庭净资产（Net Family Property），参与均分计算。',
    ],
    details: [
      '在安省《家庭法》框架下，财产均衡（Equalization of Net Family Property）的计算基础是双方的全球净资产，不限于加拿大境内的资产。只要你们长期生活在安省并在安省办理离婚，全球范围内的资产都在披露和计算范围之内。',
      '中国的养老金（如企业年金、职业年金）和社会保险（如城镇职工基本养老保险）都属于需要披露的资产类别。对方需要提供相关账户的现金价值，以及婚姻开始日（Date of Marriage）和分居日（Date of Separation）的具体数值。',
      '计入均分的是婚姻存续期间的增长部分——也就是分居日的价值减去结婚日的价值。结婚前已有的部分作为婚前资产可以扣除，不参与均分。',
      '实际操作中，要求对方如实披露中国境内的养老金和社保信息往往存在一定难度。如果对方拒绝披露或刻意隐瞒，可以通过法律途径申请强制披露令（Disclosure Order），或在庭审中请求法官对不披露行为作出不利推断。',
      '何淼律师持中加双重律师执照，专门处理涉及中国境内资产的跨境离婚案件，熟悉中国养老金和社保体系的披露要求。如需咨询，请致电647-930-6688或微信372656035。',
    ],
    metaTitle: '对方在中国有养老金和社保，安省离婚时能分吗？| 何淼律师',
    metaDescription:
      '可以纳入计算。安省净家庭财产均等分割以全球资产为基础，中国养老金须披露现金价值，婚姻期间增长部分计入净家庭财产参与差额结算。647-930-6688。',
    keywords:
      '中国养老金离婚分割安省,社保离婚加拿大能分吗,全球净资产分割安省,跨境离婚中国资产安省,Net Family Property中国资产,华人离婚律师多伦多,中文家庭法律师安省,华人家庭法律师万锦,华人离婚律师列治文山,中文离婚咨询大多伦多,何淼律师Markham,Miao He律师安省跨境离婚',
    featured: true,
  },
  {
    slug: 'lihun-zhengshu-hou-zaihun',
    capsule:
      '拿到离婚证书（Certificate of Divorce）之后任何时候都可以再婚。离婚证书在离婚令颁发31天后才能申请，拿到证书才是法律上正式离婚。',
    question: '拿到离婚证书之后，什么时候可以再婚？',
    summary:
      '拿到离婚证书（Certificate of Divorce）后，法律上即可再婚，无额外等待期。须注意：法院先颁发离婚令（Divorce Order），通常经过31天无上诉期后方可申请离婚证书；以离婚证书作为婚姻正式解除的证明。',
    answers: [
      '拿到离婚证书（Certificate of Divorce）之后，任何时候都可以再婚，没有等待期。',
      '注意：离婚分两个阶段——法院先颁发离婚令（Divorce Order），31天后才能申请离婚证书。只有拿到证书，才算法律上正式解除婚姻关系。',
    ],
    details: [
      '在安省，离婚程序的最后两个步骤容易被混淆。法院颁发的离婚令（Divorce Order / Divorce Judgment）并不等于离婚完成——它是法院宣告离婚的命令，但婚姻关系在法律上还没有正式解除。',
      '离婚令颁发后有31天的上诉期。如果双方都不提出上诉，31天后可以向法院申请离婚证书（Certificate of Divorce）。这份证书才是证明婚姻已正式解除的法律文件。',
      '拿到离婚证书之后，你可以在任何时候再婚，没有额外的等待期限制。',
      '实际操作提示：如果你计划再婚，特别是需要在中国登记结婚，通常需要将离婚证书进行公证认证（Apostille或中国领事认证），以便在中国民政局或使领馆被接受。建议提前了解目的地国家的文件要求。',
      '何淼律师提示：如果你需要加急办理离婚证书，或需要协助处理离婚文件的海外认证，请致电647-930-6688或微信372656035预约咨询。',
    ],
    metaTitle: '拿到离婚证书之后什么时候可以再婚？| 安省离婚律师 何淼',
    metaDescription:
      '拿到离婚证书后即可再婚，无额外等待期。法院先颁发离婚令，经31天无上诉期后方可申请离婚证书，以此作为婚姻正式解除的证明。647-930-6688。',
    keywords:
      '离婚证书再婚安省,Certificate of Divorce安省,离婚令和离婚证书区别,安省离婚多久可以再婚,离婚后再婚程序加拿大,华人离婚律师多伦多,中文家庭法律师安省,华人家庭法律师万锦,华人离婚律师列治文山,中文离婚咨询大多伦多,何淼律师Markham,Miao He律师安省',
    featured: true,
  },
  {
    slug: 'fumu-shougou-toukuan',
    capsule: '父母出了首付，离婚时这笔钱能要回来吗？',
    question: '父母当年给我们买房出了首付，现在离婚，这笔钱能要回来吗？',
    summary: '不一定。要看房子是婚房还是投资房，钱款打给了谁的账户，父母当时是赠与还是借款，以及有没有协议或聊天记录能证明真实意图。每个案件情况不同，没有固定答案，需要带上所有证据咨询律师。',
    answers: [
      '不一定。这要看你们房子的性质是婚房还是投资房，同时要看这笔钱当时是打到谁的账户——只打给你、打给双方共同账户、还是打给对方个人账户，这些都会影响这笔钱的法律性质。',
      '除此之外，还要看你父母和你们之间有没有协议。如果有协议，协议里写的是赠与还是借款？如果没有协议，聊天记录、转账备注、或者任何书面沟通，都可能揭示双方当时的真实意图。根据何律师的经验，这类案件没有固定答案，每个人的情况都不一样，建议带上所有证据来咨询律师。',
    ],
    details: [
      '很多在大多伦多地区定居的华人家庭买房时，父母会出一部分甚至全部首付。离婚时，这笔首付款的归属往往成为争议的核心。安省《家庭法》下的净资产均等分割原则，对这笔钱的处理方式取决于几个关键因素。',
      '第一：房子的性质。婚房（Matrimonial Home）和投资房在安省法律下有不同的处理规则。婚房在分居日的全部市场价值都要计入净资产，不能扣除婚前或父母出资的部分；投资房则相对灵活，婚前价值可以作为"结婚日资产"被扣除。这意味着同样一笔首付款，用在婚房上和用在投资房上，最终能追回多少，可能差别很大。',
      '第二：钱款打给了谁。如果这笔钱只转入了你个人的账户，更容易主张是父母对你个人的赠与或借款；如果打入了双方共同账户或对方账户，则更难证明这笔钱的归属，对方可以主张这是对双方的共同赠与。',
      '第三：赠与还是借款。如果父母当时签了借条，或者有明确还款约定，这笔钱在离婚时可以作为债务在净资产中扣除，不需要参与均等分割。如果是赠与，处理就更复杂，特别是用于婚房购买时，很可能无法从婚房价值中排除。',
      '第四：有没有沟通记录。即便当时没有正式协议，微信聊天记录、邮件、转账备注，都可能成为证明这笔钱性质的关键证据。法院会综合所有材料来判断双方的真实意图。建议您把所有相关记录整理好，预约何淼律师详细分析您的案件。欢迎致电647-930-6688。',
    ],
    metaTitle: '父母给我们出的首付，离婚时能要回来吗？| 安省家庭法律师',
    metaDescription:
      '不一定。取决于是婚房还是投资房、钱打给了谁、是赠与还是借款，以及有无证据证明真实意图。每个案件不同，需带证据咨询律师。647-930-6688。',
    keywords: '父母首付款离婚,离婚首付能要回来吗,安省婚房分割华人,多伦多离婚财产律师,何淼律师',
  },
  {
    slug: 'lihun-tiaojian',
    capsule: '在安省离婚需要满足什么条件？',
    question: '在加拿大安省离婚需要什么条件？',
    summary: '离婚有三种情形：分居满一年、一方有婚外情、或存在家暴。三种情形出现任何一种都可以申请离婚。实际上绝大多数案件走的是分居满一年这条路，因为最简单、最容易举证。',
    answers: [
      '离婚通常有三种情形：双方分居超过一年、任何一方有婚外情、或存在家暴。以上三种情况出现任何一种都可以申请离婚。',
      '根据何律师的经验，很多人会把"分居一年法院才能判决离婚"和"什么时候能递交离婚申请"搞混。递交离婚申请是在分居后任何时间都可以做的，分居满一年是法院发出离婚证的条件，而不是递交申请的条件。',
    ],
    details: [
      '加拿大《离婚法》规定了三种可以申请离婚的情形。第一种是分居满一年，这是绝大多数案件走的路，因为不需要证明对方的过错，只要双方已经分居满一年，法院就可以发出离婚证。第二种是婚外情，需要提供证据证明对方有出轨行为。第三种是身体或精神虐待，包括家暴。',
      '这里有一个非常重要但经常被误解的细节：分居满一年是法院发出离婚证的条件，不是你递交离婚申请的条件。也就是说，你在分居后的任何时间都可以向法院递交离婚申请，法院会在你分居满一年之后才正式发出离婚证。',
      '根据何律师的经验，很多人分居后就开始等满一年，以为等到了才能做任何事。实际上，分居后就可以立即聘请律师、开始谈判分居协议、处理财产和子女安排，不需要等到分居满一年。越早开始，越能保护自己的权益。',
      '如果您不确定自己是否符合离婚条件，或者想了解哪种方式最适合您的情况，欢迎致电647-930-6688预约何淼律师初次咨询。',
    ],
    metaTitle: '在加拿大安省离婚需要什么条件？| 何淼律师 · 多伦多华人家庭法',
    metaDescription:
      '安省离婚有三种情形：分居满一年、婚外情、或家暴。绝大多数案件走分居满一年，最简单易举证。何淼律师详解离婚条件与申请流程。647-930-6688。',
    keywords: '安省离婚条件,加拿大离婚要求,分居一年离婚,何淼律师,多伦多华人离婚律师',
  },
  {
    slug: 'shangfating',
    capsule: '在安省离婚一定要上法庭吗？',
    question: '在安省离婚一定要上法庭吗？',
    summary: '不一定。无争议离婚完全不需要上法庭，双方签署分居协议后向法院递交申请书即可。根据何律师的经验，约有50%的离婚案件是无争议离婚，另外50%才需要诉讼。',
    answers: [
      '不一定。安省的离婚分为无争议离婚和有争议的诉讼离婚，无争议离婚不需要上法庭。',
      '根据何律师的经验，大约50%的离婚案件是无争议离婚，双方先协商签署分居协议，再向法院递交申请书申请离婚证就可以，全程不需要出庭。',
    ],
    details: [
      '安省的离婚大致分为两种：无争议离婚和有争议的诉讼离婚。无争议离婚是指双方对财产分割、子女安排、抚养费等所有事项都达成了一致，可以签署分居协议，再向法院递交申请书申请离婚证，全程不需要出庭。',
      '根据何律师的实际经验，接手的案件中大约50%是无争议离婚，这类案件效率高、费用低、对双方心理伤害也小。另外50%是有争议的诉讼离婚，也就是大家通常理解的"打离婚官司"，这类案件需要多次出庭，周期较长且不确定。',
      '很多来咨询的客户一开始以为离婚就是打官司、一定要上法庭，其实不是这样。如果双方能够协商，无争议离婚是更好的选择。即便双方有一些分歧，通过律师协助谈判，很多情况下也能在不上庭的情况下达成协议。',
      '想了解您的情况是否适合无争议离婚，欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '在安省离婚一定要上法庭吗？| 多伦多华人离婚律师 何淼',
    metaDescription:
      '不一定。无争议离婚完全不需要上法庭，签署分居协议后递交申请即可。约50%的离婚案件是无争议离婚，另外50%才需要诉讼。647-930-6688。',
    keywords: '安省离婚上法庭吗,无争议离婚,有争议离婚,离婚诉讼,何淼律师',
  },
  {
    slug: 'jiehunzheng',
    capsule: '没有结婚证可以离婚吗？',
    question: '没有结婚证可以离婚吗？',
    summary: '通常需要结婚证，但结婚证遗失可以向政府申请补发。法庭也接受结婚证的电子版。需要注意的是，Marriage Record不等于结婚证，需要向政府申请Marriage Certificate才是真正的结婚证。',
    answers: [
      '通常情况下，离婚需要结婚证。如果结婚证遗失，可以向政府申请补发。法庭接受结婚证的电子版。',
      '根据何律师的经验，很多人会把Marriage Record当成结婚证，但实际上Marriage Record不是结婚证。拿到Marriage Record后，需要向政府另行申请Marriage Certificate，这才是法律意义上的结婚证。',
    ],
    details: [
      '离婚申请通常需要提交结婚证，因为法院需要确认婚姻合法成立。如果法院无法确认结婚证的真实性，可能无法发出离婚证。',
      '结婚证遗失的情况很常见，特别是在中国结婚、之后来到加拿大生活的夫妻，原件可能在多次搬家中遗失。好消息是，安省法院接受电子版结婚证，同时，遗失了原件也可以向中国民政局申请补发。',
      '根据何律师的经验，有一个非常常见的误区需要特别提醒：很多人拿着Marriage Record来办离婚，以为这就是结婚证，但实际上Marriage Record只是婚姻登记记录，不等于Marriage Certificate（结婚证）。如果您只有Marriage Record，需要向政府申请正式的Marriage Certificate才能用于离婚申请。',
      '如果您的情况比较特殊，例如证件不齐全或者涉及跨国婚姻文件认证，建议提前咨询律师了解具体需要准备哪些材料。欢迎致电647-930-6688。',
    ],
    metaTitle: '没有结婚证可以离婚吗？| 安省离婚律师 何淼',
    metaDescription:
      '通常需要结婚证，遗失可向政府申请补发。注意Marriage Record不等于结婚证，须申请Marriage Certificate。法庭接受电子版结婚证。647-930-6688。',
    keywords: '没有结婚证离婚,结婚证遗失,Marriage Certificate,Marriage Record,何淼律师',
  },
  {
    slug: 'fenju-zhutiaojian',
    capsule: '分居后必须分开住吗？',
    question: '分居后必须分开住吗？还住在同一个房子算分居吗？',
    summary: '不是必须分开住。根据安省法律，双方住在同一屋檐下、但分住不同房间，也可以被认定为分居。但这种情况下税务申报可能面临挑战，建议提前咨询律师了解风险。',
    answers: [
      '不是必须分开住。根据加拿大安省的法律，双方可以住在同一屋檐下、分住不同房间，也可以被认定为分居，这是法律允许的。',
      '根据何律师的经验，这种情况下面临的主要风险是税务申报——由于地址相同，CRA可能会要求提供更多材料来证明分居状态，有时候会导致分居身份无法被成功申报。',
    ],
    details: [
      '很多夫妻由于经济原因或者为了孩子，分居后仍然继续住在同一个房子里。根据安省法律，这种情况是被允许的，住在同一屋檐下也可以算作分居，关键是双方是否以分居的状态生活——例如分房睡、经济独立、不再以夫妻身份参加社交活动等。',
      '但根据何律师的经验，这种"同住分居"的情况在实际操作中会面临一些风险。最常见的是税务申报问题：当您想以"分居"状态申报个人所得税时，由于双方地址相同，加拿大税务局（CRA）可能会要求提供额外材料来证明分居状态，有时候会导致分居身份申报失败。',
      '另一个风险是，如果日后对分居日期产生争议，同住分居的情况会让举证变得复杂。法院会综合考量多种因素来判断分居日期，包括双方是否还会一起吃饭、是否还以夫妻身份参加社交活动、经济是否已经独立等。',
      '如果您正处于同住分居的情况，建议尽早咨询律师，了解如何在这种情况下保护自己的权益并降低风险。欢迎致电647-930-6688。',
    ],
    metaTitle: '分居后必须分开住吗？同住一个房子算分居吗？| 何淼律师',
    metaDescription:
      '不是必须分开住。安省法律认可同屋分居，分住不同房间可被认定为分居。但税务申报可能面临挑战，建议提前咨询律师了解风险。647-930-6688。',
    keywords: '分居不分开住,同一屋檐下分居,分居条件安省,何淼律师',
  },
  {
    slug: 'zhengming-fenju',
    capsule: '如何证明已经分居一年？',
    question: '如何证明已经分居一年？',
    summary: '无争议离婚中，法院主要依据双方自述，只要两人都同意分居日期，法院通常接受。如果双方有争议，法院会综合考量是否分房住、经济是否独立、是否还以夫妻身份参加社交活动等多重因素。',
    answers: [
      '如果是无争议离婚，法院主要看双方的自述，只要两人都同意分居日期，法院可以认可。',
      '如果双方做法庭诉讼且对分居日期有争议，法院会多方面考量：是否还住在一起、经济是否独立、日常生活如何安排、是否还会一起吃饭、是否还以夫妻身份参加社交活动等。法官会综合判断。',
    ],
    details: [
      '分居日期的认定在无争议离婚和有争议的诉讼离婚中有很大差别。在无争议离婚中，双方都同意某一天是分居日期，法院通常不会深究，直接接受双方的自述。',
      '在有争议的诉讼中，分居日期往往成为争议焦点，因为它直接影响财产分割的计算基准日。法院会综合考量多种因素来判断真实的分居日期，包括：双方是否还住在同一个房间、经济账户是否已经独立、是否还会一起吃饭买菜、是否还以夫妻身份参加社交活动或出席家庭聚会等。',
      '根据何律师的经验，建议在分居后尽快以书面形式记录分居日期，例如发一封确认分居的邮件或信件给对方，或者保留分居后分开居住的相关证据（例如租房合同、水电账单等）。这些记录在日后出现争议时可以作为重要证据。',
      '如果您对分居日期的认定有疑问，或者担心对方会对分居日期提出异议，建议尽早咨询律师。欢迎致电647-930-6688。',
    ],
    metaTitle: '如何证明已经分居一年？| 安省家庭法律师 何淼',
    metaDescription:
      '无争议离婚中双方自述一致，法院通常接受。有争议时法院综合考量是否分房住、经济是否独立、是否还以夫妻身份参加社交活动等多重因素。647-930-6688。',
    keywords: '证明分居一年,分居日期认定,分居证据,何淼律师',
  },
  {
    slug: 'wuzhengyi-lihun',
    capsule: '没有分居协议可以直接离婚吗？',
    question: '没有分居协议可以直接离婚吗？',
    summary: '可以。分居协议不是离婚的必要文件。如果双方没有共同财产、没有孩子，可以直接向法院递交申请申请离婚证。但如果有财产或孩子，强烈建议先签署分居协议，明确各方权益。',
    answers: [
      '可以。分居协议不是离婚必须提交的法律文件。如果没有共同财产可以分割、也没有孩子，可以直接向法院提交申请，申请离婚证。',
      '但如果有财产或孩子，一定要签分居协议。不签分居协议，财产分配和孩子抚养的事项就无法确定，日后容易产生争议。',
    ],
    details: [
      '很多人以为离婚必须先签分居协议，其实不是这样。分居协议是一个可选的法律工具，不是强制要求。如果双方没有共同财产、没有孩子，完全可以跳过分居协议这个步骤，直接向法院递交离婚申请。',
      '但如果双方有共同财产（例如房子、存款、投资账户）或者有孩子，强烈建议先签署分居协议，把财产分割方案和子女安排白纸黑字写清楚。不签分居协议直接离婚，意味着这些事项悬而未决，日后很容易引发争议甚至诉讼。',
      '双方同意离婚后，如果不想请律师代理全程，也可以参考法院网站上的步骤自行办理无争议离婚。在需要签宣誓书时，可以到法院，法院会提供免费的Commissioner of Oaths服务帮您见证签署。',
      '如果您不确定自己的情况是否需要分居协议，欢迎致电647-930-6688咨询何淼律师。',
    ],
    metaTitle: '没有分居协议可以直接离婚吗？| 安省离婚律师 何淼',
    metaDescription:
      '可以。分居协议不是离婚的必要文件。无共同财产、无孩子可直接申请。但有财产或孩子，强烈建议先签分居协议明确各方权益。647-930-6688。',
    keywords: '没有分居协议离婚,无分居协议直接离婚,离婚申请,何淼律师',
  },
  {
    slug: 'lihun-dichuan',
    capsule: '离婚申请在哪里提交？',
    question: '离婚申请在哪里提交？必须在安省吗？',
    summary: '住在哪里就向哪里的法院提交申请。如果您住在安省，就在安省提交。在加拿大任何地方住满一年以上，当地法院都有管辖权审理您的离婚申请。',
    answers: [
      '住在哪里就向哪里的法院提交。如果您生活居住在安省，就在安省提交；如果在加拿大其他省份，就向那个省的法院提交。',
      '在加拿大任何地方住超过一年，法院都会审理您的离婚申请，不限于安省。',
    ],
    details: [
      '加拿大的离婚申请是按居住地管辖的。无论您在哪个省居住，只要在当地住满一年以上，当地法院就有权审理您的离婚申请。',
      '如果您目前住在安省，离婚申请就在安省法院提交。安省的离婚申请通常在家庭法庭（Family Court）或安省高等法院（Ontario Superior Court of Justice）提交，具体取决于您所在的城市。大多伦多地区的案件通常在多伦多家庭法庭提交。',
      '如果您和配偶分别住在不同的省份，通常由提出申请的一方选择在自己居住的省份提交申请。但如果双方对管辖地有争议，可能需要法院裁决。',
      '如果您的情况涉及跨省居住或者不确定应该在哪里提交申请，建议咨询律师。欢迎致电647-930-6688与何淼律师预约。',
    ],
    metaTitle: '离婚申请在哪里提交？必须在安省吗？| 何淼律师',
    metaDescription:
      '住在哪里就向哪里的法院提交。在加拿大任何地方住满一年以上，当地法院都有管辖权审理离婚申请。647-930-6688。',
    keywords: '离婚申请哪里提交,安省离婚申请,法院管辖,何淼律师',
  },
  {
    slug: 'xuyao-lushi-ma',
    capsule: '双方同意离婚还需要律师吗？',
    question: '双方同意离婚还需要律师吗？',
    summary: '如果有孩子或财产需要处理，需要律师起草分居协议。无争议离婚的法院申请步骤，可以参照法院网站自行操作，不是必须请律师代理。但律师能帮您避免协议漏洞，降低日后争议风险。',
    answers: [
      '如果需要签署分居协议（有孩子或财产分割），还是需要律师帮忙起草。无争议离婚的法院申请部分，可以根据法院网站的步骤自行进行，不是必须请律师代理。',
      '到法院需要签宣誓书时，法院提供免费的Commissioner of Oaths见证服务，不需要另外找公证人。',
    ],
    details: [
      '双方同意离婚，但不代表不需要律师。关键问题是：你们有没有孩子？有没有共同财产？如果两者都没有，理论上可以完全自己操作，参照安省法院网站的无争议离婚指引，自行准备文件、递交申请、等待法院发出离婚证。',
      '但如果有孩子或者有共同财产，就需要先签署分居协议，把子女安排和财产分割写清楚。分居协议是一份法律合同，措辞和条款非常重要，一旦签署就有法律约束力。如果自行起草，很容易出现措辞模糊、条款不完整或者遗漏重要事项的问题，日后可能引发争议。',
      '即便双方关系和平，请律师审阅或起草分居协议仍然是值得的投资。律师会从专业角度帮您识别潜在风险，确保协议保护您的权益。',
      '如果你们想尽量节省费用，可以请一位律师起草协议，另一方再请一位律师审阅并出具独立法律意见（ILA）。欢迎致电647-930-6688了解无争议离婚的套餐服务。',
    ],
    metaTitle: '双方同意离婚还需要律师吗？| 安省家庭法律师 何淼',
    metaDescription:
      '有孩子或财产需要处理时需要律师。无争议离婚的法院申请可自行操作，但律师能帮您避免协议漏洞，降低日后争议风险。647-930-6688。',
    keywords: '双方同意离婚需要律师吗,无争议离婚律师,分居协议律师,何淼律师',
  },
  {
    slug: 'xinmyimin',
    capsule: '新移民在安省可以离婚吗？',
    question: '新移民在加拿大安省可以离婚吗？',
    summary: '可以。安省处理离婚不看移民身份，只要夫妻任何一方在安省居住满一年，安省法庭就有管辖权。无论是PR、工签、学签还是访客签，均可在安省提起离婚，移民身份与离婚程序完全分开处理。',
    answers: [
      '可以。<strong>安省处理离婚不看移民身份</strong>，只要有任何一方在安省居住满一年以上，安省法庭都会审理您的离婚案件。',
      '无论是PR、工签还是其他身份，只要满足居住时间要求，都可以在安省提起离婚。移民身份与离婚程序完全分开处理。',
    ],
    details: [
      '很多新移民来咨询时第一句话就是"我还没拿到PR，能离婚吗？"答案是可以。加拿大《离婚法》的管辖权建立在居住地上，而不是移民身份上。只要您或您的配偶在安省住满一年，安省法院就可以受理您的离婚申请。',
      '在中国合法登记的婚姻，在加拿大同样受到认可，可以直接在安省法院提起离婚，不需要先回中国办理任何手续。如果涉及证明婚姻存在的文件，中国民政局出具的结婚证可能需要经过公证认证才能在加拿大法庭使用，建议提前准备好相关文件。',
      '另一个常见问题是分居一年的计算。分居不等于必须搬出去住——在同一屋檐下但以分居状态生活，也可以算作分居起点。但这种情况在法律上的认定比较复杂，需要有足够的证据支撑，建议尽早咨询律师确认分居日期。',
      '何淼律师持中加双重执照，熟悉新移民在离婚过程中面临的特殊情况。如有疑问，欢迎致电647-930-6688预约咨询。',
    ],
    metaTitle: '新移民在加拿大安省可以离婚吗？| 多伦多华人离婚律师',
    metaDescription:
      '可以。安省处理离婚不看移民身份，只要任何一方在安省居住满一年即有管辖权。PR、工签、学签、访客签均可在安省提起离婚。647-930-6688。',
    keywords: '新移民离婚加拿大,新移民离婚安省,PR离婚加拿大,工签离婚安省,何淼律师,多伦多华人离婚律师',
  },
  {
    slug: 'zhongguo-jiehun-ansheng-lihun',
    capsule: '在中国结婚可以在安省离婚吗？',
    question: '在中国结婚可以在安省离婚吗？',
    summary: '可以，只要有任何一方在安省连续居住满一年。需要注意的是，如果曾经回中国生活导致在安省的居住时间中断，安省法院可能对管辖权提出异议，建议提前咨询律师确认是否符合条件。',
    answers: [
      '可以。根据加拿大安省的法律，如果有任何一方在安省长期居住生活一年以上，就可以在安省办理离婚，不管婚姻是在哪里登记的。',
      '根据何律师的经验，有些人在安省生活一段时间后又回到了中国，导致在安省的居住时间中断。在这种情况下，安省法院可能会对管辖权提出异议，需要特别注意。',
    ],
    details: [
      '在中国结婚、在安省生活的夫妻，完全可以在安省申请离婚。加拿大法律认可中国的婚姻登记，中国的结婚证在加拿大法庭有效（可能需要公证认证）。',
      '关键条件是居住时间：夫妻任何一方在安省连续居住满一年，安省法院就有管辖权。"连续"这个字很重要——根据何律师的经验，有些客户在安省生活了一段时间，然后回中国待了较长时间，再回来安省。在这种情况下，居住时间可能被认为是中断的，安省法院可能对管辖权提出异议。',
      '如果您的居住情况比较复杂，例如经常往返中加两国，建议在正式提交离婚申请前先咨询律师，确认安省法院是否有管辖权，避免申请被法院拒绝。',
      '何淼律师持中加双重执照，对涉及跨国婚姻的离婚案件有深入了解。欢迎致电647-930-6688咨询您的具体情况。',
    ],
    metaTitle: '在中国结婚可以在安省离婚吗？| 安省家庭法律师 何淼',
    metaDescription:
      '可以，只要任何一方在安省连续居住满一年。若曾回中国生活导致居住时间中断，法院可能对管辖权提出异议，建议提前咨询律师确认。647-930-6688。',
    keywords: '中国结婚安省离婚,跨国婚姻离婚,安省离婚管辖,何淼律师',
  },
  {
    slug: 'yimin-shenfen',
    capsule: '离婚会影响我的移民身份或PR吗？',
    question: '离婚会影响我的移民身份或PR吗？',
    summary: '不会。如果婚姻和移民材料真实合法，离婚不会影响移民身份或PR。只有在移民材料不真实的情况下，移民身份才会被挑战。离婚本身不是移民局审查移民身份的考量因素。',
    answers: [
      '不会。如果您的婚姻和移民都是合法合规办理的，正常在一起生活，离婚了不会影响您的移民身份和PR。',
      '只有在移民身份材料不真实的情况下，移民身份才会被挑战。离婚并不是要挑战双方移民身份的考量因素。',
    ],
    details: [
      '这是很多移民来咨询时最担心的问题之一：离婚会不会导致PR被取消？答案是不会。加拿大移民法和离婚法是两个独立的法律体系，离婚是家庭法范畴的事，不会直接影响已经获批的移民身份。',
      '只有一种情况例外：如果移民申请本身存在虚假信息，例如婚姻是虚假的移民婚姻，移民局发现后可能会撤销PR。但如果您的婚姻和移民都是真实的，两人确实在一起生活过，离婚了不会对移民身份产生任何影响。',
      '如果您是通过配偶担保移民到加拿大，并且担保期还没有结束，建议在离婚前咨询移民律师，了解担保期的具体条款和您的权利。',
      '如有关于离婚的法律问题，欢迎致电647-930-6688与何淼律师预约咨询。',
    ],
    metaTitle: '离婚会影响我的移民身份或PR吗？| 安省家庭法律师 何淼',
    metaDescription:
      '不会。婚姻和移民材料真实合法，离婚不影响移民身份或PR。离婚本身不是移民局审查移民身份的考量因素。647-930-6688。',
    keywords: '离婚影响移民身份,离婚PR,离婚永久居民,何淼律师',
  },
  {
    slug: 'zhongguo-lihun-jiada',
    capsule: '在中国已经离婚，还需要在加拿大再办一次吗？',
    question: '在中国已经离婚，还需要在加拿大再离一次吗？',
    summary: '符合《离婚法》第 22 条时，通常不必在加拿大再离一次。中国判决没分的加拿大财产，安省仍可能走均等化，但要注意第 7(3) 条时效。赡养费不是同一套：为躲义务去境外离的，法院可以不认。',
    answers: [
      '通常不必再离一次。符合联邦《离婚法》第 22 条的外国离婚，安省一般承认身份已经解除。但这不是每份证书都自动过关——为了躲开安省赡养费去境外离的，法院可以不认。',
      '财产是另一件事。中国判决没处理的加拿大房子、账户，仍可能在安省走均等化。时效是离婚后两年、分居后六年，以较早者为准。展开见<a href="/blog/china-divorce-ontario-equalization-afterward/">中国离了婚，安省还能分财产吗</a>。',
    ],
    details: [
      '加拿大《离婚法》第 22 条承认符合条件的外国离婚，常见条件是起诉时前配偶一方已在该国惯常居住至少一年。符合时，不必在安省再走一遍离婚程序。承认规则和执行限制，见<a href="/blog/china-canada-divorce-judgment-recognition/">中国离婚判决在加拿大如何承认</a>。',
      '离婚证只证明婚姻关系可能已经解除，不代表财产已经结清。中国判决没写的加拿大房产、存款、投资，仍可按《家庭法》申请均等化——上诉法院在 Cheng v. Liu 里确认了这条。详见<a href="/blog/china-divorce-ontario-equalization-afterward/">中国离了婚，安省还能分财产吗</a>。',
      '诉讼时效按安省《家庭法》第 7(3) 条：离婚后两年，或分居后六年，或配偶去世后六个月，以最早到期者为准。外国离婚被承认，两年钟可能已经从那张证书的日期起算。',
      '配偶赡养费不要和财产写成一句。外国离婚被承认后，《离婚法》的附带救济通常走不通；若境外离婚就是为了躲安省义务，法院可以不认，赡养费继续审。见<a href="/blog/foreign-divorce-avoid-spousal-support-ontario/">跑到国外离婚，就能躲掉安省赡养费吗</a>。何淼律师持中加双重执照，欢迎致电 647-930-6688。',
    ],
    metaTitle: '在中国已经离婚，还需要在加拿大再离一次吗？| 何淼律师',
    metaDescription:
      '符合条件时通常不必再离一次。中国判决没分的加拿大财产，安省仍可能均等化，但有时效。为躲赡养费去境外离的，法院可以不认。647-930-6688。',
    keywords: '中国离婚加拿大承认,在中国离婚还需要加拿大吗,离婚诉讼时效,何淼律师',
  },
  {
    slug: 'zhongguo-liguoguo',
    capsule: '在中国离过婚，在加拿大再婚后又要离婚怎么办？',
    question: '在中国离过婚，可以在加拿大安省再次离婚吗？',
    summary: '可以。在中国离过婚不影响在加拿大安省办理现有婚姻的离婚手续，只要当前婚姻合法成立、满足居住要求即可。中国出具的离婚证明可能需要公证认证后才能在加拿大使用。',
    answers: [
      '可以。<strong>在中国离过婚不影响在加拿大安省再次办理离婚</strong>，只要您的婚姻是合法办理的即可。',
      '需要注意的是，中国的离婚记录和婚姻证明可能需要经过公证认证才能在加拿大使用。建议提前整理好相关文件，咨询律师了解具体手续。',
    ],
    details: [
      '在中国离过婚的客户，再次在加拿大结婚并面临婚姻破裂时，有时会担心"前一段婚姻的离婚记录会不会影响现在的离婚"。答案是不会。加拿大法院只关注当前婚姻是否合法成立、是否满足居住要求、婚姻是否已经破裂，与您过去的婚史无关。',
      '需要注意的是，如果在诉讼过程中需要证明您的婚姻状况，中国民政局出具的离婚证书可能需要经过公证处公证后，再经认证，才能作为有效文件在加拿大法庭使用。这个流程需要一定时间，建议提前准备。',
      '另一个相关问题是：在中国办理的离婚，在加拿大是否被认可？一般情况下，中国法院作出的离婚判决在加拿大会被承认，但如果涉及财产分割部分需要在加拿大执行，可能需要在安省法院单独申请执行令。',
      '每个案件的文件要求不尽相同，建议在正式启动离婚程序前咨询律师。何淼律师持中加双重执照，欢迎致电647-930-6688咨询。',
    ],
    metaTitle: '在中国离过婚，可以在安省再次离婚吗？| 何淼律师',
    metaDescription:
      '可以。在中国离过婚不影响在安省办理现有婚姻的离婚手续，只要当前婚姻合法、满足居住要求即可。中国离婚证明可能需要公证认证后才能在加拿大使用。647-930-6688。',
    keywords: '中国离婚加拿大承认,在加拿大再次离婚,中国婚姻证明加拿大,何淼律师',
  },
  {
    slug: 'youhaizi',
    capsule: '有孩子的情况下如何离婚？',
    question: '有孩子的离婚和没有孩子的离婚有什么不同？',
    summary: '有孩子的离婚，法院最优先保护的是孩子的利益。子女抚养费是法定义务，父母不能协议放弃；子女监护权和育儿时间必须在离婚过程中明确处理，法院不会批准忽略孩子权益的离婚协议。',
    answers: [
      '有明显不同。<strong>有孩子的离婚，法院更看重孩子的利益</strong>，双方即便达成一致意见，也不能损害孩子的利益。',
      '最典型的例子是子女抚养费（Child Support）——这是必须有的。即便父母双方都同意不要抚养费，法院也不会发出离婚令，因为抚养费是孩子的权利，不是父母可以放弃的选项。',
    ],
    details: [
      '有孩子和没孩子的离婚，程序上有根本性的不同。没有孩子的离婚，双方协议一致后相对容易推进；有孩子的离婚，即便父母双方完全同意某个安排，法院还是会审查这个安排是否符合孩子的最大利益，不符合的话法院可以拒绝批准。',
      '子女抚养费（Child Support）是孩子的法定权利，不是父母可以协议放弃的选项。即便父母双方都说"我们同意不要抚养费"，法院也不会接受。Child Support的金额按联邦《子女抚养费指引》计算，主要依据支付方的收入和子女人数确定。为什么分居协议里不能放弃，见<a href="/blog/child-support-waiver-separation-agreement-ontario/">分居协议里可以不写子女抚养费吗</a>。',
      '子女监护权和育儿时间的安排，也需要在离婚过程中明确处理。加拿大2021年更新的《离婚法》已经用"Decision-Making Responsibility（决策责任）"和"Parenting Time（育儿时间）"取代了传统的"监护权"和"探视权"概念。',
      '有孩子的离婚案件往往比无孩子的案件复杂得多。何淼律师有丰富的涉及子女监护的诉讼经验，如需咨询，请致电647-930-6688。',
    ],
    metaTitle: '有孩子的离婚和没有孩子的离婚有什么不同？| 安省家庭法律师',
    metaDescription:
      '有孩子的离婚，法院最优先保护孩子利益。子女抚养费是法定义务不可协议放弃；监护权和育儿时间必须明确处理，法院不会批准忽略孩子权益的协议。647-930-6688。',
    keywords: '有孩子离婚加拿大,子女抚养费安省,离婚监护权,Child Support加拿大,何淼律师',
  },
  {
    slug: 'haizi-guishui',
    capsule: '离婚后孩子归谁，法院怎么判？',
    question: '离婚后孩子归谁，法院怎么判？',
    summary: '法院会优先考虑维持孩子现有的生活状态，通常倾向于让主要照顾者继续照顾孩子。另一方会获得相应的育儿时间。具体安排取决于各家庭的实际情况，没有固定公式。',
    answers: [
      '通常情况下，法院会优先考虑孩子平时由谁照顾，倾向于不改变孩子现有的生活状态。如果一方是孩子的主要照顾者，法院通常让主要照顾者继续照顾孩子。',
      '法院同时会审理给另一方相应的育儿时间（Parenting Time）。具体给多少，是一周三次、双方平等分配，还是有监护人在场的探视，取决于不同家庭情况和案件事实。',
    ],
    details: [
      '法院在裁决子女安排时，唯一的标准是"子女最大利益（Best Interests of the Child）"。法官会综合考量多种因素，包括孩子与每位家长的关系质量、每位家长的抚养能力、孩子自己的意愿（根据年龄和成熟度给予不同权重）等。',
      '根据何律师的经验，法院非常重视维持孩子生活的稳定性。如果一方是孩子从小的主要照顾者，法院不愿意轻易改变这个状态。但这不代表另一方就没有育儿权利，法院通常会给另一方安排合理的育儿时间。',
      '育儿时间的安排没有固定模式：可以是每周固定几天、可以是双方平等分配时间（Equal Parenting Time）、在特殊情况下也可以是有第三方在场的监督探视。具体安排取决于每个家庭的实际情况。',
      '何淼律师有丰富的子女监护诉讼经验，包括在对方有刑事记录的情况下为客户争取到平等育儿时间。如需咨询，请致电647-930-6688。',
    ],
    metaTitle: '离婚后孩子归谁？法院怎么判？| 多伦多华人家庭法律师',
    metaDescription:
      '法院优先维持孩子现有生活状态，通常让主要照顾者继续照顾。另一方获得育儿时间。具体安排取决于家庭实际情况，没有固定公式。647-930-6688。',
    keywords: '离婚孩子归谁,子女监护权判决,育儿时间安省,何淼律师',
  },
  {
    slug: 'jianhuguan',
    capsule: '子女监护权是怎么判定的？',
    question: '加拿大的子女监护权是怎么规定的？',
    summary: '加拿大2021年更新的《离婚法》以"子女最大利益"为唯一标准，用"决策责任"和"育儿时间"取代了传统"监护权"概念。法院综合评估双方的抚养能力、亲子关系、生活稳定性等因素作出裁决。',
    answers: [
      '加拿大《离婚法》已经更新，用<strong>Decision-Making Responsibility（决策责任）</strong>取代了传统的"抚养权"概念。所有安排都以子女最大利益为核心原则。',
      '何淼律师有丰富的子女监护权诉讼经验，包括在对方有刑事记录的情况下为客户争取到平等育儿时间。',
    ],
    details: [
      '2021年加拿大联邦《离婚法》修订后，原来的"监护权（Custody）"和"探视权（Access）"概念被正式替换。现在的法律框架分为两个部分：Decision-Making Responsibility（决策责任），指谁有权为孩子做重大决定；Parenting Time（育儿时间），指孩子实际和哪位家长生活在一起的时间安排。',
      '法院在裁决子女安排时，唯一的标准是"子女最大利益"。法官会综合考量：孩子与每位家长的关系质量、每位家长的抚养能力、孩子的意愿、孩子的教育和社交需求、家庭暴力记录、以及维持孩子生活稳定性的能力等。',
      '很多客户误以为法院会偏向母亲，或者有刑事记录的一方一定会失去育儿权。实际情况更为复杂，法院的裁决高度依赖具体证据和双方律师的论证。',
      '如果双方对子女安排有争议，专业的法律代理至关重要。何淼律师在子女监护诉讼方面经验丰富，欢迎致电647-930-6688咨询您的具体情况。',
    ],
    metaTitle: '加拿大子女监护权是怎么规定的？| 多伦多华人家庭法律师',
    metaDescription:
      '2021年更新的《离婚法》以"子女最大利益"为唯一标准，用"决策责任"和"育儿时间"取代传统"监护权"概念。法院综合评估双方抚养能力、亲子关系、生活稳定性作出裁决。647-930-6688。',
    keywords: '子女监护权加拿大,Decision Making Responsibility,加拿大抚养权,子女最大利益原则,何淼律师',
  },
  {
    slug: 'burang-jianhuren',
    capsule: '对方不让我见孩子怎么办？',
    question: '对方不让我见孩子怎么办？',
    summary: '可以向法院申请紧急动议（Urgent Motion）。安省法院对于一方不让另一方见孩子的情况设有紧急程序，通常两周内排庭，速度非常快。法院发出的探视令可以请求警察协助执行。',
    answers: [
      '在加拿大安省，如果对方不让你见孩子，可以申请法院紧急令。安省法院对于此类情况设置了Urgent Motion，通常在两周内排庭，开庭速度非常快。',
      '法院发出的探视令（Parenting Time Order），是可以请求警察局协助执行的。',
    ],
    details: [
      '对方不让你见孩子是非常严重的情况，加拿大安省的法律体系对此有专门的紧急救济程序。您不需要等待普通动议排期，可以申请Urgent Motion，法院通常在两周内安排开庭。',
      '申请Urgent Motion时，需要向法院证明情况的紧迫性，例如对方无合理原因拒绝您的探视、孩子的安全可能面临风险等。律师会帮您准备相关材料，向法院陈述为什么这个情况需要紧急处理。',
      '法院发出探视令（或Parenting Time Order）后，如果对方仍然拒绝执行，您可以联系警察局，警察有权协助执行法院命令。对方无故拒绝执行法院命令可能面临藐视法庭（Contempt of Court）的处罚，包括罚款甚至监禁。',
      '如果对方不让您见孩子，请尽快联系律师，时间非常重要。欢迎致电647-930-6688与何淼律师紧急咨询。',
    ],
    metaTitle: '对方不让我见孩子怎么办？| 安省家庭法律师 何淼',
    metaDescription:
      '可向法院申请紧急动议（Urgent Motion）。安省法院设有紧急程序，通常两周内排庭。法院发出的探视令可请求警察协助执行。647-930-6688。',
    keywords: '对方不让见孩子,探视权安省,Urgent Motion,何淼律师',
  },
  {
    slug: 'fuyanfei',
    capsule: '子女抚养费和配偶赡养费怎么计算？',
    question: '抚养费怎么计算？',
    summary: '抚养费分两类。子女抚养费按联邦指引强制计算，依据支付方收入和子女人数确定，双方不能自行约定低于标准。配偶赡养费参考联邦指引，但法院有更大裁量空间，不是每个案件都需要支付。',
    answers: [
      '抚养费分两类：子女抚养费（Child Support）按联邦Child Support Guidelines强制计算，根据支付方收入和子女人数确定金额，双方不能自行约定低于标准。',
      '配偶赡养费（Spousal Support）有Spousal Support Advisory Guidelines可参考，但属参考性指引，法院有更大裁量空间，不像子女抚养费那样强制执行。',
    ],
    details: [
      '子女抚养费（Child Support）的计算相对透明，有固定公式。联邦《子女抚养费指引》提供了一张按收入和子女人数划分的标准表格。Child Support是孩子的法定权利，父母不能协议约定低于指引标准。',
      '育儿时间安排会影响Child Support的计算方式。如果一方育儿时间超过40%，可能适用"分担养育"（Shared Parenting）的计算方式，双方的收入差额决定最终净支付额。',
      '配偶赡养费（Spousal Support）的计算则复杂得多。法院会综合考量婚姻长度、经济依赖程度、牺牲职业发展的程度、双方现有和未来的经济能力等因素，决定是否支付、支付多少、支付多久。并非所有离婚案件都需要支付配偶赡养费。',
      '抚养费的数额在条件变化后可以申请变更。如果对方隐瞒真实收入，法院可以根据实际情况推定收入（Income Imputation）。分居之后一直没起诉，以前那截不是自动归零，见<a href="/blog/retroactive-child-support-ontario/">分居了还没起诉，以前的抚养费还能要吗</a>。欢迎致电647-930-6688咨询。',
    ],
    metaTitle: '抚养费怎么计算？子女抚养费与配偶赡养费详解 | 何淼律师',
    metaDescription:
      '子女抚养费按联邦指引强制计算，依据支付方收入和子女人数确定。配偶赡养费法院有裁量空间，不是每个案件都需要支付。647-930-6688。',
    keywords: '子女抚养费怎么算加拿大,Child Support Guidelines,配偶赡养费安省,Spousal Support计算,何淼律师',
  },
  {
    slug: 'bupay-fuyanfei',
    capsule: '可以不付抚养费吗？',
    question: '可以不付抚养费吗？',
    summary: 'Child Support不可以不付，这是孩子的法定权利，没有例外。Spousal Support如果双方协议同意不付，法院可以尊重。如果是法庭诉讼，是否需要支付Spousal Support会根据多重因素综合判断。',
    answers: [
      '这要区分两种抚养费。如果是Child Support，不可以不付，这是孩子的法定权利，没有例外。',
      '如果是Spousal Support，双方达成协议同意不付的话，法院可以尊重你们双方的意愿。如果是法庭诉讼，法院会根据多重因素综合计算是否需要支付。',
    ],
    details: [
      'Child Support和Spousal Support是两个完全不同的概念，规则也完全不同。Child Support是孩子的权利，不是父母可以协议放弃的——即便双方都同意不支付，法院也不会批准。联邦《离婚法》第 11(1)(b) 条要求法院确认已有合理安排，否则可以中止发出离婚令。详见<a href="/blog/child-support-waiver-separation-agreement-ontario/">分居协议里可以不写子女抚养费吗</a>。',
      'Spousal Support则不同，它是夫妻之间的经济补偿，双方可以协议决定是否支付、支付多少。如果双方在分居协议中约定不支付Spousal Support，法院通常会尊重这个约定（前提是协议是在双方知情、自愿的情况下签署的）。',
      '如果是法庭诉讼，是否需要支付Spousal Support，法院会考量多种因素：婚姻长度、双方的收入差距、一方是否因为婚姻牺牲了职业发展、婚后的经济依赖程度等。婚姻时间越长、收入差距越大、经济依赖程度越高，支付Spousal Support的可能性就越大。',
      'Spousal Support的计算比较复杂，建议咨询律师了解您的具体情况。欢迎致电647-930-6688与何淼律师预约。',
    ],
    metaTitle: '可以不付抚养费吗？| 安省家庭法律师 何淼',
    metaDescription:
      '子女抚养费（Child Support）是孩子的法定权利，不可不付。配偶赡养费（Spousal Support）双方可协议免除；诉讼情形下法院综合多重因素判断是否需要支付。647-930-6688。',
    keywords: '不付抚养费,Child Support,Spousal Support,配偶赡养费,何淼律师',
  },
  {
    slug: 'fangzi-pingfen',
    capsule: '离婚时房子一定要平分吗？',
    question: '离婚时房子一定要平分吗？',
    summary: '不一定。安省实行家庭净资产均等分割原则，房子是这个计算中的一部分，而不是单独平分。涉及债务、赠与、借款、代持等情况，最终分到的金额可能与房子总价值差异很大。',
    answers: [
      '不一定。根据加拿大安省的法律，离婚时房子不是直接平分的。安省实行的是婚姻财产净值均等分割原则，房子是净资产计算的一部分，并不是单独拿出来平分。',
      '根据何律师的经验，很多婚房涉及债务、赠与、借款、代持等法律关系，会导致即便这个房子是婚房，也不会被平分。如果是投资房，那就更不一定平分了。',
    ],
    details: [
      '安省《家庭法》实行的是"家庭净资产均等分割（Equalization of Net Family Property）"原则，不是简单地把每一样财产切成两半。计算方式是：每个人统计自己在分居日名下的所有资产总值，减去结婚日的资产总值，再减去各种债务和可以排除的财产，得出"净家庭财产"，然后双方的净家庭财产做比较，多的一方给少的一方差额的一半。',
      '婚房在这个计算中有特殊规则：婚房（Matrimonial Home）在分居日的全部价值都要计入计算，不能扣除婚前已有的部分。但这不等于房子要直接平分——最终谁分多少，取决于整个净资产计算的结果。',
      '根据何律师的经验，很多婚房的处理比想象中复杂，因为涉及各种法律关系：父母赠与的首付款怎么算？一方婚前存款买的房子怎么算？有人代持份额怎么算？这些都会影响最终的分配结果，不能简单地说"房子平分"。',
      '如果您想了解您的房子在离婚时应该怎么处理，欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '离婚时房子一定要平分吗？| 安省家庭法律师 何淼',
    metaDescription:
      '不一定。安省实行净家庭财产均等分割，房子是计算的一部分而非单独平分。涉及债务、赠与、借款等情况，最终金额可能与房价差异很大。647-930-6688。',
    keywords: '离婚房子平分,婚房分割,安省家庭净资产,Matrimonial Home,何淼律师',
  },
  {
    slug: 'fangzi-buxiang-mai',
    capsule: '离婚时房子可以不卖吗？',
    question: '离婚时房子可以不卖吗？',
    summary: '可以。如果双方同意，其中一方可以买断另一方在房子中的权益（Buy Out），不需要卖房。想保留房子的一方需要去银行重新办理贷款，把对方的份额折现支付给对方。',
    answers: [
      '可以。如果双方达成一致意见，其中一方可以买断另外一方的权益（Buy Out），不需要卖房。',
      '想保留房子的一方需要去银行办理新的贷款（Refinance），把之前的房贷还清，同时把对方在房子中的份额折现支付给对方。',
    ],
    details: [
      '房子的处理方式不止出售一种。最常见的两种方式是：出售房子然后分配房款，或者一方买断另一方的权益（Buy Out）。如果您或对方希望保留这套房子，可以选择Buy Out方式，不需要卖房。',
      'Buy Out的操作方式是：想保留房子的一方去银行申请新的贷款，贷款金额需要覆盖两个部分：一是还清现有的房贷余额，二是支付给对方在房子中的份额。办完之后，房子就完全转到保留一方的名下。',
      '买断的关键是银行是否批准新的贷款。银行会根据保留房子那一方的个人收入、信用评分、债务情况来审核贷款申请。如果收入不足以单独支撑房贷，Buy Out可能无法实现，这种情况下可能需要考虑其他方案，例如延迟出售、共同持有一段时间等。',
      '房子的处理方案需要结合双方的财务状况和具体需求来制定。欢迎致电647-930-6688与何淼律师咨询，我们帮您评估最适合的方案。',
    ],
    metaTitle: '离婚时房子可以不卖吗？| 安省家庭法律师 何淼',
    metaDescription:
      '可以。双方同意可以买断（Buy Out），一方将对方份额折现支付。想保留房子的一方需重新办理贷款，把房子转到自己名下。647-930-6688。',
    keywords: '离婚房子不卖,买断房子,Buyout离婚,离婚房产处理,何淼律师',
  },
  {
    slug: 'fangdai-weihuan',
    capsule: '房贷还没还清，离婚怎么办？',
    question: '房贷还没还清，离婚怎么办？',
    summary: '离婚时房产的处理分几种情况：如果出售房子，房款自动清偿房贷，剩余部分双方分配；如果一方想保留房子，需要重新办理贷款（Refinance），将房子转到自己名下并还清原有房贷。',
    answers: [
      '离婚时房产的处理分几种情况。如果要卖房，卖房款会自动清偿剩余房贷，剩下的钱再在双方之间按协议分配。',
      '如果有一方想承接房子，需要去银行办理新贷款（Refinance），新贷款会把之前存在的房贷还清，同时把房子转到承接方名下。',
    ],
    details: [
      '很多人担心离婚时房贷还没还清会很麻烦，其实处理方式相对清晰。最关键的是先确定这套房子要卖还是要保留，然后根据这个决定来处理房贷问题。',
      '如果决定出售房子：买家付款后，首先用来还清剩余的房贷，然后扣除各种交易费用，剩余的净款按双方协议分配。如果房贷余额大于卖价（即underwater），双方需要共同承担差额，这种情况相对少见。',
      '如果一方要保留房子：需要去银行申请重新融资（Refinance）。新贷款的金额需要覆盖：还清现有房贷，以及支付给另一方应得的份额。银行会根据申请方的个人财务状况审批新贷款。如果银行不批，则可能无法单独保留房子，需要考虑其他方案。',
      '房贷问题涉及银行、法律和财务多个层面，建议在处理离婚时早早开始了解自己的贷款情况和选项。欢迎致电647-930-6688与何淼律师咨询。',
    ],
    metaTitle: '房贷还没还清，离婚怎么办？| 多伦多华人离婚律师',
    metaDescription:
      '出售房子时房款自动清偿房贷，剩余部分双方分配。一方想保留房子须重新办理贷款（Refinance），将房子转到自己名下并还清原有房贷。647-930-6688。',
    keywords: '离婚房贷处理,离婚Refinance,房贷还没还清离婚,何淼律师',
  },
  {
    slug: 'fangzi-yifang-mingyi',
    capsule: '房子只写一方名字，另一方有权分吗？',
    question: '房子只写一方名字，离婚时另一方有权分吗？',
    summary: '有权分。安省家庭净资产计算包括每个人名下的全部财产，无论房子写在谁的名下，另一方都有权在净资产计算中主张房屋的价值。婚房和投资房的处理规则略有不同。',
    answers: [
      '有权分。即便房子只在一方名下，在离婚时计算家庭净资产时，这套房子仍然要被纳入计算，另外一方可以通过净资产均等分割分到相应的价值。',
      '根据何律师的经验，婚房和投资房的处理规则略有不同，双方最终分到的比例可能会不一样。',
    ],
    details: [
      '很多人误以为房子写在谁名下就是谁的，离婚时对方无权分。这是一个常见的误解。在安省，无论房子的产权登记在谁的名下，在离婚时计算家庭净资产时，这套房子的价值都要被纳入计算。',
      '安省《家庭法》规定，每个人需要统计自己在分居日名下所有财产的总值，包括只写在自己名下的房子。然后双方的净家庭财产做比较，多的一方需要给少的一方差额的一半，这就是为什么即便房子不在你名下，你也有权分到相应价值的原因。',
      '根据何律师的经验，婚房（Matrimonial Home）和投资房的处理规则略有不同。婚房在分居日的全部市场价值都要计入，不能扣除婚前已有的部分；而投资房如果是婚前购买的，婚前价值可以作为"结婚日资产"被扣除。这意味着两种房产在最终计算中，双方分到的金额可能会不同。',
      '如果您不确定您的情况下应该如何处理房产，欢迎致电647-930-6688与何淼律师预约咨询。',
    ],
    metaTitle: '房子只写一方名字，另一方有权分吗？| 安省家庭法律师 何淼',
    metaDescription:
      '有权分。安省家庭净资产计算包括全部财产，无论写在谁名下，另一方都有权主张房屋价值。婚房和投资房的处理规则略有不同。647-930-6688。',
    keywords: '房子写一方名字离婚分割,婚房分割,安省离婚房产,何淼律师',
  },
  {
    slug: 'hunqian-caichan',
    capsule: '婚前财产离婚时要分吗？',
    question: '婚前财产离婚时要分吗？',
    summary: '婚前财产可以在净资产计算中被扣除，但有条件：需要提供证据证明婚前财产的价值，且该财产没有被混入婚房中。对方也可以在一定条件下挑战婚前财产的扣除。',
    answers: [
      '根据加拿大安省法律，婚前的财产可以作为"结婚日资产扣除（Date of Marriage Deduction）"被扣除。如果在结婚日有资产，并且没有混入婚房中，您可以申请扣除婚前财产的价值。',
      '但这里有其他条件：如果另一方可以证明婚前的财产被转移走了，并没有在婚姻中消耗，或者在分居日仍然存在，那么也可以申请挑战婚前财产的扣除。',
    ],
    details: [
      '安省家庭净资产的计算公式是：分居日的资产总值减去结婚日的资产总值，再减去债务。这意味着婚前已有的资产（结婚日的资产）可以被扣除，不需要参与均等分割。这就是为什么婚前财产理论上不需要被分割的原因。',
      '但在实际操作中，要成功扣除婚前财产，需要提供证据证明：婚前财产的具体数额（需要有银行账单、评估报告等）、以及该财产没有被"混入"婚房（Matrimonial Home）——因为婚房的全部价值都要计入计算，不论是婚前还是婚后部分。',
      '另一方也可以在特定情况下挑战婚前财产的扣除。例如，如果一方声称婚前有大量资产，但对方可以证明这些资产其实还存在（没有在婚姻中消耗掉），而另一方在分居日却没有申报这些资产，法院可能会对扣除申请提出质疑。',
      '婚前财产的处理在实际案件中往往比较复杂，建议咨询律师了解如何最好地保护您的婚前财产权益。欢迎致电647-930-6688。',
    ],
    metaTitle: '婚前财产离婚时要分吗？| 安省家庭法律师 何淼',
    metaDescription:
      '婚前财产可在净资产计算中被扣除，但需证明婚前财产价值且未混入婚房。对方在一定条件下可挑战扣除。何淼律师详解举证要求。647-930-6688。',
    keywords: '婚前财产离婚,Date of Marriage Deduction,婚前财产分割,何淼律师',
  },
  {
    slug: 'jiating-jingzichan',
    capsule: '离婚时如何计算家庭净资产？',
    question: '离婚时如何计算家庭净资产？',
    summary: '根据安省《家庭法》，每人计算分居日名下资产总额，减去结婚日资产总额，再减去已有债务和排除财产，得出净家庭财产。双方净资产相减，多的一方给少的一方差额的一半。',
    answers: [
      '根据加拿大安省<em>Family Law Act</em>的规定，双方各自计算分居时名下的财产总额，减去结婚时的财产总额，同时减去已存在的各种债务或排除不计入的财产，得出每个人的净家庭财产。',
      '计算清楚后，比较双方的净家庭财产，多的一方要给少的一方两人财产差值的一半。',
    ],
    details: [
      '安省《家庭法》规定的家庭净资产均等分割（Equalization of Net Family Property）是安省离婚财产分割的核心原则。计算公式如下：每个人的"净家庭财产"= 分居日的全部资产总值 - 分居日的全部债务 - 结婚日的资产总值 - 可以排除的财产价值。',
      '可以排除的财产包括：婚后继承的遗产、婚后获得的赠与（但赠与产生的收益不能排除）、人身伤害赔偿、婚前财产在婚后的增值部分（有条件）等。婚房（Matrimonial Home）是个例外，婚房在分居日的全部价值都要计入，不能扣除婚前部分。',
      '计算完双方各自的净家庭财产后，比较两个数字。如果A的净家庭财产是50万，B的净家庭财产是20万，差额是30万，那么A需要给B 15万（差额的一半）。这就是Equalization Payment。',
      '家庭净资产的计算在实际案件中往往相当复杂，特别是涉及跨境资产、公司股权、复杂投资等情况。建议咨询律师进行专业计算。欢迎致电647-930-6688。',
    ],
    metaTitle: '离婚时如何计算家庭净资产？| 安省家庭法律师 何淼',
    metaDescription:
      '每人计算分居日资产总额，减去结婚日资产、债务和排除财产，得出净家庭财产。双方净资产相减，多的一方给少的一方差额的一半。647-930-6688。',
    keywords: '家庭净资产计算,Equalization of Net Family Property,安省离婚财产分割,何淼律师',
  },
  {
    slug: 'gongsi-lihun',
    capsule: '公司或生意在离婚时怎么分？',
    question: '公司或生意在离婚时怎么分？',
    summary: '离婚时不会直接分配公司股权，而是把股权的市场价值纳入家庭净资产计算。公司股权需要专业估值，估出的市场价值按净资产均等分割原则处理，另一方分到的是价值而不是股权本身。',
    answers: [
      '根据加拿大安省法律，离婚时要看公司的股东是谁，把股权的市场价值作为家庭净资产的一部分纳入计算。公司股权本身不会被直接分配，但股权背后的市场价值需要分配给另一方。',
      '举例来说，如果一方持有公司50%股份，离婚时不会让双方一人持有25%，而是把这50%股权估值作价，将其市场价值纳入净资产计算，另一方分到的是这个价值对应的金额，而不是实际股权。',
    ],
    details: [
      '公司和生意的处理是离婚案件中最复杂的领域之一。首先，公司股权需要专业估值——通常需要聘请注册商业估值师（Chartered Business Valuator，CBV）对公司进行评估，确定股权的市场价值。这个过程本身就可能成为争议焦点，双方各聘一位CBV，估出来的价值往往相差很大。',
      '公司的分红和收入也需要纳入考量。公司的分红或股东收入要算作个人收入，影响Child Support和Spousal Support的计算。如果公司账目上有保留盈余（Retained Earnings），这部分也可能被视为资产纳入净资产计算。',
      '离婚时不会直接分割公司股权——不会让对方成为公司的股东。实际操作是把股权的市场价值作为净家庭财产的一部分，通过Equalization Payment的方式，用现金或其他资产支付给对方对应的价值。',
      '公司股权的处理非常复杂，每个案件都不一样。何淼律师在处理涉及公司资产的离婚案件方面有丰富经验。欢迎致电647-930-6688咨询。',
    ],
    metaTitle: '公司或生意离婚时怎么分？| 安省家庭法律师 何淼',
    metaDescription:
      '离婚时不直接分配股权，而是把股权市场价值纳入净资产计算。公司股权需专业估值，另一方分到的是价值而非股权本身。647-930-6688。',
    keywords: '公司离婚分割,生意离婚,股权估值离婚,CBV估值,何淼律师',
  },
  {
    slug: 'bupeihe',
    capsule: '对方不配合离婚怎么办？',
    question: '对方不配合离婚怎么办？',
    summary: '对方不配合不是死路。安省法院不需要对方同意就可以推进离婚案件，分居满一年后可直接提起单方离婚申请。对方拒绝签字、拒绝出庭，都不能阻止案件推进，只会增加自己的法律风险。',
    answers: [
      '对方不配合，<strong>您可以直接提起诉讼</strong>，法院不需要对方同意就可以推进案件。',
      '如果没有孩子、没有财产需要分割，提起Simple Application即可；如果有孩子或有财产需要处理，则需要走完整的离婚诉讼程序，由法院裁决双方争议。',
    ],
    details: [
      '对方不配合是我们接到的最常见情况之一。很多客户以为离婚需要"双方同意"，实际上并非如此。加拿大《离婚法》允许单方面提起离婚申请，法院不需要对方点头才能审理案件。',
      '分居满一年后，即便对方拒绝配合、拒绝签署任何文件，您依然可以向法院申请离婚。法院会向对方送达文件，给对方一定时间回应。如果对方收到文件后仍然不回应，法院可以在对方缺席的情况下作出判决，法律上称为"Default Judgment"。',
      '对方不配合，反而有时对您更有利。拒绝配合的一方往往在法庭上给法官留下负面印象，而频繁的无理阻挠还可能导致对方被判承担您的部分律师费用（Costs Award）。',
      '关键是尽早采取行动，掌握主动权。何淼律师有丰富的单方离婚和对抗性诉讼经验，欢迎致电647-930-6688，我们帮您评估最佳推进方案。',
    ],
    metaTitle: '对方不配合离婚怎么办？| 安省离婚律师 何淼',
    metaDescription:
      '对方不配合不是死路。安省法院不需要对方同意，分居满一年后可直接提起单方离婚申请。对方拒绝签字或出庭只会增加自己的法律风险。647-930-6688。',
    keywords: '对方不配合离婚加拿大,单方面离婚安省,对方拒绝离婚怎么办,何淼律师,多伦多华人离婚律师',
  },
  {
    slug: 'fenju',
    capsule: '已经分居了怎么办理离婚？',
    question: '已经分居了，怎么办理离婚？',
    summary: '已经分居，下一步取决于双方争议程度。如果双方能协商，优先签署分居协议，再向法院申请离婚证书，是最省时省钱的方式。如果有争议，则需要通过诉讼由法院裁决财产、子女等问题。',
    answers: [
      '分两种情况：如果双方已签署分居协议，拿着分居协议去法庭申请离婚证书即可，是最快捷的方式。',
      '如果尚未签署分居协议，财产或孩子的问题还没解决，需要决定是通过分居协议协商解决，还是通过法庭诉讼解决。分居协议是最省时、省钱、程序最简单的方式，推荐有条件的双方优先考虑。',
    ],
    details: [
      '分居是离婚的前置条件，但分居本身不等于离婚。在安省，分居满一年后才可以向法院申请离婚证书。分居期间，双方的财产、子女安排等问题可以通过分居协议先行处理，不需要等到离婚完成。',
      '如果双方已经达成一致，可以签署分居协议（Separation Agreement）。分居协议涵盖财产分割、子女监护、育儿时间、抚养费等所有离婚相关事项。签署后，双方按协议执行，分居满一年后再向法院申请离婚证书。',
      '如果双方对某些事项有争议，则需要通过诉讼解决。诉讼期间，双方可以申请临时命令（Temporary Orders）先处理紧急事项，例如谁暂时居住婚房、临时子女安排等。',
      '分居日期的确认非常重要，因为它直接影响财产分割的计算基准日。建议在分居后尽早咨询律师，确认分居日期并妥善保存相关证据。欢迎致电647-930-6688预约咨询。',
    ],
    metaTitle: '已经分居了，怎么办理离婚？| 安省离婚律师 何淼',
    metaDescription:
      '双方能协商则优先签分居协议再申请离婚证书，是最省时省钱的方式。有争议则需通过诉讼由法院裁决财产、子女等问题。647-930-6688。',
    keywords: '分居后如何离婚加拿大,分居协议安省,已分居离婚手续,何淼律师',
  },
  {
    slug: 'haizi-zhongguo',
    capsule: '对方要带孩子回中国怎么办？',
    question: '对方要带孩子回中国怎么办？',
    summary: '这是极度紧急的情况，必须立即联系律师。中国不是海牙公约成员国，孩子一旦离境追回极难。安省法院可以发出紧急禁止出境令，但时间至关重要——孩子离境后法律救济难度成倍增加。',
    answers: [
      '<strong>这是非常紧急的情况，请立即联系律师。</strong>',
      '中国不是海牙儿童保护公约（Hague Convention）的成员国。一旦孩子被带回中国，追回的难度极大，可能面临长期的法律拉锯。安省法院可以发出禁止带孩子离境的Order，<strong>时间非常关键</strong>。',
    ],
    details: [
      '如果您发现对方有带孩子回中国的迹象——无论是买了机票、申请了护照、还是直接告知——请立即联系律师，不要等待观望。这类案件以小时计，不以天计。',
      '中国没有加入《海牙国际儿童诱拐民事方面公约》。这意味着一旦孩子被带到中国，加拿大法院的命令在中国没有直接的执行机制，追回孩子需要通过中国国内法律程序，难度极大，耗时可能以年计。',
      '在孩子离境前，安省法院可以采取多项紧急措施：发出禁止带孩子出境的命令（Non-Removal Order）、要求对方交出孩子的护照、通知加拿大边境服务局（CBSA）将孩子列入观察名单等。这些措施需要律师紧急申请，法院可以在非工作时间处理此类紧急申请。',
      '请立即致电647-930-6688，我们会告诉您现在可以做什么。',
    ],
    metaTitle: '对方要带孩子回中国怎么办？| 安省家庭法律师 何淼',
    metaDescription:
      '极度紧急，必须立即联系律师。中国不是海牙公约成员国，孩子一旦离境追回极难。安省法院可发出紧急禁止出境令，时间至关重要。647-930-6688。',
    keywords: '对方带孩子回中国怎么办,禁止带孩子出境安省,海牙公约中国,儿童跨境监护,何淼律师',
  },
  {
    slug: 'jiabao',
    capsule: '遇到家暴怎么办？',
    question: '遇到家暴怎么办？',
    summary: '第一步报警，确保人身安全。其他法律问题可以之后处理，但安全是第一位的。家暴对离婚案件有一定法律影响，包括可能影响子女监护安排，但影响程度因案而异，需要律师具体评估。',
    answers: [
      '<strong>第一步：先报警，保证自身安全。</strong>这是最重要的一步，其他所有法律问题都可以之后处理。',
      '家暴对离婚案件有一定影响，但影响程度因案件具体情况而异。遭遇家暴之后，应尽快咨询家庭法律师，了解可以采取哪些法律手段保护自己和孩子。',
    ],
    details: [
      '遭遇家暴的第一步永远是保证自身安全，拨打911报警。警方的出警记录、医院的诊断记录，在之后的法律程序中都可能成为重要证据。',
      '报警之后，法院可以发出紧急保护令或不得骚扰令（Restraining Order），要求施暴方离开共同居所、禁止接触您和孩子。这些命令可以在没有对方在场的情况下申请。',
      '家暴在离婚案件中的法律影响因案件情况而异。在子女监护方面，法院在评估子女最大利益时，会将家暴行为列为重要考量因素——施暴方可能面临受限的育儿时间安排。在财产分割方面，家暴本身通常不直接影响财产均等分割的原则，但具体案件中可能有例外。',
      '很多家暴受害者因为语言障碍、经济依赖或对法律程序不熟悉而不敢寻求帮助。何淼律师提供全程普通话服务，请立即致电647-930-6688，我们会为您保密并提供专业建议。',
    ],
    metaTitle: '遇到家暴怎么办？| 安省家庭法紧急法律援助 · 何淼律师',
    metaDescription:
      '第一步报警确保人身安全。家暴对离婚案件有法律影响，包括可能影响子女监护安排。何淼律师详解家暴情况下的法律保护与离婚程序。647-930-6688。',
    keywords: '家暴离婚加拿大,家暴法律援助安省,遭遇家暴怎么办加拿大,何淼律师',
  },
  {
    slug: 'bushouban',
    capsule: '对方不肯搬出婚房怎么办？',
    question: '对方不肯搬出婚房怎么办？',
    summary: '对方不肯搬出不等于案件无法推进。在安省，配偶对婚房拥有法定居住权，但法院可以发出独占婚房令要求一方离开。即便对方坚持不走，房屋出售、财产分割等程序仍可以照常推进。',
    answers: [
      '在安省法律下，配偶对婚房拥有Possession Right（居住权），对方不肯搬出去，并不意味着案件无法推进。',
      '我们帮助过很多客户在这种情况下成功出售婚房、分割房款，案件照常推进。您能采取的方案不止一种，律师可以根据您的具体情况给出最优建议。',
    ],
    details: [
      '在安省，无论婚房登记在谁的名下，夫妻双方在婚姻存续期间都对婚房拥有法定居住权（Possession Right）。这意味着您不能单方面要求对方搬出去，对方也可以用同样的理由拒绝离开。',
      '但这并不是没有出路。如果双方同住造成家庭关系高度紧张、或存在家暴风险，您可以向法院申请独占婚房令（Exclusive Possession Order），要求法院命令对方离开婚房。',
      '即便对方坚持不搬出去，房屋的处理仍可以通过法律程序推进。如果双方无法就房屋处置达成一致，法院可以发出命令强制出售婚房，并按照各自的权益分配房款。',
      '婚房的处理方式不止出售一种，还包括一方买断另一方权益、延迟出售等方案。欢迎致电647-930-6688与何淼律师预约咨询。',
    ],
    metaTitle: '对方不肯搬出婚房怎么办？| 安省家庭法律师 何淼',
    metaDescription:
      '法院可签发 Exclusive Possession Order 要求配偶搬离婚房。即使对方拒绝离开，仍可依法申请出售婚房并推进财产分割。何淼律师长期处理多伦多家庭法争议，普通话英语双语服务，帮助华人客户高效解决婚房占有与离婚财产问题。647-930-6688',
    keywords: '对方不搬出婚房加拿大,婚房居住权安省,离婚房产处理,Possession Right安省,何淼律师',
  },
  {
    slug: 'yincang-shouru',
    capsule: '对方隐瞒收入怎么办？',
    question: '对方隐瞒收入怎么办？',
    summary: '可以向法院申请推定收入（Imputed Income）。申请方需要提供证据证明对方实际收入高于申报收入，法院会根据证据和Case Law判断是否推定更高的收入水平。不可以凭猜测，必须有证据支撑。',
    answers: [
      '在离婚诉讼中如果有一方隐瞒收入，可以向法院申请推定对方的收入（Imputed Income），要求法院认定对方的收入是更高的数值。',
      '根据加拿大Case Law，申请推定收入必须提供证据，不可以凭猜测。申请方需要提供证据支撑，法院才会推定对方的收入高于申报数额。',
    ],
    details: [
      '对方隐瞒收入在离婚案件中并不罕见，特别是自雇人士、有公司的一方，有时候会通过各种方式压低账面收入，以减少需要支付的Child Support或Spousal Support。',
      '加拿大法律提供了"推定收入（Imputed Income）"这个工具来应对这种情况。如果法院认为一方的实际收入高于其申报的收入，可以推定一个更高的收入数额来计算抚养费。根据加拿大Case Law，申请推定收入的一方必须提供证据，不能仅凭猜测或感觉。',
      '可以用来支撑推定收入申请的证据包括：对方的银行账单、信用卡记录、生活消费水平（例如豪华假期、名车、高档餐饮等与申报收入不符的消费）、公司账目、税务申报记录、以及对方在社交媒体上显示的生活状态等。',
      '收集这些证据需要经验和策略。何淼律师在处理隐瞒收入的案件方面有实战经验，欢迎致电647-930-6688咨询如何有效应对这种情况。',
    ],
    metaTitle: '对方隐瞒收入怎么办？| 推定收入 · 多伦多华人离婚律师',
    metaDescription:
      '可向法院申请推定收入（Imputed Income）。需提供证据证明对方实际收入高于申报收入，法院根据证据判断是否推定更高收入水平。647-930-6688。',
    keywords: '隐瞒收入离婚,Imputed Income,推定收入,何淼律师',
  },
  {
    slug: 'chaxun-zichan',
    capsule: '如何查对方的银行账户或资产？',
    question: '如何查对方的银行账户或资产？',
    summary: '需要掌握一些基本信息才能追查。如果知道对方在某银行有账户但对方拒不提供账单，可以申请法院命令让对方出具授权书，律师凭授权书向银行索取资料。但不同银行配合程度不同。',
    answers: [
      '你需要知道一些基本信息才能追查。如果完全不知道对方有什么账户，是很难查的。',
      '如果你知道对方在某个银行有账户但对方拒绝提供账单，可以申请法院下达命令，让对方出具授权书，授权给你的律师联系银行索取相关材料。目前Scotiabank会配合这样的授权书提供资料，但TD银行在某些情况下不提供。',
    ],
    details: [
      '追查对方的资产是离婚案件中的难点之一，特别是对方刻意隐藏的情况下。首先，追查需要有一些起点——你需要知道对方可能在哪些银行有账户、有什么投资、有什么资产，才能有针对性地申请披露。',
      '在离婚诉讼中，双方都有全面财务披露的法律义务，包括所有银行账户、投资账户、房产、公司权益等。如果对方拒绝提供，可以向法院申请Motion to Compel，要求法院强制对方提供。如果对方仍然拒绝执行法院命令，可能面临藐视法庭的处罚。',
      '对于银行账户的查询，可以向法院申请命令，要求对方出具授权书（Authorization Letter），授权律师直接联系银行索取账单。根据经验，Scotiabank通常会配合这样的授权书提供账单，但TD银行在某些情况下不予配合，可能需要通过法院直接向银行发出传票（Subpoena）。',
      '对于中国境内的资产追查，情况更为复杂，需要通过专门渠道处理。何淼律师持中加双重执照，在跨境资产追查方面有实战经验。欢迎致电647-930-6688咨询。',
    ],
    metaTitle: '如何查对方的银行账户或资产？| 安省家庭法律师 何淼',
    metaDescription:
      '可申请法院命令让对方出具授权书，律师凭授权书向银行索取资料。需掌握基本信息才能追查，不同银行配合程度不同。647-930-6688。',
    keywords: '查对方银行账户离婚,资产追查离婚,财务披露离婚,何淼律师',
  },
  {
    slug: 'caiwu-pilu',
    capsule: '法院要求提供哪些财务披露？',
    question: '法院会要求提供哪些财务披露？',
    summary: '法院要求的财务披露是全面的，范围是全球性的，包括所有有价值的财产：银行账户、股票投资、RRSP、TFSA、各类奢侈品、国内外房产、基金等。隐瞒任何财产都可能影响案件结果。',
    answers: [
      '法院要求的财务披露是全面的，包括账户、股票投资、RRSP、TFSA、各种奢侈品、国内外房产、基金等所有有价值的财产。',
      '范围是全球性的——加拿大安省的法院要求披露全球范围内的财产，不仅限于加拿大境内。',
    ],
    details: [
      '在安省离婚诉讼中，双方都有全面财务披露的法律义务，这是整个财产分割流程的基础。财务披露的范围非常广泛，包括：所有银行账户、投资账户（股票、基金、债券）、退休储蓄（RRSP）、免税储蓄账户（TFSA）、房产（加拿大境内和境外）、公司权益和股权、人寿保险的现金价值、养老金、各类奢侈品（珠宝、名表、名包、艺术品）等。',
      '特别重要的是：财务披露的范围是全球性的，不局限于加拿大境内的财产。如果您在中国有房产、在其他国家有投资账户，这些都需要披露。隐瞒财产是非常严重的行为，一旦被发现，不仅会影响案件结果，还可能导致法院不利于您的判决，甚至面临藐视法庭的处罚。',
      '财务披露通常通过填写法院规定的财务报表（Financial Statement，Form 13或Form 13.1）来完成，需要列出所有资产、负债、收入和支出的详细信息，并附上相关证明文件。这份文件是宣誓签署的法律文件，任何虚假陈述都有法律后果。',
      '如果您对财务披露的具体要求有疑问，或者担心如何处理复杂的财产情况，建议咨询律师。欢迎致电647-930-6688。',
    ],
    metaTitle: '法院要求提供哪些财务披露？| 安省家庭法律师 何淼',
    metaDescription:
      '财务披露范围是全球性的，包括银行账户、股票、RRSP、TFSA、国内外房产、奢侈品等所有有价值财产。隐瞒任何财产都可能影响案件结果。647-930-6688。',
    keywords: '离婚财务披露,Financial Statement离婚,全球资产披露,何淼律师',
  },
  {
    slug: 'bu-tigong-cailiao',
    capsule: '对方不提供财务材料怎么办？',
    question: '如果对方不提供财务材料怎么办？',
    summary: '可以向法院申请强制披露命令（Motion to Compel）。如果对方在法院命令下仍然不提供，法院可以给对方罚款、不允许对方就相关事项提出异议，甚至直接支持您的主张，相当于判定您赢得该事项。',
    answers: [
      '不用担心，法院会强制对方提供。可以向法院申请Motion to Compel，要求对方必须提供。',
      '如果法院已经命令对方提供，对方依旧不提供，法院可以给对方罚款，不允许对方就相关事项提出异议，甚至直接支持您向法庭主张的请求——也就是判定您赢得该事项。',
    ],
    details: [
      '对方不提供财务材料是一种严重的违反法律义务的行为。在安省，双方都有法律义务进行全面财务披露，拒绝披露会有严重的法律后果。',
      '如果对方拒绝自愿提供财务材料，您可以向法院申请Motion to Compel，要求法院发出强制披露命令。法院通常会批准这类申请，并设定对方提供材料的截止日期。',
      '如果对方在法院命令下仍然拒绝提供，法院可以采取多种惩罚措施：发出罚款命令、不允许对方就未披露的财产事项提出任何主张或异议、以及在极端情况下，直接支持您的立场——也就是说，如果您主张对方有某项资产但对方拒绝披露，法院可能直接按您的主张作出裁决。',
      '对方不提供材料不会让您的案件陷入僵局，只会让对方的处境越来越不利。建议在对方出现这种行为时立即咨询律师，采取正确的法律步骤。欢迎致电647-930-6688。',
    ],
    metaTitle: '对方不提供财务材料怎么办？| 安省家庭法律师 何淼',
    metaDescription:
      '可向法院申请强制披露命令（Motion to Compel）。对方在法院命令下仍不提供，法院可罚款、限制其就相关事项提出异议，甚至直接支持您的主张。647-930-6688。',
    keywords: '对方不提供财务材料,Motion to Compel,强制披露,何淼律师',
  },
  {
    slug: 'hunqian',
    capsule: '婚前协议在安省有法律效力吗？',
    question: '婚前协议在安省有效吗？',
    summary: '有效，但必须满足严格条件：双方自愿签署、各自获得独立法律意见、完整披露财产状况、协议内容公平合理。私下口头或书面达成的婚前协议，在法庭上很可能被认定无效。',
    answers: [
      '<strong>有效，但必须合法合规签署才有效。</strong>私下达成的婚前协议可能被法院认定无效。',
      '法院会审查：双方是否自愿签署、有无受到胁迫、是否做了完整的财产公开、是否完全理解法律内容、有无律师解释、协议条款是否清晰无歧义。任何一项不符合，协议都可能被认定无效。',
    ],
    details: [
      '婚前协议（在安省法律上称为Marriage Contract）如果合法签署，可以在离婚时改变安省《家庭法》的默认规则。但如果签署过程有瑕疵，协议可能在最需要的时候被法院推翻。',
      '安省法院在审查婚前协议时，会重点关注：双方是否完全自愿签署，没有受到任何压力或胁迫；双方是否在签署前做了完整的财产披露；双方是否各自获得了独立的法律意见（ILA）；协议条款是否清晰、无歧义。',
      '最常见的失效原因是：一方没有获得ILA、协议在婚礼前几天仓促签署、财产披露不完整、或者协议条款极度不公平。我们见过不少案件，客户以为自己有一份有效的婚前协议，结果在离婚时被法院认定无效，损失惨重。',
      '正规的婚前协议应该由律师起草，双方各自聘请独立律师审阅并出具ILA证书，在充分考虑后签署。何淼律师从诉讼律师的角度起草婚前协议，会提前识别可能被法院质疑的条款。欢迎致电647-930-6688咨询。',
    ],
    metaTitle: '婚前协议在安省有效吗？| 安省家庭法律师 何淼',
    metaDescription:
      '有效，但需满足严格条件：双方自愿、各自获得独立法律意见、完整披露财产、内容公平合理。口头或私下书面协议在法庭上很可能被认定无效。647-930-6688。',
    keywords: '婚前协议有效吗加拿大,Marriage Contract安省,婚前协议无效条件,婚前协议律师,何淼律师',
  },
  {
    slug: 'fenjuxieyi',
    capsule: '分居协议是什么？有什么好处？',
    question: '什么是分居协议？有什么好处？',
    summary: '分居协议是双方就离婚所有事项——财产分割、子女安排、抚养费——通过协商达成的法律合同。它是最省时、省钱的离婚方式，双方对结果有更多掌控权，避免了法庭诉讼的不确定性和高昂费用。',
    answers: [
      '分居协议是双方通过协商解决离婚争议的法律文件，涵盖财产分割、子女安排、抚养费等所有事项。',
      '分居协议的优势是：花费时间最少、法律费用最低、程序最简单、双方对结果有更多掌控权。对于有条件协商的双方，这是我们最推荐的方式。',
    ],
    details: [
      '分居协议（Separation Agreement）是一份具有法律约束力的合同，由夫妻双方在分居后协商签署，处理离婚涉及的所有事项：婚房和其他资产如何分割、债务如何承担、孩子主要跟谁住、育儿时间如何安排、子女抚养费和配偶赡养费金额等。',
      '分居协议最大的优势是成本和效率。与诉讼相比，协议离婚的律师费通常低得多，时间也短得多。更重要的是，双方对最终结果有更多掌控权——法庭判决的结果往往出乎双方预料，而协议是双方共同达成的，执行起来也更顺畅。',
      '为了确保协议的法律效力，双方应各自聘请独立律师审阅协议内容，并获得独立法律意见（ILA）。协议签署前双方必须完整披露各自的财产状况，隐瞒资产可能导致协议被推翻。',
      '即便双方关系紧张，只要在某些核心问题上能够达成基本共识，通过律师协助的协商往往比直接进入对抗性诉讼更有利。欢迎致电647-930-6688了解详情。',
    ],
    metaTitle: '什么是分居协议？有什么好处？| 安省家庭法律师 何淼',
    metaDescription:
      '分居协议是就财产分割、子女安排、抚养费协商达成的法律合同。最省时省钱，双方对结果有更多掌控权，避免法庭诉讼的不确定性和高昂费用。647-930-6688。',
    keywords: '分居协议加拿大,Separation Agreement安省,分居协议好处,离婚分居协议律师,何淼律师',
  },
  {
    slug: 'kuajing',
    capsule: '离婚时有中国房产或跨境资产怎么处理？',
    question: '离婚时有跨境资产怎么处理？',
    summary: '跨境资产离婚是高度专业化的领域，需要在启动离婚程序前制定完整策略。中国房产、境外账户、海外投资都在安省财产分割的范围内，但追查和执行涉及两国法律，必须由持双重执照的律师处理。',
    answers: [
      '如果双方有跨境资产（如中国房产、境外账户、海外投资），<strong>在提起诉讼前要格外谨慎</strong>。建议在启动离婚程序前，先找律师全面盘点境内外资产，制定好方案，再进行离婚。',
      '何淼律师持中加双重执照，对中国境内资产的法律性质和处理方式有深入了解，是处理跨境资产离婚案件的专业选择。',
    ],
    details: [
      '在安省，婚姻财产均等分割的原则适用于夫妻在婚姻期间积累的所有资产，不论资产在哪个国家。这意味着中国的房产、国内的银行存款、境外的投资账户，理论上都需要纳入财产分割的计算范围。',
      '跨境资产离婚面临的核心挑战有三个：资产的发现和估值、证据的获取（需要通过特定渠道并经认证才能在加拿大法庭使用）、以及判决的执行（安省法院的命令在中国没有自动执行的机制）。',
      '时机和策略至关重要。在对方还没有转移资产之前采取行动，与在资产已经被转移后追讨，难度完全不同。我们处理过对方将婚姻财产转入中国境内试图规避分割的案件，也处理过需要在中国境内追查隐藏资产的案件。',
      '何淼律师持中国律师协会和安省律师公会双重执照，是大多伦多地区极少数能够同时处理中加两国法律问题的律师。如果您的离婚案件涉及跨境资产，强烈建议在启动任何程序前先咨询律师。欢迎致电647-930-6688预约。',
    ],
    metaTitle: '离婚时有跨境资产怎么处理？| 中加双执照律师 何淼',
    metaDescription:
      '中国房产、境外账户、海外投资都在安省财产分割范围内。追查和执行涉及两国法律，必须由持双重执照的律师处理。何淼律师持中加双重执照。647-930-6688。',
    keywords: '跨境资产离婚加拿大,中国房产离婚分割,境外资产离婚安省,跨境财产分割律师,何淼律师',
  },
  {
    slug: 'duojiu',
    capsule: '在安省离婚需要多久？',
    question: '在安省离婚需要多久？',
    summary: '无争议离婚通常需要4至8个月；有争议的诉讼离婚视复杂程度，可能需要1至3年甚至更长。案件涉及的议题越多、双方争议越大，时间越长。尽早聘请律师、确立策略，是缩短时间最有效的方式。',
    answers: [
      '根据安省实际法庭经验，无争议离婚通常需要4至8个月；有争议的诉讼离婚时间因案件复杂程度而异，可能需要1至3年甚至更长。',
      '案件涉及的议题越多，时间越长。只需处理财产分割的案件，比同时涉及财产、子女监护、育儿时间、Child Support和Spousal Support的案件，所需时间短得多。',
    ],
    details: [
      '离婚所需时间受多个因素影响，最关键的是双方争议程度和案件复杂程度。如果双方能够协商一致，签署分居协议，再向法院申请离婚证书，整个过程通常在4至8个月内完成——其中大部分时间是在等法院处理文件。',
      '一旦进入诉讼，时间就难以预测。简单的财产分割案件可能在1年内解决；涉及子女监护争议、跨境资产、商业估值的复杂案件，可能需要2至3年甚至更长。',
      '影响时间的因素包括：双方的配合程度、案件涉及的议题数量、是否需要专家证人（如注册商业估值师CBV）、是否涉及跨境资产需要调查取证等。每增加一个争议点，通常意味着增加数月的时间。',
      '尽早聘请律师是缩短时间最有效的方式。何淼律师每月出庭20小时以上，熟悉安省法院的实际运作节奏。欢迎致电647-930-6688咨询。',
    ],
    metaTitle: '在安省离婚需要多久？| 多伦多华人离婚律师 何淼',
    metaDescription:
      '无争议离婚通常4至8个月；诉讼离婚视复杂程度需1至3年甚至更长。尽早聘请律师确立策略是缩短时间最有效的方式。647-930-6688。',
    keywords: '安省离婚要多久,加拿大离婚时间,无争议离婚多久,离婚诉讼时间安省,何淼律师',
  },
  {
    slug: 'duoshao',
    capsule: '在安省离婚需要多少钱？',
    question: '在安省离婚需要多少钱？',
    summary: '费用因案件类型差异很大。无争议离婚费用相对固定、透明；诉讼离婚按小时计费，最终费用取决于案件复杂程度和双方配合程度。初次咨询30分钟$220+HST，帮您评估大致费用范围。',
    answers: [
      '费用取决于离婚方式：无争议离婚，我们事务所提供套餐价格，费用透明可预期；诉讼离婚通常按小时收费，最终费用取决于案件复杂程度，无法事先精确预估。',
      '建议先预约初次咨询，根据您的案件情况给出大致费用范围。初次咨询30分钟，$220+HST。',
    ],
    details: [
      '离婚费用是客户最关心的问题之一，也是最难给出统一答案的问题，因为费用完全取决于案件的复杂程度和双方的配合程度。大致来说，费用分为两类：法院规费和律师费。法院规费是固定的政府收费，离婚申请的基本法院规费约为200至250加元。律师费则差别很大。',
      '无争议离婚（双方协议一致）是费用最低的方式。我们事务所为此类案件提供套餐收费，费用透明，客户在开始前就知道大概需要付多少。',
      '诉讼离婚的费用则难以事先精确预估，因为它取决于案件涉及多少争议点、对方的配合程度、是否需要专家证人等因素。对方越不配合、案件越复杂，费用越高。',
      '一个重要但常被忽视的点：尽早解决争议通常比拖延更省钱。欢迎致电647-930-6688预约何淼律师的初次咨询。',
    ],
    metaTitle: '在安省离婚需要多少钱？| 律师费用说明 · 何淼律师',
    metaDescription:
      '无争议离婚费用相对固定透明；诉讼离婚按小时计费，取决于案件复杂程度。初次咨询30分钟$220+HST，帮您评估费用范围。647-930-6688。',
    keywords: '安省离婚费用,加拿大离婚律师费,离婚要多少钱,无争议离婚价格,何淼律师',
  },
  {
    slug: 'toronto-lihun-feiyong-duoshao',
    capsule: '多伦多离婚费用大概多少？',
    question: '多伦多离婚费用大概是多少？',
    featured: true,
    pairedFaqPath: '/en/faq/toronto-divorce-cost-how-much/',
    metaTitle: '多伦多离婚费用大概多少｜无争议约5000加元起｜分居协议+离婚申请｜何淼律师',
    metaDescription:
      '多伦多离婚费用没有统一标准：无争议离婚常见路径为先分居协议再离婚申请，整体多可控制在约5000加元量级起；有争议诉讼多为按小时计费，视争议与出庭而定。万锦、列治文山、士嘉堡、多伦多市区华人离婚律师收费差异说明。预约何淼律师647-930-6688。',
    summary:
      '没有统一标准，关键看是无争议还是有争议离婚。无争议通常先分居协议再递交离婚申请，整体费用相对可控，多数情况下约<strong>5,000 加元</strong>左右可完成全流程（视文件复杂度会略有浮动）。有争议进入诉讼的，律师多按小时计费，总费用随争议点、取证与出庭次数拉长而不确定。',
    answers: [
      '无争议离婚：双方对分居、财产、孩子等已达成一致时，流程通常为先起草<strong>分居协议（Separation Agreement）</strong>，再递交<strong>离婚申请（Divorce Application）</strong>。对许多安省华人离婚律师而言属流程化案件，整体成本较可预估，常见区间约<strong>5,000 加元</strong>左右起。',
      '有争议离婚：涉及子女抚养/探视、财产分割争议、对方不同意协议或需走法院程序时，律师费多为<strong>按小时</strong>计费；费率因律师与案情而异，周期被对方拖长时费用会持续累积，事先难以给出精确总价。',
    ],
    details: [
      '「多伦多离婚费用」之所以难一句话回答，是因为在安省（含多伦多、万锦、列治文山、士嘉堡/世嘉堡等）家庭法律师（含华人离婚律师）的收费结构会随<strong>案件类型</strong>差很多：无争议与有争议几乎是两种完全不同的计费逻辑。',
      '<strong>一、无争议离婚（费用相对可控）</strong>：双方已就分居事实、财产分割、子女安排等达成一致时，常见路径是先完成分居协议，再就离婚本身向法院递交申请。此类案件材料与步骤相对标准化，许多律所可以按阶段报价或打包服务，使客户在开始前对区间心里有数。用户常问的「大概多少钱」，在这一类型里才最有意义。',
      '<strong>二、有争议离婚（费用不确定）</strong>：若存在子女抚养权或探视争议、财产分割谈不拢、一方不同意离婚或不同意签署协议、需要多次交换材料与出庭等情形，工作量和时间难以事先框定。实务中，大多伦多地区离婚律师普遍采用小时费率；争议越多、对方越不配合、程序越多，账单越高。',
      '<strong>三、为什么诉讼型离婚很难提前估算？</strong>：争议点多会直接拉长律师工作时间；披露、书面材料、动议、庭前会议与庭审次数都会推高成本。在安省家庭法框架下，即使您一方希望尽快收尾，只要对方继续争议，程序仍可能持续推进，费用随之累积。',
      '<strong>总结</strong>：可把多伦多离婚费用简单理解为——无争议：相对可控，约<strong>5,000 加元</strong>左右起（以分居协议+离婚申请为常见组合，具体仍视文件与资产结构而定）；有争议：取决于复杂程度与诉讼进展，无法在未了解案情前承诺固定总价。',
      '因此，咨询多伦多华人离婚律师时，比只问「多少钱」更重要的是先厘清：您的案件更接近无争议协议路径，还是已进入或可能进入争议诉讼。案件结构越清晰，费用区间才越可预估。欢迎致电<strong>647-930-6688</strong>预约何淼律师，在了解事实与目标后讨论可行路径与预算区间。',
    ],
    keywords:
      '多伦多离婚费用,多伦多离婚多少钱,多伦多离婚成本,GTA离婚费用,大多伦多离婚律师费,多伦多华人离婚律师费用,普通话离婚律师多伦多,粤语家庭法律师多伦多,无争议离婚多伦多费用,多伦多分居协议费用,多伦多离婚申请费用,分居协议加离婚申请多少钱,安省离婚费用多伦多,万锦离婚费用,万锦华人离婚律师费,列治文山离婚费用,列治文山华人律师,士嘉堡离婚律师费,世嘉堡离婚费用,北约克离婚律师,旺市离婚费用,密西沙加离婚费用,多伦多Downtown离婚律师,安省家庭法按小时收费,有争议离婚律师费多伦多,离婚官司多少钱多伦多,子女抚养争议律师费,财产分割离婚费用,何淼律师,何淼律师多伦多,Miao He Toronto,6479306688',
  },
  {
    slug: 'lushi-fei-duoshao',
    capsule: '在安省请离婚律师大概多少钱？',
    question: '在安省请离婚律师大概多少钱？',
    summary: '以何淼律师事务所为例：无争议离婚提供套餐价格，通常在$2,000至$5,000不等。有争议的诉讼离婚按小时收费，如案件复杂、持续时间长，一年的律师费可能在$5万至$8万加币。',
    answers: [
      '以我们律所为例：无争议离婚提供套餐价格，通常在$2,000至$5,000不等，根据不同人的情况会有差异。',
      '有争议的离婚诉讼，加拿大安省的离婚律师收费按小时计费。如果案件复杂、持续时间久，一年的律师费可能在$5万至$8万加币。',
    ],
    details: [
      '不同律所的收费标准不同，以下以何淼律师事务所为例提供参考。无争议离婚（双方已协商一致）我们提供套餐价格，而不是按小时计费，让客户在开始前就知道大概花多少钱。根据不同案件的复杂程度，套餐价格通常在$2,000至$5,000之间。',
      '有争议的离婚诉讼则按小时收费。案件越复杂、时间越长，总费用越高。如果案件持续一年以上并涉及多次出庭，律师费达到$5万至$8万加币并不罕见，在特别复杂的案件中可能更高。',
      '根据何律师的经验，很多客户在协商阶段拖延太久，最终没有签成分居协议，但律师费已经花了不少。在这种情况下，往往是有一方在恶意拖延。如果协商迟迟没有进展，应该尽早与律师评估是否转入诉讼程序，而不是继续无休止地谈判。',
      '欢迎致电647-930-6688预约初次咨询，我们会根据您的具体情况给出大致费用范围和建议。',
    ],
    metaTitle: '在安省请离婚律师大概多少钱？| 多伦多华人离婚律师 何淼',
    metaDescription:
      '无争议离婚套餐通常$2,000至$5,000。有争议的诉讼离婚按小时收费，案件复杂时一年律师费可能在$5万至$8万加币。初次咨询30分钟$220+HST。647-930-6688。',
    keywords: '离婚律师费用,安省离婚律师收费,离婚律师多少钱,何淼律师',
  },
  {
    slug: 'shui-fu-lushifei',
    capsule: '谁来承担律师费？可以让对方付吗？',
    question: '谁来承担律师费？可以让对方付吗？',
    summary: '无争议离婚由双方自行协商谁付律师费。有争议的诉讼中，胜诉方可以申请让败诉方承担部分律师费（Costs Award），但法院有固定的计算标准，通常无法100%追回所有律师费。',
    answers: [
      '无争议离婚中，双方自行协商谁来支付律师费用。有争议的离婚诉讼中，获得胜利的一方可以请求败诉方支付部分律师费。',
      '但要知道，Costs Award是有比例的，并不能100%拿回所有费用。例如如果离婚花了$20万律师费，不能期待把这$20万全部让对方承担。',
    ],
    details: [
      '在无争议离婚中，律师费通常由各自承担，双方也可以在分居协议中约定由一方支付全部或部分律师费。',
      '在有争议的诉讼中，法院有权发出Costs Order，要求败诉方支付胜诉方的部分律师费。这个制度的目的是惩罚无理取闹的一方，鼓励双方合理解决争议。如果对方在诉讼中提出不合理的主张、拒绝接受合理的和解提议、或者行为不当，法院更有可能发出对对方不利的Costs Order。',
      '但是，Costs Award通常不是100%的律师费报销。法院有自己的计算标准，通常分为"实质性费用（Substantial Indemnity）"和"部分费用（Partial Indemnity）"两种，前者约为实际律师费的90%，后者约为60%。即便获得了Costs Order，实际收回的金额可能与实际支出有差距。',
      '在诉讼策略上，了解Costs Award的运作方式非常重要。欢迎致电647-930-6688与何淼律师咨询。',
    ],
    metaTitle: '谁来承担律师费？可以让对方付吗？| 安省离婚律师 何淼',
    metaDescription:
      '无争议离婚由双方协商。诉讼中胜诉方可申请让败诉方承担部分律师费（Costs Award），但法院有固定计算标准，通常无法100%追回全部律师费。647-930-6688。',
    keywords: '律师费谁承担,Costs Award离婚,对方付律师费,何淼律师',
  },
  {
    slug: 'daguan-haishi-xieyi',
    capsule: '离婚打官司和协议离婚哪个好？',
    question: '离婚打官司和协议离婚哪个好？',
    summary: '如果双方可以协商，协议离婚更经济、更快、对双方伤害更小。但如果一方在恶意拖延协商，要及时识别并转入诉讼，不要让对方利用拖延转移资产或占据有利位置。',
    answers: [
      '如果双方可以协商，当然是协议离婚更好：经济、时间短、速度快、给双方带来的伤害更小。',
      '但很多人无法达成协议。在双方无法签分居协议时，不要恋战，不要把协商时间拖得过长。根据何律师的经验，有客人协商了两年都没签成，但律师费一分没少花，最后什么结果都没有，还浪费了宝贵时间。',
    ],
    details: [
      '协议离婚（无争议离婚）在所有方面都占优：费用低、时间短、结果可以由双方掌控、对孩子和双方的心理伤害也小。如果有可能协商，应该优先尝试。',
      '但现实是，很多离婚案件无法通过协商解决，要么是双方对某些核心事项无法达成一致，要么是一方根本没有诚意谈判，只是在拖延时间。根据何律师的经验，有一个客户与对方协商了整整两年，两年里双方律师来回谈判，最终分居协议一直没能签成。两年的律师费花出去了，对方也没有做出任何实质性让步，反而利用这段时间暗中处置了部分资产。',
      '如果您发现协商陷入僵局，对方明显在拖延，要及时与律师评估是否应该转入诉讼。主动提起诉讼不一定代表要"打到底"，很多案件在进入诉讼程序后，反而因为诉讼压力促成了双方最终达成和解。',
      '如何判断是协议还是诉讼，需要根据您的具体情况来决定。欢迎致电647-930-6688与何淼律师咨询，我们帮您评估最适合您的方案。',
    ],
    metaTitle: '离婚打官司和协议离婚哪个好？| 安省离婚律师 何淼',
    metaDescription:
      '双方可协商则协议离婚更经济快速、伤害更小。但若一方恶意拖延，要及时识别并转入诉讼，避免对方利用拖延转移资产或占据有利位置。647-930-6688。',
    keywords: '离婚打官司还是协议,协议离婚优缺点,离婚诉讼,何淼律师',
  },

  // ── 分居与离婚系列（2026-04-19）──────────────────────────────
  {
    slug: 'fenju-jiushi-lihun-ma',
    capsule: '分居就是离婚吗？',
    question: '分居就是离婚吗？',
    summary:
      '不是。分居（Separation）和离婚（Divorce）是完全不同的两件事。分居是事实行为——夫妻事实上停止共同生活，不需要法院程序，不需要任何文件；离婚是法律程序——必须向加拿大法院申请并获得离婚判决书才生效。',
    answers: [
      '<strong>分居是事实行为</strong>——夫妻双方事实上停止共同生活，不需要法院程序，不需要任何文件。',
      '<strong>离婚是法律程序</strong>——必须向加拿大法院申请，并获得<strong>离婚判决书（Divorce Certificate）</strong>才生效。',
    ],
    details: [
      '分居后，双方在法律上<strong>仍然是夫妻</strong>：不能再婚、仍享有配偶权利义务、分居状态不会自动变成离婚。只有经过法院程序拿到离婚证，婚姻关系才彻底终止。',
      '很多华人客户误以为"分居了就等于离婚了"。这是一个代价极高的误解——在法律上仍处于已婚状态，不仅无法再婚，财产界定也会变得更复杂。',
      '如果您想正式结束婚姻关系，必须通过<a href="/family-law/divorce-litigation">离婚诉讼</a>程序向法院提出申请。如果双方对财产、抚养等事项有共识，也可以先签订<a href="/family-law/domestic-contracts#separation">分居协议</a>，为后续离婚做好准备。',
      '完整解析请阅读：<a href="/blog/separation-vs-divorce-ontario/">《分居和离婚的区别详解》</a>。欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '分居就是离婚吗？| 安省离婚律师 · 万锦华人家庭法',
    metaDescription:
      '不是。分居是事实行为，无需法院程序；离婚是法律程序，必须向法院申请并获得判决书才生效。两者法律后果完全不同。647-930-6688。',
    keywords: '分居就是离婚吗,分居和离婚的区别,分居法律后果安省,离婚判决书加拿大,何淼律师',
    pairedFaqPath: '/en/faq/is-separation-the-same-as-divorce/',
    featured: true,
  },
  {
    slug: 'fenju-yinian-zidong-lihun',
    capsule: '分居一年就自动离婚了吗？',
    question: '分居一年就自动离婚了吗？',
    summary:
      '不会。加拿大没有"自动离婚"这一说。无论分居多久——1年、5年、甚至10年——只要不主动向法院申请离婚，婚姻关系永远存在。分居满1年只是让您有资格申请离婚，不代表自动离婚。',
    answers: [
      '加拿大《<a href="https://laws-lois.justice.gc.ca/eng/acts/d-3.4/"><em>Divorce Act</em></a>》规定：<strong>分居满1年</strong>是申请离婚最常见的理由（"婚姻破裂"）。但这只是让您<strong>有资格申请离婚</strong>，不代表自动离婚。',
      '必须主动向加拿大省级高等法院提交<a href="/family-law/divorce-litigation">离婚申请</a>，经过法院审理并获得离婚判决书，婚姻才正式终止。',
    ],
    details: [
      '何淼律师曾代理过一位<strong>分居10年</strong>仍未离婚的客户。10年里他一直认为"时间长了就自动离了"——结果法律上仍是已婚状态，无法再婚，财产界定极为复杂。',
      '分居满1年后，如需启动离婚程序，需要准备分居日证明、财产清单等材料，向法院提交申请，等待法院审理，最终获得离婚判决书。',
      '"分居久 = 自动离婚"是代价高昂的误解。建议在分居满一年后尽早咨询律师，了解<a href="/family-law/divorce-litigation">离婚申请</a>流程与所需文件。',
      '完整解析请阅读：<a href="/blog/separation-vs-divorce-ontario/">《分居和离婚的区别详解》</a>。欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '分居一年就自动离婚了吗？| 安省离婚律师 何淼',
    metaDescription:
      '不会。加拿大没有自动离婚。无论分居多久，不主动向法院申请，婚姻关系永远存在。分居满一年只是有资格申请离婚，不代表自动离婚。647-930-6688。',
    keywords: '分居一年自动离婚,加拿大自动离婚,分居多久可以离婚,Divorce Act分居满一年,何淼律师',
    pairedFaqPath: '/en/faq/separation-one-year-automatic-divorce/',
    featured: true,
  },
  {
    slug: 'fenju-zhihou-fanhou-hehao',
    capsule: '分居之后可以反悔和好吗？',
    question: '分居之后可以反悔和好吗？',
    summary:
      '可以，但时间有讲究。加拿大法律认可夫妻分居后重新和好（reconciliation）。关键规则是：重新共同生活不超过90天，原分居日不变；超过90天，之前的分居期被打断，未来如再次分手需重新计算分居日。',
    answers: [
      '可以。加拿大法律认可夫妻分居后<strong>重新和好（reconciliation）</strong>。',
      '关键是<strong>90天规则</strong>：重新共同生活不超过90天，分居日继续有效；超过90天，之前的分居期被打断，未来如再次分手需<strong>重新计算分居日</strong>。',
    ],
    details: [
      '加拿大《<a href="https://laws-lois.justice.gc.ca/eng/acts/d-3.4/"><em>Divorce Act</em></a>》第8(3)条给出明确标准：短暂复合（不超过90天）不会重置分居期；超过90天被视为关系已修复，之前的分居期被打断。',
      '分居日直接影响：财产分割的计算基准日、申请离婚的时效起算、<a href="/family-law/domestic-contracts#separation">分居协议</a>的适用范围。这些都是直接关系到您财产权益的关键节点。',
      '建议保留短信、邮件、独立账户等可证明分居日的证据。如果曾经短暂复合又再次分居，及时咨询律师确认分居日是否需要重新认定。',
      '完整解析请阅读：<a href="/blog/separation-vs-divorce-ontario/">《分居和离婚的区别详解》</a>。欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '分居之后可以反悔和好吗？| 安省家庭法律师 何淼',
    metaDescription:
      '可以，但有时间规则。重新共同生活不超过90天，原分居日不变；超过90天则之前的分居期被打断，未来再次分手需重新计算分居日。647-930-6688。',
    keywords: '分居后可以和好吗,90天复合规则加拿大,分居日重置,reconciliation Divorce Act,何淼律师',
    pairedFaqPath: '/en/faq/can-you-reconcile-after-separation/',
    featured: true,
  },
  {
    slug: 'lihun-zhihou-keyi-fanhui',
    capsule: '离婚之后可以反悔吗？',
    question: '离婚之后可以反悔吗？',
    summary:
      '法律上，不可以。一旦拿到离婚判决书（Divorce Certificate），婚姻关系在法律上彻底终止。如果之后想恢复夫妻关系，必须重新办理结婚登记——与陌生人结婚是同样的程序。',
    answers: [
      '不可以。一旦拿到<strong>离婚判决书（Divorce Certificate）</strong>，婚姻关系在法律上<strong>彻底终止</strong>。',
      '如果想复合，<strong>必须重新办理结婚登记</strong>。加拿大法律下没有"复婚证"，只有一张新的"结婚证"——与陌生人结婚是同样的程序。',
    ],
    details: [
      '这与分居完全不同：分居可以通过重新共同生活超过90天"反悔"；但离婚一旦完成，无论双方之后关系如何改善，法律上的婚姻关系已不存在。',
      '如果您对婚姻仍有不确定，请在递交<a href="/family-law/divorce-litigation">离婚申请</a>前慎重考虑。一旦法院发出离婚判决，不可逆转。',
      '现实中也有夫妻离婚后决定复合，他们的选择是重新举行婚礼、重新办理结婚登记，这与初次结婚程序完全相同。',
      '完整解析请阅读：<a href="/blog/separation-vs-divorce-ontario/">《分居和离婚的区别详解》</a>。欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '离婚之后可以反悔吗？| 安省离婚律师 何淼',
    metaDescription:
      '法律上不可以。拿到离婚判决书后婚姻关系彻底终止。如之后想恢复夫妻关系，必须重新办理结婚登记，与第一次结婚程序相同。647-930-6688。',
    keywords: '离婚后可以复婚吗,离婚反悔加拿大,离婚判决书不可逆,复婚程序加拿大,何淼律师',
    pairedFaqPath: '/en/faq/can-you-reconcile-after-divorce/',
    featured: true,
  },
  {
    slug: 'tongwuyan-fenju',
    capsule: '同一个屋檐下可以分居吗？',
    question: '同一个屋檐下可以分居吗？',
    summary:
      '可以。加拿大安省法院认可"同屋分居"（Separation Under One Roof）。法律不强求物理上分开住，而是看双方是否实质上停止了夫妻生活——分房睡、财务独立、生活分开、社交独立等。同屋分居法律效力与常规分居相同，但举证更难，建议留好证据。',
    answers: [
      '可以。加拿大安省法院认可<strong>"同屋分居"（Separation Under One Roof）</strong>，不强求物理上分开居住。',
      '关键看<strong>是否实质上停止了夫妻生活</strong>：分房睡、财务独立、生活分开、社交独立、对外表态等。不需要全部满足，但越多越能证明实质分居。',
    ],
    details: [
      '现实中很多夫妻因为经济压力、孩子抚养、房屋产权等原因，分居时无法立刻分开居住。加拿大法律充分考虑到这一现实，不以"是否搬出"作为分居的判断标准。',
      '法院常见的认定因素包括：分房睡、不再有夫妻亲密关系；财务独立——分别管理账户、各自承担开支；生活分开——不一起吃饭、购物、做家务；社交独立——不以夫妻身份出席活动；对外表态——告知家人朋友双方已分居。',
      '"同屋分居"在法律上完全等同于常规分居，分居日一样重要，<a href="/family-law/domestic-contracts#separation">分居协议</a>一样适用。但因为双方仍住在一起，举证比"搬出去住"更难——建议通过<a href="/blog/electronic-evidence-ontario-family-court/">短信、邮件等电子记录</a>保留分居证据。',
      '完整解析请阅读：<a href="/blog/separation-vs-divorce-ontario/">《分居和离婚的区别详解》</a>。欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '同一个屋檐下可以分居吗？| 安省家庭法律师 何淼',
    metaDescription:
      '可以。安省法院认可同屋分居，关键是双方是否实质上停止夫妻生活——分房睡、财务独立、生活分开。同屋分居法律效力与常规分居相同，但举证更难，建议留好证据。647-930-6688。',
    keywords: '同屋分居安省,separation under one roof,同住可以分居吗,分居证据保留,何淼律师',
    pairedFaqPath: '/en/faq/separation-under-one-roof/',
    featured: true,
  },
  {
    slug: 'gongqing-yu-dongfalv-tongyang-zhongyao',
    capsule: '家庭纠纷带温度；听懂你「为什么」要打，才能精准争取核心利益。',
    question: '在安省找家庭法律师，为什么「能共情」和「懂法律」同样重要？',
    summary:
      '法条是框架，但家庭纠纷往往带着强烈情绪与具体处境。律师若不能与你共情，就很难真正理解你为何坚持某项诉求、为何在此案不能退让；只有听懂「为什么」，才能把法律工具对准你最核心的利益。懂法是底线，能结合情感深度理解客户，才能在谈判与庭审中成为更可靠的后盾。',
    answers: [
      '法律条文是冰冷的，但家庭纠纷是带着温度和血泪的。若律师不能与你共情，就很难站在你的立场理解：你为何坚持某项诉求、为何在此案不能退让。',
      '只有听懂你「为什么」要打这个案子，律师才能更精准地为你争取最核心的利益。懂法律只是及格线；能从情感深度理解客户，往往在谈判与法庭上更能形成有效策略。',
    ],
    details: [
      '法律条文是冰冷的，但家庭纠纷是带着温度和血泪的。如果一个律师不能与你共情，他就无法真正站在你的立场去理解：你为何坚持要某个特定的诉求？你为何在这个案子里不能退让？只有听懂了你「为什么」要打这个案子，律师才能精准地为你争取到最核心的利益。懂法律只是及格线，能从情感深度理解客户，才能在谈判和法庭上成为你最坚实的后盾。',
      '想了解更多关于「有温度的法律服务」与选律师的思路，请阅读何淼律师的<a href="/blog/how-to-choose-markham-family-lawyer/">《如何选好的万锦家庭法律师：在安省，找一个能听懂你委屈、更能帮你拿回权益的人》</a>。您也可以对照<a href="/blog/how-to-choose-toronto-chinese-divorce-lawyer/">《如何挑选多伦多最好的华人离婚律师？》</a>中关于面谈与匹配的段落。',
      '欢迎致电647-930-6688或通过微信372656035预约咨询。',
    ],
    metaTitle: '安省找家庭法律师，为什么共情和懂法同样重要？| 何淼律师',
    metaDescription:
      '律师若不能共情，就难以真正理解你为何坚持某项诉求。只有听懂「为什么」，才能把法律工具对准你最核心的利益。懂法是底线，深度理解客户才是真正的后盾。647-930-6688。',
    keywords: '安省家庭法律师,共情与法律,选家庭法律师,万锦家庭法律师,有温度的法律服务,何淼律师',
    pairedFaqPath: '/en/faq/empathy-and-law-both-matter-ontario-family-lawyer/',
    featured: true,
  },
  {
    slug: 'lushi-weishenme-you-shi-dazhuan-chenshu',
    capsule: '打断未必是不尊重：听完全貌后的取舍，与从头到尾不耐烦，是两回事。',
    question: '为什么我的律师有时会打断我的陈述？这是否意味着他不尊重我？',
    summary:
      '不一定。关键看打断发生在什么阶段：若律师已完整了解故事脉络，之后在沟通中打断，往往是为了过滤与案件无关的信息、节省律师费并聚焦有效争点；若从头到尾缺乏耐心、频繁打断，则可能忽略全貌甚至遗漏细节中的关键证据。真正的专业，是「听完」之后的精准取舍。',
    answers: [
      '这取决于律师打断的时机。若律师已经完整听过你的故事全貌，之后在具体沟通中打断你，通常是为了帮你过滤掉与案件无关的信息，节省律师费，并专注于最有力的法律争点与证据。',
      '但若律师从头到尾都没耐心听你讲完就频繁打断，这可能意味着他忽略了案件的全貌，甚至可能漏掉隐藏在细节里的关键证据。真正的专业，是「听完」后的精准取舍，而不是一开始的冷漠打断。',
    ],
    details: [
      '若律师已经完整听过你的故事全貌，之后在具体沟通中打断你，通常是为了帮你过滤掉与案件无关的信息，节省你的律师费，并专注于最有力的法律武器（争点、证据与程序步骤）。',
      '但如果律师从头到尾都没耐心听你讲完，就频繁打断，这可能意味着他忽略了案件的全貌，甚至可能漏掉隐藏在细节里的关键证据。',
      '这与何淼律师在<a href="/blog/how-to-choose-markham-family-lawyer/">《如何选好的万锦家庭法律师…》</a>一文中讨论的「接住情绪、同时管理咨询时间」并不矛盾：专业倾听不等于无边界重复叙述，而是把有限时间用在能改变结果的信息上。关于按小时计费下成本如何累积，可结合<a href="/faq/lushi-fei-duoshao/">在安省请离婚律师大概多少钱？</a>一并理解。',
      '欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    metaTitle: '律师打断我的陈述，是不尊重我吗？| 安省家庭法律师',
    metaDescription:
      '不一定。律师已了解全貌后的打断，是为了过滤无关信息、节省律师费并聚焦有效争点。从头频繁打断才可能遗漏关键证据。真正的专业是「听完」后的精准取舍。647-930-6688。',
    keywords: '律师打断陈述,按小时计费咨询,家庭法律师沟通,万锦家庭法律师,何淼律师',
    pairedFaqPath: '/en/faq/lawyer-interrupts-during-family-law-consult/',
    featured: true,
  },
  {
    slug: 'fating-zhengju-yu-qingxu-zaoyin',
    capsule: '情绪表达与可采信证据不同；涉及家暴或人身伤害时，「感受」也可能成为关键证据。',
    question: '在家庭法案件中，如何区分「法庭证据」与「情绪噪音」？',
    summary:
      '「情绪噪音」多是缺乏客观佐证的纯主观感受，法官难以单独采信；「法庭证据」则是能落到书面记录、转账凭证、第三方证言等可检验事实上的材料。经验丰富的律师会在倾听时追问可转化细节。注意：情感伤害并非全是噪音——若涉及家庭暴力或人身伤害，可能构成刑事或家庭法上的重要证据，需要律师具备扎实的实务判断力。',
    answers: [
      '「情绪噪音」通常是纯粹的个人感受（如「我不开心」「我觉得很受伤」），若没有客观材料支持，法官很难单独采信。',
      '「法庭证据」往往体现为可检验的事实：是否有书面记录、转账凭证、第三方人证、医疗或报警材料等。经验丰富的律师会在听你诉说时敏锐追问这些细节。',
    ],
    details: [
      '这是一个极其考验律师经验的过程。情绪噪音通常是纯粹的个人感受，如「我不开心」「我觉得很受伤」，若没有客观支持，法官很难采信。',
      '法庭证据则要求能把叙述对应到可提交、可质证的材料上。经验丰富的律师会在听你诉说时，敏锐地追问：「是否有书面记录？」「是否有转账凭证或第三方人证？」这些能转化的细节才是核心。',
      '特别注意：情感伤害并非全是噪音。如果涉及家庭暴力（Family Violence）或人身伤害，这就可能上升到刑事或家庭法上的重要证据。区分这两者，需要律师具备深厚的实战洞察力。',
      '关于电子记录与书面材料在庭审中的运用，可参考<a href="/blog/electronic-evidence-ontario-family-court/">《安省家庭法庭上的电子证据》</a>。何淼律师在<a href="/blog/how-to-choose-markham-family-lawyer/">《如何选好的万锦家庭法律师…》</a>中也谈到如何把「有用的声音」带到法庭。欢迎致电647-930-6688预约咨询。',
    ],
    metaTitle: '家庭法案件中如何区分法庭证据与情绪噪音？| 安省家庭法律师 何淼',
    metaDescription:
      '情绪噪音是缺乏客观佐证的主观感受；法庭证据是书面记录、转账凭证、第三方证言等可检验事实。家庭暴力或人身伤害可能构成重要证据，需律师具备扎实实务判断。647-930-6688。',
    keywords: '家庭法证据,情绪噪音,法庭证据,家庭暴力证据,安省家庭法,何淼律师',
    pairedFaqPath: '/en/faq/court-evidence-vs-emotional-noise-family-law/',
    featured: true,
  },
  {
    slug: 'an-sheng-xiaoshoufei-kongzhi-susong-chengben',
    capsule: '省钱的关键在首次咨询：先对齐策略与走向，再启动委托；避免重复沟通同一问题。',
    question: '安省律师按小时计费，我该如何有效控制自己的诉讼成本？',
    summary:
      '秘诀在于「首次咨询的深度」：不必等案子打到一半才想省钱。第一次咨询时就应与律师充分讨论办案思路、预判走向并共同拟定初步策略，双方对战略达成共识后再启动委托。最费钱的是反复沟通相同问题；在战略一致的前提下高效推进谈判或诉讼，才是最合理的省钱之道。',
    answers: [
      '秘诀在于<strong>「首次咨询的深度」</strong>。你不必等到案子打到一半再想怎么省钱。在第一次咨询时，你就应该与律师深入探讨办案思路、预判案件走向并共同制定初步诉讼策略；只有当你和律师在策略上达成共识后，再启动委托，往往更省时间与账单。',
      '最费钱的做法是「反复沟通相同的问题」。在双方达成战略一致的基础上，高效、有针对性地推进谈判或诉讼，才是最合理的省钱之道。',
    ],
    details: [
      '你不必等到案子打到一半再想怎么省钱。在第一次咨询时，你就应该与律师深入探讨办案思路、预判案件走向并共同制定初步诉讼策略。只有当你和律师在策略上达成共识后，再启动委托，后续工作才更容易「一次做对方向」。',
      '最费钱的做法是反复沟通相同的问题。在双方达成战略一致的基础上，高效、有针对性地推进谈判或诉讼，才是最合理的省钱之道。',
      '想知道我们如何帮客户把沟通集中在「能改变结果的事项」上、并优化诉讼成本思路，可阅读<a href="/blog/how-to-choose-markham-family-lawyer/">《如何选好的万锦家庭法律师…》</a>。费用区间与变量还可对照<a href="/faq/lushi-fei-duoshao/">在安省请离婚律师大概多少钱？</a>、<a href="/faq/toronto-lihun-feiyong-duoshao/">多伦多离婚费用大概是多少？</a>。',
      '欢迎致电647-930-6688预约咨询。',
    ],
    metaTitle: '安省律师按小时计费，如何控制诉讼成本？| 安省家庭法律师 何淼',
    metaDescription:
      '秘诀在于首次咨询的深度。第一次咨询就与律师讨论办案思路、预判走向并拟定策略。最费钱的是反复沟通相同问题，战略一致后高效推进才是最合理的省钱之道。647-930-6688。',
    keywords: '安省律师按小时计费,诉讼成本控制,首次咨询策略,家庭法律师费用,何淼律师',
    pairedFaqPath: '/en/faq/control-litigation-costs-hourly-ontario/',
    featured: true,
  },
  {
    slug: 'markham-wai-pinren-an-sheng-jiatingfa',
    capsule: '持安省执照可代理全省家庭法事务；远程会议、电子递交等使距离不再是障碍。',
    question: '如果我住在万锦（Markham）以外，还能聘请你们处理安省的案子吗？',
    summary:
      '可以。只要律师持有安省律师执照，即可依法代理全省范围内的家庭法业务。无论您身处多伦多、渥太华、伦敦还是温莎，通过远程视频会议、电子签名及安省法院电子递交等方式，通常都能获得与万锦本地客户同等质量的专业服务。法律的公正覆盖全省，专业服务亦可覆盖全省（个案是否接受以利益冲突与时间表等为准）。',
    answers: [
      '完全可以。只要持有安省律师执照，我们就可以依法代理全省范围内的家庭法业务。',
      '无论您身处多伦多、渥太华、伦敦还是温莎，距离早已不再是法律服务的障碍。通过远程视频会议、电子签名系统以及安省法院的电子递交平台，我们可以为您提供与万锦本地客户一致的高质量、深度服务。',
    ],
    details: [
      '只要持有安省律师执照，我们就可以依法代理全省范围内的家庭法业务。',
      '无论您身处多伦多、渥太华、伦敦还是温莎，距离早已不再是法律服务的障碍。通过远程视频会议、电子签名系统以及安省法院的电子递交平台，我们可以为您提供与万锦本地客户一致的高质、深度服务。',
      '法律的公正覆盖全省，我们的专业服务亦然。是否建立律师—客户关系仍需个案评估（例如利益冲突核查、时间表与案件类型等）。何淼律师在<a href="/blog/how-to-choose-markham-family-lawyer/">《如何选好的万锦家庭法律师…》</a>结语中也说明了服务范围与个案评估的关系。',
      '欢迎致电647-930-6688，说明您的居住地与案件概况，我们再讨论是否适合建立代理关系。',
    ],
    metaTitle: '住在万锦以外，可以聘请何淼律师处理安省案件吗？| 家庭法律师',
    metaDescription:
      '可以。持安省律师执照可代理全省家庭法业务。通过远程视频、电子签名及安省法院电子递交，与万锦本地客户获得同等质量的专业服务。647-930-6688。',
    keywords: '万锦以外聘请律师,安省家庭法全省代理,远程法律咨询安省,Markham律师安省,何淼律师',
    pairedFaqPath: '/en/faq/outside-markham-hire-ontario-family-lawyer/',
    featured: true,
  },
  {
    slug: 'book-consultation-miao-he',
    capsule: '致电或邮件预约；30 分钟初次咨询 $220+HST；普通话或英文；万锦面谈或视频。',
    question: '如何预约何淼律师的咨询？',
    summary:
      '请致电647-930-6688或发邮件至 miao@miaohelaw.ca。初次咨询为30分钟固定费用$220+HST，可使用普通话或英文，在万锦办公室面谈或通过视频会议。咨询后无强制委托义务。',
    answers: [
      '致电647-930-6688或发邮件至 miao@miaohelaw.ca，即可预约30分钟初次咨询，费用为$220+HST。',
      '咨询可使用普通话或英文；可在万锦办公室（208-7050 Woodbine Ave）当面进行，也可通过视频会议远程进行。',
    ],
    details: [
      '何淼律师提供30分钟初次咨询，固定收费$220+HST。您可根据需要选择普通话或英文沟通。',
      '面谈地点为恒律师事务所（H. LAW FIRM）办公室：安大略省万锦市 Woodbine Avenue 7050号208室；若您不便到场，也可预约视频会议。',
      '初次咨询旨在帮助您了解自身法律处境：享有哪些权利、程序大致如何、有哪些可行选项。咨询结束后，您没有必须继续委托律师的义务。',
      '何淼律师服务大多伦多及安省各地客户，持安省律师执照（LSO #83315K）与中国律师资格，擅长处理涉及加拿大与中国因素的家庭法事务。',
      '预约请致电647-930-6688或发邮件至 miao@miaohelaw.ca；您也可通过本站<a href="/contact/">联系我们</a>页面提交留言。',
    ],
    metaTitle: '如何预约何淼律师的咨询？| 多伦多华人家庭法律师',
    metaDescription:
      '致电647-930-6688或发邮件至miao@miaohelaw.ca。初次咨询30分钟$220+HST，普通话或英文，万锦办公室面谈或视频会议均可，咨询后无强制委托义务。647-930-6688。',
    keywords: '预约何淼律师,何淼律师咨询,万锦家庭法律师咨询,初次咨询费用,647-930-6688',
    pairedFaqPath: '/en/faq/book-consultation-miao-he/',
  },
  {
    slug: 'xianzhiling-yimin-gongqian-fengye',
    capsule:
      '警方临时「不准接触」≠刑事定罪；家事法庭民事限制令本身也不等于有罪判决。是否影响工签、枫叶卡等须看后续是否进入刑事程序及个案情节。',
    question: '如果我申请了限制令，会影响对方的移民身份吗？（例如工签或枫叶卡）',
    summary:
      '一般而言：仅警方在调查或释放阶段作出的临时不准接触安排，本身不等于刑事定罪，通常<strong>不会单独据此</strong>自动撤销对方的工签或枫叶卡。家事法庭的民事限制令（Restraining Order）同样是<strong>民事救济</strong>，不等于对方已被判刑。若之后出现<strong>刑事定罪</strong>，是否触发移民法上的不利后果要<strong>依罪名、刑期、记录及个案</strong>判断，并非「只要定罪就一定会被取消工签或枫叶卡」。涉及在留身份的具体策略，应同时咨询<strong>持牌移民顾问或移民律师</strong>。',
    answers: [
      '若只是警方给出的临时「不准接触」类安排（常见于释放条件等），性质是<strong>在刑事程序进行期间限制双方接触</strong>，并不代表法院已作出<strong>刑事有罪判决</strong>，因此<strong>通常不会单凭这一点</strong>就等同于移民身份必然被撤销。',
      '您在<strong>家事法庭</strong>申请的民事<strong>限制令（Restraining Order）</strong>，由法官依据家庭法审查证据后签发，属于<strong>民事保护</strong>范畴；对方是否会被移民部门采取不利措施，与<strong>是否构成刑事定罪、以及具体罪名与情节</strong>不是同一回事。',
      '退一步说，即便将来出现刑事定罪，是否会影响工签、学签、枫叶卡或入籍申请，也要看<strong>罪名类别、量刑、是否属于严重刑事犯罪（serious criminality）等</strong>综合判断，<strong>不是</strong>「只要定罪就必然吊销签证」的固定公式。',
    ],
    details: [
      '许多当事人会把三件事混在一起：<strong>（1）警方临时限制接触</strong>、<strong>（2）家事法庭民事限制令</strong>、以及<strong>（3）刑事法院的有罪判决</strong>。三者法律性质不同。您申请或持有民事限制令，核心是向家庭法庭说明<strong>合理恐惧与安全风险</strong>，并不自动替对方「定罪」。',
      '警方在调查、逮捕或释放等环节要求双方不得联系，往往是为了<strong>降低冲突升级风险、保全现场与证据</strong>。在<strong>尚未形成有罪判决</strong>之前，这类安排本身一般<strong>不会单独构成</strong>移民法意义上「因某罪已定必吊销身份」的结论——但个案仍可能因<strong>并行程序、边境询问、后续指控</strong>等出现复杂情况，需要分阶段评估。',
      '关于<strong>家事法庭 Restraining Order</strong>的申请条件、宣誓书与紧急动议，可参阅本站<a href="/blog/ontario-restraining-order-chinese-family/">《在安省申请 Restraining Order：华人家庭面对家暴时，法律能提供哪些紧急保护？》</a>；同类主题文章索引见<a href="/tag/restraining-order-ontario/">Restraining Order Ontario 专题标签页</a>。',
      '若您关心的是<strong>无过错离婚制度下家暴对子女监护、住房与扶养等家事结果的影响</strong>，可对照阅读<a href="/blog/no-fault-divorce-domestic-violence-ontario/">《安省是「无过错离婚」，但家暴真的对结果没影响吗？》</a>。',
      '<strong>重要提示：</strong>何淼律师执业领域为<strong>安省家庭法</strong>。移民法（IRPA/IRPR）适用与执法高度个案化。涉及工签、枫叶卡、入境或遣返风险时，建议在家庭法策略之外，另行咨询<strong>加拿大持牌移民顾问（RCIC）或移民诉讼律师</strong>，以获得针对性的移民法意见。',
      '如需就<strong>限制令申请、证据组织、与离婚/监护程序的衔接</strong>讨论家庭法方案，欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    keywords:
      '限制令移民身份,Restraining Order工签枫叶卡,不准接触令移民,安省家事法庭限制令,刑事定罪移民后果,华人家庭法律师多伦多,何淼律师',
    featured: true,
    metaTitle: '申请限制令会影响对方工签或枫叶卡吗？｜何淼律师 多伦多家庭法',
    metaDescription:
      '警方临时不准接触与家事法庭民事限制令不等于刑事定罪；是否影响工签、枫叶卡等须视后续刑事程序与个案而定，并非必然吊销签证。',
    pairedFaqPath: '/en/faq/restraining-order-immigration-pr-work-permit/',
  },
  {
    slug: 'toronto-jinji-xianzhiling-zhouqi',
    capsule:
      '先分清两条路：警方在刑事相关程序里作出的临时不准接触（non-contact order，有时当场）与家事法庭依<a href="https://www.ontario.ca/laws/statute/90f03" target="_blank" rel="noopener noreferrer">《家庭法》</a>签发的民事 Restraining Order（常走 urgent motion，力争约两周内首轮聆讯，视法院排期与材料而定）。',
    question: '在多伦多申请「紧急限制令」，通常需要多长时间能拿到？',
    summary:
      '实务上常把两类措施都口语化成「限制令」，但<strong>路径不同、时间也不同</strong>：<strong>（1）</strong>一方报警并说明人身安全或家暴等风险后，警方可能在调查/释放等环节<strong>很快</strong>作出<strong>临时不准接触（non-contact order）</strong>类安排，以<strong>先稳住接触边界</strong>；这不等于家事法法庭已签发民事 Restraining Order。<strong>（2）</strong>若不走或不仅走刑事路径，而向<strong>家事法庭</strong>申请民事 Restraining Order，律师通常会通过<strong>紧急动议（urgent motion）</strong>争取排期；在多伦多地区常见经验是<strong>力争在约两周内</strong>排到<strong>首轮聆讯</strong>，法官审查证据后如认为符合法定条件，才会签发命令。<strong>具体排期因法院、分院、案件量与材料完备度而异</strong>，「两周」并非法律上的硬性保证。',
    answers: [
      '若您指的是<strong>报警后警方作出的临时不准接触（non-contact order）</strong>：在说明存在即时安全风险或家暴等情形时，警方有可能在<strong>接案后相当短的时间内</strong>对接触方式作出限制，以<strong>先降低当下冲突</strong>。这类措施往往依附于<strong>刑事调查/释放条件</strong>等框架，与家事法庭签发的<strong>民事 Restraining Order</strong>不是同一份「纸」。',
      '若您指的是在<strong>家事法庭</strong>申请<strong>民事 Restraining Order</strong>：实务上多通过<strong>urgent motion（紧急动议）</strong>推进。何律师在多伦多/大多地区案件中的常见经验是：<strong>力争约两周内</strong>获得<strong>首轮开庭/聆讯</strong>的机会，但<strong>实际排期会因法院与季节、案件积压、是否材料齐全、对方是否到庭等因素浮动</strong>。',
      '无论哪条路径，<strong>能否「拿到」以及「拿到什么强度」的命令</strong>，最终都取决于<strong>事实、证据与法律标准</strong>（例如合理恐惧、是否适合单方面申请等），而不是单纯「越快越好」的口号。',
    ],
    details: [
      '先把概念对齐：英文语境里有时会把 <strong>restraining order</strong> 与 <strong>non-contact order</strong> 混着说，但在安省语境下，您更需要区分的是——<strong>警方在刑事相关程序中作出的临时接触限制</strong>，与<strong>家事法官依据<a href="https://www.ontario.ca/laws/statute/90f03" target="_blank" rel="noopener noreferrer">《家庭法》</a>（FLA）或<a href="https://www.ontario.ca/laws/statute/90c12" target="_blank" rel="noopener noreferrer">《儿童法改革法》</a>（CLRA）签发的民事 Restraining Order</strong>。两者<strong>申请主体、程序、证据标准与后续救济</strong>都不同。',
      '在<strong>警方路径</strong>上，若一方报警并陈述存在被跟踪、骚扰、暴力或升级风险，警方可能基于当时掌握的信息，在调查、逮捕或释放等环节<strong>较快</strong>采取<strong>限制双方接触</strong>的安排（您口语中说的 non-contact order）。其首要目的通常是<strong>即时风险控制</strong>，并不自动等同于「家庭法层面的一切问题都已终局解决」。',
      '在<strong>家事法庭路径</strong>上，律师通常会准备宣誓书与证据包，通过 <strong>urgent motion</strong> 说明为何需要<strong>紧急听审</strong>以及为何符合<strong>单方面（ex parte）或短期听审</strong>等程序条件（以个案策略为准）。在<strong>多伦多及大多地区</strong>，常见目标是<strong>争取约两周内的首轮聆讯窗口</strong>；但<strong>法院排期并非律师可单方承诺</strong>，极端繁忙时期也可能更长。',
      '聆讯后，法官只有在认为<strong>证据与法律要件</strong>满足时，才会签发或续签 Restraining Order，并可能同时处理<strong>临时育儿安排、婚姻住房排他性居住</strong>等联动议题（视申请内容而定）。更完整的程序与证据清单，见<a href="/blog/ontario-restraining-order-chinese-family/">《在安省申请 Restraining Order…》</a>及<a href="/tag/restraining-order-ontario/">专题标签页</a>。',
      '若您同时关心「限制令与对方移民身份」的交叉问题，可先阅读本站<a href="/faq/xianzhiling-yimin-gongqian-fengye/">如果我申请了限制令，会影响对方的移民身份吗？（例如工签或枫叶卡）</a>。',
      '如需评估您是否符合<strong>紧急动议</strong>、材料如何组织、以及多伦多/新市等<strong>具体分院排期</strong>的现实区间，欢迎致电647-930-6688预约何淼律师咨询。',
    ],
    keywords:
      '多伦多紧急限制令,urgent motion restraining order Toronto,non-contact order多伦多,家事法庭限制令排期,安省家暴律师,何淼律师',
    featured: true,
    metaTitle: '多伦多紧急限制令多久能拿到？警方 vs 家事法庭｜何淼律师',
    metaDescription:
      '警方临时不准接触可能较快作出；家事法庭民事 Restraining Order 常走 urgent motion，常见经验力争约两周内首轮聆讯，实际以法院排期与案情为准。',
    pairedFaqPath: '/en/faq/toronto-urgent-restraining-order-timeline/',
  },
  {
    slug: 'ansheng-jiatingfa-jiatingbao-xingwei-fanwei',
    capsule:
      '安省语境下的「家庭暴力」不限于动手：可包括言语虐待、骚扰、跟踪、限制行动自由、胁迫控制，以及利用经济不对等实施的经济控制/经济虐待等，须结合证据与个案认定。',
    question: '在安省的家庭法中，除了肢体冲突，哪些行为会被认定为家暴？',
    summary:
      '在安省家庭法相关争议（尤其是<strong>子女安排与安全</strong>）中，<strong>家庭暴力（family violence）</strong>常被理解为<strong>很宽的一组行为</strong>，并不限于殴打或肢体冲突。实务与立法表述中，除身体暴力外，还可能包括：<strong>言语虐待、恐吓、骚扰、跟踪监视、限制人身自由或社交</strong>，以及利用经济权力不对等实施的<strong>经济控制/经济虐待</strong>（例如控制账户、切断必要开支、以金钱胁迫对方服从等，视证据而定）。是否「构成」家庭暴力语境下的风险，最终看法官或决策者如何认定<strong>事实与证据</strong>。',
    answers: [
      '除肢体冲突外，常见被纳入讨论的行为包括：<strong>言语侮辱、威胁、恐吓</strong>；<strong>反复骚扰、跟踪、监视</strong>对方行踪或通讯；<strong>限制对方与外界联系、限制离家或社交</strong>；以及<strong>经济控制</strong>——利用收入、账户、信用卡、房租/房贷与生活费分配等不对等，实质性地<strong>支配或剥夺</strong>对方可支配资源，从而胁迫其留在关系中或服从安排（是否构成需个案证据）。',
      '需要强调的是：法律上「宽定义」不等于<strong>随口指控即可成立</strong>。无论是哪一种行为类型，进入诉讼或动议后，仍要回到<strong>可证明的事实、时间线、第三方材料与可信性</strong>。',
    ],
    details: [
      '在安省，涉及子女最大利益、安全与接触安排时，法院会审查是否存在<strong>家庭暴力（family violence）</strong>或相关风险。立法与判例语境下的「暴力」往往强调<strong>对安全、自主权与心理安全的侵害</strong>，而不仅是「有没有被打」的单一线索。',
      '<strong>精神/言语虐待与胁迫控制（coercive control）</strong>在实务中越来越常被严肃对待：例如长期贬低、羞辱、威胁「让你见不到孩子」「让你身败名裂」、控制通讯与行程等，若证据充分，可能与身体暴力一样进入法院的风险评估。',
      '<strong>经济虐待（financial abuse）</strong>通常指一方利用<strong>经济依赖或信息不对称</strong>，通过控制银行账户、拒绝支付必要家庭开支、切断信用卡、隐瞒或转移资金、迫使对方签署不利文件等方式，实质限制对方<strong>离开关系或独立决策</strong>的能力。您提到的「financial assault / 经济层面控制」在中文讨论里常被归入这一类，但<strong>具体措辞与认定标准仍以证据与法律论证为准</strong>。',
      '此外，<strong>性胁迫、破坏财物、利用孩子传话施压、在社交媒体上公开羞辱</strong>等，也可能在个案中成为「家庭暴力」事实矩阵的一部分。',
      '若您正在同时考虑<strong>限制令、紧急动议与离婚/监护程序</strong>的衔接，可参阅<a href="/blog/ontario-restraining-order-chinese-family/">《在安省申请 Restraining Order…》</a>、<a href="/blog/no-fault-divorce-domestic-violence-ontario/">《安省是「无过错离婚」，但家暴真的对结果没影响吗？》</a>，以及<a href="/faq/xianzhiling-yimin-gongqian-fengye/">限制令与移民身份常见问答</a>。',
      '若您已报警但警方当时未逮捕对方，仍可评估向家事法庭申请民事限制令的可行性，见本站<a href="/faq/jingcha-wei-buzhua-jiatingfa-xianzhiling/">如果我报警了，警察没有把人抓走，我还能去家庭法（家事法庭）申请限制令吗？</a>。',
      '若您关心<strong>家暴是否会改变子女监护与育儿时间的裁判</strong>，见<a href="/faq/jiabao-yingxiang-jianhu-yuershijian-ontario/">家暴会影响安省法官对于子女监护权和育儿时间的裁决吗？</a>。',
      '何淼律师提示：若您正处于<strong>即时危险</strong>，请先拨打<strong>911</strong>；也可拨打 Assaulted Women’s Helpline <strong>1-866-863-0511</strong>（多语言）。如需家庭法层面的策略与证据组织，欢迎致电647-930-6688预约咨询。',
    ],
    keywords:
      '安省家庭暴力定义,family violence Ontario,经济虐待离婚,胁迫控制华人家庭,言语家暴法律,跟踪骚扰家庭法,何淼律师',
    featured: true,
    metaTitle: '除肢体冲突外，哪些行为算家暴？安省家庭法｜何淼律师',
    metaDescription:
      '安省家庭法语境下家庭暴力含义较宽，可含言语虐待、骚扰、跟踪、限制自由与经济控制等；是否成立取决于事实与证据，非口头标签即可。',
    pairedFaqPath: '/en/faq/ontario-family-violence-beyond-physical/',
  },
  {
    slug: 'jingcha-wei-buzhua-jiatingfa-xianzhiling',
    capsule:
      '可以。刑事执法/检控与家事法庭民事限制令的证明标准与程序不同；警方未逮捕或未进入有罪判决，并不自动排除在符合条件时向家事法庭申请 Restraining Order。',
    question: '如果我报警了，警察没有把人抓走，我还能去家庭法（家事法庭）申请限制令吗？',
    summary:
      '可以。<strong>刑事路径</strong>与<strong>家事法庭民事路径</strong>适用的是<strong>不同的法律标准与程序</strong>。警方在某一时刻是否逮捕、检控机关是否提出指控，依据的是<strong>刑事执法与检控门槛</strong>；而家事法官审理<strong>民事限制令（Restraining Order）</strong>时，依据的是<strong><a href="https://www.ontario.ca/laws/statute/90f03" target="_blank" rel="noopener noreferrer">《家庭法》</a>（FLA）、<a href="https://www.ontario.ca/laws/statute/90c12" target="_blank" rel="noopener noreferrer">《儿童法改革法》</a>（CLRA）等下的民事要件</strong>。因此，即便对方<strong>尚未在刑事程序中被认定有罪</strong>，只要您能在民事动议框架下提出<strong>充分、可采纳的证据</strong>，并满足法定条件，仍<strong>可能</strong>获得限制令；<strong>是否最终签发取决于个案证据与听审结论</strong>。',
    answers: [
      '可以。警方「有没有抓人」并不等于家事法上「能不能申请限制令」。刑事上常见考量包括是否有<strong>合理根据</strong>逮捕、证据是否达到检控标准、公共利益等；家事法上则更聚焦于<strong>人身安全、接触边界、合理恐惧与骚扰/暴力风险</strong>等民事救济要件（以具体申请类型为准）。',
      '因此，即便对方<strong>没有被逮捕</strong>，或<strong>尚未被刑事定罪</strong>，只要您的材料能够支撑法官在民事程序中作出有利于您的认定，法院仍<strong>可以</strong>签发 Restraining Order。反之亦需注意：<strong>刑事上有动作也不自动等于</strong>家事法庭必然会签发命令——两条线<strong>相互独立又可能并行</strong>，策略上需要分轨评估。',
    ],
    details: [
      '先把概念对齐：许多人会把<strong>报警后的警方处理结果</strong>与<strong>家事法庭能否给民事保护</strong>绑在一起，但二者<strong>法律性质不同</strong>。警方未逮捕，可能反映的是当时证据、风险评估、执法裁量或程序阶段等多种因素，并不自动写入「家事法上一定没有风险」的结论。',
      '在向家事法庭申请限制令时，核心通常是：如何在宣誓书与证据包中，清晰呈现<strong>事实时间线、风险升级点、以及为何需要法院命令来限制接触或接近</strong>等（以个案策略为准）。这与刑事法庭是否最终定罪，是<strong>不同法庭、不同角色、不同证明框架</strong>下的问题。',
      '实务提醒：<strong>情绪叙述与可呈堂证据不是一回事</strong>。无论是短信、邮件、通话记录、证人陈述、医疗或报警记录、还是第三方文件，都涉及<strong>可采性与相关性</strong>的筛选，建议在律师指导下系统整理。',
      '关于<strong>紧急动议、排期与「警方 non-contact」和民事 Restraining Order 的区别</strong>，可参阅<a href="/faq/toronto-jinji-xianzhiling-zhouqi/">《在多伦多申请「紧急限制令」通常需要多长时间？》</a>；关于<strong>限制令与移民身份</strong>的交叉问题，见<a href="/faq/xianzhiling-yimin-gongqian-fengye/">限制令与工签/枫叶卡常见问答</a>；程序与材料总览见<a href="/blog/ontario-restraining-order-chinese-family/">《在安省申请 Restraining Order…》</a>及<a href="/tag/restraining-order-ontario/">专题标签页</a>。',
      '若您同时关心<strong>除肢体冲突外哪些行为可能纳入家暴语境</strong>，见<a href="/faq/ansheng-jiatingfa-jiatingbao-xingwei-fanwei/">在安省的家庭法中，除了肢体冲突，哪些行为会被认定为家暴？</a>。',
      '若您同时关心<strong>家暴对子女监护与育儿时间的影响</strong>，见<a href="/faq/jiabao-yingxiang-jianhu-yuershijian-ontario/">家暴会影响安省法官对于子女监护权和育儿时间的裁决吗？</a>。',
      '何淼律师提示：若您正处于<strong>即时危险</strong>，请先拨打<strong>911</strong>。如需就限制令申请、证据组织与程序策略咨询家庭法，欢迎致电647-930-6688预约。',
    ],
    keywords:
      '报警警察没抓人限制令,未逮捕申请Restraining Order,刑事民事标准不同,安省家事法庭限制令,紧急动议家暴,何淼律师',
    featured: true,
    metaTitle: '报警后警察没抓人，还能申请家事法庭限制令吗？｜何淼律师',
    metaDescription:
      '刑事与家事法庭标准不同；警方未逮捕不妨碍在证据充分且符合家庭法要件时向安省家事法庭申请民事 Restraining Order，是否签发以个案为准。',
    pairedFaqPath: '/en/faq/no-arrest-restraining-order-ontario-family-law/',
  },
  {
    slug: 'jiabao-yingxiang-jianhu-yuershijian-ontario',
    capsule:
      '视个案而定：纯经济控制等若不指向孩子即时安全风险，未必显著改变育儿安排；肢体暴力、身体虐待及影响孩子安全的行为，则更可能进入法官对子女最大利益的裁量。',
    question: '家暴会影响安省法官对于子女监护权和育儿时间的裁决吗？',
    summary:
      '<strong>取决于您所指的家暴在具体事实中属于哪一类。</strong>正如本站常见问答<a href="/faq/ansheng-jiatingfa-jiatingbao-xingwei-fanwei/">在安省的家庭法中，除了肢体冲突，哪些行为会被认定为家暴？</a>中所述，安省家庭法语境下的<strong>家庭暴力（family violence）</strong>含义较宽，可包括<strong>经济虐待、言语威胁与其他形态的胁迫或控制</strong>等。<strong>若主要是配偶之间的经济支配或资源控制</strong>，且<strong>在证据与论证上并不表明孩子会面临即时安全风险或不利养育环境</strong>，有时对<strong>育儿时间（parenting time）与决策责任（decision-making responsibility）</strong>的最终安排<strong>影响可能相对有限</strong>（仍以个案为准）。<strong>若事实包含肢体冲突、身体虐待，或牵涉孩子安全、接触边界与稳定养育环境的风险</strong>，则更可能<strong>显著影响</strong>法官在<strong>子女最大利益</strong>框架下的判断。',
    answers: [
      '<strong>类型不同，影响往往不同。</strong>经济层面的控制若主要体现为夫妻之间的财务安排、账户控制等，且不与孩子安全直接挂钩，未必自动「决定」监护权归属；但法官仍可能将其放入<strong>整体事实矩阵</strong>（例如是否影响一方稳定养育能力、信息披露与可信性等）。',
      '<strong>一旦涉及身体暴力、恐吓升级、或让孩子目睹/卷入冲突、以及威胁孩子安全或稳定接触的行为</strong>，通常更容易进入法院对<strong>接触安排、临时或长期育儿方案</strong>的审慎评估，并可能对<strong>决策权分配、主要居所、监督接触</strong>等产生实质影响。',
    ],
    details: [
      '安省法院在子女相关争议中，核心标准是<strong>子女最大利益（best interests of the child）</strong>。<a href="https://www.ontario.ca/laws/statute/90c12" target="_blank" rel="noopener noreferrer">《儿童法改革法》</a>等框架下的考量因素很多，家庭暴力或安全风险只是其中一类需要结合<strong>证据、严重度、频率、是否持续、以及与孩子之间的关联</strong>来评估的要素。',
      '因此，「家暴」一词本身<strong>不等于</strong>自动失去监护权或自动减少育儿时间；关键是<strong>哪些事实被证明</strong>、这些事实如何与<strong>孩子的安全、稳定、情感需求与发展</strong>相连。',
      '关于<strong>无过错离婚制度下家暴是否仍可能影响结果</strong>（包括育儿与财产披露等侧面），可对照阅读<a href="/blog/no-fault-divorce-domestic-violence-ontario/">《安省是「无过错离婚」，但家暴真的对结果没影响吗？》</a>。',
      '若您尚未对齐「哪些行为可能被纳入家暴语境」，请先阅读<a href="/faq/ansheng-jiatingfa-jiatingbao-xingwei-fanwei/">在安省的家庭法中，除了肢体冲突，哪些行为会被认定为家暴？</a>；若同时涉及<strong>限制令与紧急动议</strong>，见<a href="/blog/ontario-restraining-order-chinese-family/">《在安省申请 Restraining Order…》</a>及<a href="/faq/toronto-jinji-xianzhiling-zhouqi/">在多伦多申请「紧急限制令」，通常需要多长时间能拿到？</a>。',
      '何淼律师提示：若您正处于<strong>即时危险</strong>，请先拨打<strong>911</strong>。如需就<strong>监护、育儿时间、证据与动议策略</strong>咨询，欢迎致电647-930-6688预约。',
    ],
    keywords:
      '家暴子女监护权安省,育儿时间家庭暴力,family violence parenting time Ontario,经济虐待监护权,何淼律师多伦多家庭法',
    featured: true,
    metaTitle: '家暴会影响子女监护权与育儿时间吗？安省｜何淼律师',
    metaDescription:
      '视家暴类型与是否牵涉孩子安全而定：纯经济控制未必单独决定育儿安排；肢体暴力、身体虐待及影响孩子安全的行为更可能进入法官对子女最大利益的裁量。',
    pairedFaqPath: '/en/faq/does-family-violence-affect-custody-parenting-time-ontario/',
  },
  {
    slug: 'anshen-zinv-fuyangfei-jisuan',
    capsule: '安省子女抚养费按 Federal Child Support Guidelines 算，依据支付方税前年收入、子女数量和居住省份查表确定。',
    question: '在加拿大安省，子女抚养费是怎么算的？',
    summary:
      '在安省，子女抚养费首先按《Federal Child Support Guidelines（联邦子女抚养费准则）》计算，依据是支付方的<strong>税前年收入</strong>、子女人数和居住省份。这三个变量直接对应一张全国统一的表格，查表即可得出每月应付金额。注意是税前收入，不是税后；如果支付方年收入超过 15 万加币，接收方还可依据 Guidelines 第 4 条申请超出表格金额的抚养费。若需把<strong>经济安排与育儿裁量</strong>放在一起理解，可参阅<a href="/faq/haizi-yijian-jianhu-fayuan/">离婚后孩子的意见法院会听吗？</a>；中加认定差异见<a href="/blog/ontario-vs-china-child-spousal-support-comparison/">安省抚养费 vs 中国抚养费：关键认定差异</a>；本站<a href="/family-law/support/">扶养与抚养费（子女与配偶）专题</a>亦汇总相关程序与策略。',
    answers: [
      '安省的子女抚养费遵循《Federal Child Support Guidelines》。计算依据的是<strong>支付方的税前年收入</strong>、孩子的数量和居住的省份，这三项直接对应一张全国统一的标准表格，查表即得每月应付金额。',
      '需要特别注意的是<strong>税前而非税后</strong>。很多客户用到手工资来推算，得出的数字会偏低。另外，当支付方年收入超过 <strong>15 万加币</strong>时，接收方可以依据 Guidelines 第 4 条申请超出表格金额的抚养费——理由是孩子有权分享父母实际能维持的生活水平。',
    ],
    details: [
      '《Federal Child Support Guidelines》是加拿大联邦层面的全国统一规则。安省作为参与省份，离婚案件中的子女抚养费基本都按这套准则计算。Guidelines 附带的表格已经把支付方收入、孩子人数与每月金额做了精确对应，律师和法官的工作主要是核实<strong>申报的收入是否真实完整</strong>。',
      '"税前收入"指的是 T1 申报表上 Line 15000 的总收入，包括工资、自雇收入、租金、分红、海外收入等。如果支付方有未申报或低报收入的迹象，律师可以申请财务披露与第三方核查；自雇人士还要看 Line 15000 之上的实际可支配收入，避免通过公司报销虚减个人收入。',
      '当支付方年收入超过 15 万加币时，按 Guidelines 表格继续往上推算的金额可能偏低，因为高收入家庭的孩子的实际生活成本与教育投入更高。Guidelines 第 4 条允许法院在此类案件中超出表格判决，常见考量包括<strong>家庭原本的消费水平、孩子的教育安排（私校、留学、特长培养等）</strong>。',
      '此外，Guidelines 还区分 "table amount"（按表格的基础金额）与 "section 7 expenses"（特殊或特别开支，如医疗、课外活动、上大学的学费等），后者由父母按收入比例分摊，与基础抚养费分开计算。建议致电 647-930-6688 预约何淼律师咨询您的具体情况。',
    ],
    keywords: '安省子女抚养费,Federal Child Support Guidelines,子女抚养费计算,加拿大抚养费,何淼律师,中加双执照律师',
    featured: true,
    metaTitle: '在加拿大安省，子女抚养费是怎么算的？| 何淼律师',
    metaDescription:
      '安省子女抚养费按《Federal Child Support Guidelines》计算，依据支付方税前年收入、子女数量和省份查表得出。年收入 15 万以上可申请超表金额。',
    pairedFaqPath: '/en/faq/how-is-child-support-calculated-ontario/',
  },
  {
    slug: 'meiyou-shouru-yiran-keyi-zhengqu-jianhuquan',
    capsule: '可以。安省判定子女监护权的标准是"子女最大利益原则"，考虑亲子关系、照顾能力、稳定性等，不是父母的经济条件。',
    question: '在加拿大，没有收入的一方还能拿到孩子的抚养权吗？',
    summary:
      '可以。在安省，子女监护权（decision-making responsibility / parenting time）的判定标准是<strong>"子女最大利益原则（best interests of the child）"</strong>，考虑的是亲子关系、照顾能力、稳定性、家庭暴力等因素，<strong>不是父母的经济条件</strong>。收入较低的一方完全可能成为主要监护人，对方则通过支付子女抚养费来履行经济责任。这一点和中国家庭法的逻辑有根本差别。经济侧可对照<a href="/faq/anshen-zinv-fuyangfei-jisuan/">安省子女抚养费如何计算</a>；裁量框架可参考<a href="/blog/child-custody-ontario-chinese/">安省子女监护权：华人父母常见误区</a>；亦欢迎从<a href="/about/">关于律师与事务所</a>了解我们如何处理跨境与双语案件。',
    answers: [
      '<strong>可以。</strong>在安省，监护权和经济条件是<strong>分开判断的两件事</strong>。法院判定监护权（decision-making responsibility）和育儿时间（parenting time）时遵循"子女最大利益原则"，考量孩子与父母的关系、双方的照顾能力、稳定性、是否存在家庭暴力等因素。<strong>父母的收入高低本身不是判断标准</strong>。',
      '从中国移民来的客户经常误解这一点——因为在中国法院，一方的经济能力会直接影响监护权归属。但在加拿大，收入较低的一方完全可能拿到主要监护权；对方通过支付子女抚养费来履行经济责任，并不会"因为收入高就拿到孩子"。',
    ],
    details: [
      '安省 <em>Children\'s Law Reform Act</em>（《儿童法改革法》）和联邦 <em>Divorce Act</em>（《离婚法》）都明确规定，子女相关问题的核心标准是<strong>子女最大利益（best interests of the child）</strong>。法院评估的因素包括：孩子与每位父母的情感纽带、每位父母满足孩子需求的能力、孩子的稳定性需求、家庭暴力风险、孩子的意愿（在年龄和成熟度允许时）等。',
      '<strong>"父母的经济条件"不在这份清单上。</strong>背后的法律逻辑是：经济差距由抚养费制度（支付方按 Guidelines 支付）和财产分割制度来弥补，监护权本身不应该被卖给收入更高的一方。',
      '根据何淼律师的经验，许多在传统观念里"经济条件不如对方"的客户其实拥有很强的监护权主张——他们是孩子的<strong>主要照顾者（primary caregiver）</strong>，了解孩子的日常作息、医疗、学校情况，与孩子的情感连接更紧密。这些恰恰是法院最看重的因素。',
      '如果您正面临监护权争议，请提前整理：日常照顾孩子的证据（接送、就医、家长会、签字记录等）、孩子的日常作息材料、双方与孩子相处时间的记录。欢迎致电 647-930-6688 预约何淼律师咨询。',
    ],
    keywords: '没有收入监护权,安省子女监护权,best interests of the child,主要照顾者,何淼律师,中加双执照律师',
    featured: true,
    metaTitle: '没有收入还能拿到孩子的抚养权吗？安省 | 何淼律师',
    metaDescription:
      '可以。安省判定监护权的核心标准是"子女最大利益原则"，考虑亲子关系、照顾能力、稳定性等因素，不看父母经济条件。收入差距由抚养费制度弥补。',
    pairedFaqPath: '/en/faq/parent-without-income-custody-canada/',
  },
  {
    slug: 'zhongguo-fangchan-shouru-anshen-lihun-jisuan',
    capsule: '会。安省法庭计算收入和财产时看的是全球范围，中国的工资、租金、分红、房产都必须披露并纳入计算。',
    question: '我在中国有房产和收入，加拿大法庭离婚时会算进去吗？',
    summary:
      '会。安省法庭计算一个人的收入和财产时，看的是<strong>全球范围</strong>——中国的工资、租金、分红、银行账户、房产都必须披露并纳入抚养费和财产分割的计算基础。这一点和中国法院的做法差别很大：中国法院在很多情况下对境外财产不予追溯或处理。在跨境离婚案件中，中国资产的披露是一个无法绕开的环节。财产分割框架可先读<a href="/faq/jiating-jingzichan/">离婚时如何计算家庭净资产？</a>；跨境披露与举证见<a href="/blog/cross-border-assets-divorce-ontario/">跨境资产与安省离婚</a>；需要中英双语与两国法域经验时，见<a href="/dual-licensed-china-canada-family-lawyer/">中加双执照家庭法服务介绍</a>。',
    answers: [
      '<strong>会，而且这是一个无法绕开的环节。</strong>安省法庭计算抚养费收入和财产分割时，看的是<strong>全球范围（worldwide）</strong>——你在中国的工资收入、租金收入、分红、银行存款、房产、股权，都必须如实披露，并纳入安省家庭法的计算框架。',
      '这一点与中国法院的做法形成强烈反差：中国法院在很多离婚案件中对境外财产<strong>不予追溯或处理</strong>，但安省法庭对境内境外资产一视同仁。如果你以为"中国的房产法庭管不到"而不披露，将面临严重的法律后果，包括隐瞒财产判决、不利推定、甚至诉讼成本由你独自承担。',
    ],
    details: [
      '安省 <em>Family Law Act</em>（《家庭法》）第 4 条对净家庭财产（Net Family Property）的定义不限于本地资产——只要是配偶名下的财产，无论登记地在哪里，分居日的市值都计入分居日资产清单。同样，《Federal Child Support Guidelines》计算抚养费时使用的是"年收入"，包括来自全球的所有应税及未申报的收入来源。',
      '常见的中国资产类型在披露中需要分别处理：<strong>房产</strong>（提供产权证、近期估值）、<strong>银行存款</strong>（账户对账单）、<strong>租金收入</strong>（租赁合同 + 银行流水）、<strong>分红/股权</strong>（持股证明 + 分红记录）、<strong>父母处寄存的资金</strong>（视情况是否需要披露）。',
      '中国资产的<strong>估值</strong>通常需要中国当地的评估机构出具材料，律师会指导如何获取符合安省法庭采信标准的文件。<strong>翻译公证</strong>也是必备步骤——所有中文文件需要由认证翻译机构出具英文翻译件。',
      '对方有时会通过<strong>资产追查（asset tracing）</strong>查证你在中国的财产，包括通过中国银行的境内查询、不动产登记查询、第三方征信调查等。在加拿大法庭强制披露要求下，故意隐瞒会导致严重不利后果。建议在第一次咨询时就坦诚交代全部跨境资产情况。欢迎致电 647-930-6688 预约何淼律师咨询。',
    ],
    keywords: '中国房产离婚,境外资产披露,跨境离婚,worldwide income,Net Family Property,何淼律师,中加双执照律师',
    featured: true,
    metaTitle: '中国房产和收入会算进加拿大离婚吗？| 何淼律师',
    metaDescription:
      '会。安省法庭按全球范围计算财产和收入，中国的房产、租金、分红、银行账户都必须披露并纳入分割。隐瞒境外资产将面临严重法律后果。',
    pairedFaqPath: '/en/faq/disclose-chinese-assets-ontario-divorce/',
  },
  {
    slug: 'zhongguo-anshen-peiou-shanyangfei-chabie',
    capsule: '中国法院少判配偶赡养费，仅在重大疾病或丧失劳动能力等特殊情况下判付；安省则有完整独立的配偶赡养费制度。',
    question: '在中国法院和安省法院，配偶赡养费的差别是什么？',
    summary:
      '在中国，离婚时法院判付配偶赡养费的情形<strong>非常少见</strong>，通常只在一方有重大疾病或丧失劳动能力等特殊困难时才会判。但在加拿大安省，<strong>配偶赡养费是一项独立的法律请求</strong>，有完整的认定框架。即便双方都有工作能力、都没有重大健康问题，安省法院仍然可能基于婚姻期间形成的生活水平或一方为家庭所做的职业牺牲来判付。金额与年限可接续阅读<a href="/faq/anshen-peiou-shanyangfei-jine-nianxian/">安省配偶赡养费的金额和年限是怎么决定的？</a>；系统对照见<a href="/blog/ontario-vs-china-child-spousal-support-comparison/">安省抚养费 vs 中国抚养费（含配偶赡养）</a>；程序与争议类型见<a href="/family-law/divorce-litigation/">离婚与诉讼</a>专题。',
    answers: [
      '<strong>差别非常大。</strong>在中国，配偶赡养费（生活费）在普通离婚中很少被判付，主要适用于一方有重大疾病、丧失劳动能力等特殊困难情形。中国法律的默认假设是：离婚后双方各自承担经济责任，不互相依赖。',
      '但在<strong>加拿大安省</strong>，配偶赡养费（spousal support）是一项<strong>独立的法律请求</strong>，有完整的判定框架和判例支持。即便双方都健康、都有工作能力，安省法院仍然可能判付——理由可能是婚姻期间形成的生活水平、一方为家庭所做的职业牺牲（全职照顾家庭）、或为对方支持事业发展所失去的机会成本。',
    ],
    details: [
      '安省的配偶赡养费由联邦 <em>Divorce Act</em>（《离婚法》）和省级 <em>Family Law Act</em>（《家庭法》）双重规范。判付逻辑有三类基础：<strong>补偿性（compensatory）</strong>——一方为家庭做出职业牺牲；<strong>非补偿性（non-compensatory）</strong>——婚后一方陷入经济困难；<strong>合同性（contractual）</strong>——基于双方明示或默示的协议。',
      '加拿大律师协会还发布了《Spousal Support Advisory Guidelines》（SSAG，配偶赡养费咨询准则），虽然不是强制性法律，但律师和法官在大多数案件中都会参考它来计算金额范围与年限。SSAG 给出的是"区间"（如月付金额 X 到 Y、持续 N 年到 M 年），具体数字仍由法官结合个案确定。',
      '中国法院的"少判"传统并不意味着加拿大也会少判。相反，安省的判付率高、金额可观、年限可能长达数年甚至 10 年以上，<strong>对从中国来的客户来说是一项常常被低估的法律风险</strong>。如果你是高收入一方，在第一次咨询时就应该明确"是否会被判付配偶赡养费"作为核心问题之一。',
      '反过来说，如果你是相对低收入或为家庭付出更多的一方，配偶赡养费可能是你的重要权利——不要因为"中国不判这个"就主动放弃主张。欢迎致电 647-930-6688 预约何淼律师评估您的具体情况。',
    ],
    keywords: '配偶赡养费,中加离婚差别,spousal support Ontario,SSAG,Divorce Act,何淼律师,中加双执照律师',
    featured: true,
    metaTitle: '中国法院和安省法院，配偶赡养费的差别是什么？| 何淼律师',
    metaDescription:
      '中国法院少判配偶赡养费，仅在重大疾病或丧失劳动能力时判付。安省则有独立完整的配偶赡养费制度，即便双方都健康、有工作能力仍可能判付。',
    pairedFaqPath: '/en/faq/why-canada-spousal-support-vs-china/',
  },
  {
    slug: 'anshen-peiou-shanyangfei-jine-nianxian',
    capsule: '安省配偶赡养费综合考虑五个因素：婚姻年限、双方收入、健康状况、婚姻期间生活水平、为家庭做的职业牺牲。',
    question: '在安省，配偶赡养费的金额和年限是怎么决定的？',
    summary:
      '安省法院在判定配偶赡养费时综合考虑五个因素：<strong>结婚或同居的持续年限</strong>、<strong>双方各自的收入和收入能力</strong>、<strong>一方的身体状况和工作能力</strong>、<strong>婚姻期间形成的生活水平和消费习惯</strong>、以及<strong>一方为家庭所做的职业牺牲</strong>（如全职照顾家庭放弃事业）。婚姻越长，赡养义务通常越长；长婚姻案件中，赡养费持续 5–10 年甚至更长是常见的。制度背景见<a href="/faq/zhongguo-anshen-peiou-shanyangfei-chabie/">中国法院与安省法院配偶赡养费的差别</a>；执行与 FRO 等实务见<a href="/blog/spousal-child-support-ontario/">对方不付抚养费怎么办</a>；家庭法各业务线总览见<a href="/family-law/">家庭法服务</a>。',
    answers: [
      '安省法院判定配偶赡养费时综合考量<strong>五个因素</strong>：（1）结婚或同居的持续年限；（2）双方各自的收入与收入能力；（3）一方的身体状况和工作能力；（4）婚姻期间形成的生活水平和消费习惯；（5）一方为家庭所做的职业牺牲。',
      '一般来说，<strong>婚姻越长，赡养义务越长</strong>；婚姻达到 20 年以上或加上孩子年龄达到一定标准时，可能进入<strong>不限期赡养（indefinite support）</strong>的范围。长婚姻案件中，赡养费持续 5–10 年甚至更长是常见的。具体金额通常参考 SSAG（配偶赡养费咨询准则）给出的区间。',
    ],
    details: [
      'SSAG 提供了两种公式：<strong>"无子女公式"（without child support formula）</strong>用于无子女或子女已成年的婚姻；<strong>"有子女公式"（with child support formula）</strong>用于仍需支付子女抚养费的婚姻。前者主要看双方收入差与婚姻年限，后者会考虑子女抚养费支付后的可支配收入。两种公式都给出"金额区间"和"年限区间"，律师据此谈判，法官据此判决。',
      '<strong>金额</strong>的影响因素：双方税前收入差越大、婚姻越长，配偶赡养费越高。婚姻期间形成的高消费水平也会推高金额——例如长期年消费 30 万加币的家庭，离婚后仅判付每月 2,000 加币显然不符合"维持生活水平"的逻辑。',
      '<strong>年限</strong>的常见判断：婚姻每持续一年，赡养费义务约对应半年到一年。<strong>20 年以上的婚姻</strong>，或<strong>"65 岁规则"</strong>（婚姻年限 + 接收方年龄达到 65）适用时，可能进入不限期赡养。短婚姻（如 3-5 年）通常只判 1-3 年的过渡性赡养。',
      '<strong>调整与终止</strong>：配偶赡养费判付后并非一成不变。<strong>支付方收入显著下降、接收方再婚或同居、接收方收入大幅上升</strong>等情况，都可能成为申请减免或终止的理由。建议保留收入变化的证据，并在变化发生后及时咨询律师。欢迎致电 647-930-6688 预约何淼律师咨询。',
    ],
    keywords: '配偶赡养费金额,配偶赡养费年限,SSAG,spousal support guidelines,不限期赡养,何淼律师',
    featured: true,
    metaTitle: '安省配偶赡养费的金额和年限是怎么决定的？| 何淼律师',
    metaDescription:
      '安省配偶赡养费综合考虑婚姻年限、双方收入、健康状况、婚姻期间生活水平、为家庭做的职业牺牲。长婚姻持续 5–10 年常见，20 年以上可能不限期。',
    pairedFaqPath: '/en/faq/spousal-support-amount-duration-ontario/',
  },
  {
    slug: 'ssag-shi-shenme-anshen',
    capsule: 'SSAG 是配偶赡养费咨询准则，提供金额和年限的参考区间，但只是顾问性指导，不像子女抚养费那样强制。',
    question: 'SSAG 是什么？为什么 SSAG 不像子女抚养费那样必须执行？',
    summary:
      'SSAG 全称 Spousal Support Advisory Guidelines（配偶赡养费咨询准则），由加拿大司法部委托学者编制，自 2008 年起在安省家庭法案件中广泛使用。SSAG 提供配偶赡养费的<strong>金额区间</strong>和<strong>年限区间</strong>，但是<strong>顾问性参考</strong>而非强制法律——这是它与子女抚养费指导（Federal Child Support Guidelines，强制性表格计算）的根本区别。法官在判付时通常以 SSAG 为起点，但可以基于个案 factor 上下调整甚至偏离。具体金额年限计算见<a href="/faq/anshen-peiou-shanyangfei-jine-nianxian/">配偶赡养费的金额和年限怎么决定</a>；判付资格基础见<a href="/faq/peiou-fuyangfei-entitlement-zige-anshen/">谁有资格主张配偶赡养费</a>；如涉及收入争议见<a href="/faq/yincang-shouru/">对方隐瞒收入怎么办</a>。',
    answers: [
      'SSAG = <strong>Spousal Support Advisory Guidelines</strong>，是加拿大司法部委托 Carol Rogerson 教授和 Rollie Thompson 教授编制的<strong>顾问性参考文件</strong>，提供配偶赡养费的金额和年限建议区间。它<strong>不是强制法律</strong>，但安省家庭法律师和法官在大多数案件中都会参考。',
      'SSAG 与子女抚养费指导（Federal Child Support Guidelines）有本质区别：<strong>子女抚养费必须按表算</strong>，法官几乎没有裁量空间；<strong>配偶赡养费可参考 SSAG 但不强制照搬</strong>，法官根据个案各项 factor 综合裁量，可以落在 SSAG 区间低端、中端、高端，也可以高于上限或低于下限。',
    ],
    details: [
      'SSAG 提供两种公式：<strong>"无子女公式"（without child support formula）</strong>用于无未成年子女的婚姻，主要看双方收入差与婚姻年限；<strong>"有子女公式"（with child support formula）</strong>用于同时支付子女抚养费的婚姻，会先扣除子女抚养费再计算可支配收入。两种公式都给出"金额区间"（月支付低端/中端/高端三个数字）和"年限区间"（最短到最长的支付年限）。',
      '<strong>为什么 SSAG 不强制？</strong>这是政策设计的有意选择。立法者认识到配偶赡养费涉及的 factor 比子女抚养费复杂得多——婚姻长度、双方角色、补偿性诉求、健康状况等都需要法官个案权衡，强制公式无法准确反映复杂事实。因此 SSAG 设计为"顾问性指导 + 法官裁量"的混合模式。',
      '<strong>法院如何使用 SSAG？</strong>实务中，安省法官把 SSAG 区间作为<strong>起点</strong>，然后结合：（1）案件具体 factor；（2）双方提出的偏离理由；（3）相关判例；（4）公平和合理性来确定最终金额。例如长期婚姻 + 强补偿性诉求可能高于 SSAG 上限，支付方有特殊财务困难可能低于下限，或在不寻常情形下完全偏离 SSAG。',
      '<strong>实务影响</strong>：理解 SSAG 的"顾问性"对当事人有三个意义：（1）不能单凭网上 SSAG 计算器结果决定接受或拒绝和解；（2）谈判空间在 SSAG 区间内外都存在；（3）<strong>收入数字（包括 imputed income）的争议往往比"哪个 SSAG 区间"更关键</strong>——同样的婚姻长度，对方收入推定为 8 万还是 15 万，最终金额差异巨大。具体案件评估请致电 647-930-6688 预约何淼律师咨询。',
    ],
    keywords: 'SSAG,Spousal Support Advisory Guidelines,配偶赡养费咨询准则,顾问性指导,无子女公式,有子女公式,Rule of 65,何淼律师',
    featured: true,
    metaTitle: 'SSAG 是什么？为什么 SSAG 不像子女抚养费那样必须执行？| 何淼律师',
    metaDescription:
      'SSAG 是配偶赡养费咨询准则，由加拿大司法部委托学者编制，提供金额和年限参考区间。它是顾问性指导而非强制法律，与子女抚养费 Federal Guidelines 性质完全不同。',
    pairedFaqPath: '/en/faq/what-is-ssag-ontario/',
  },
  {
    slug: 'peiou-fuyangfei-entitlement-zige-anshen',
    capsule: '配偶赡养费不是离婚自动产生的，主张方必须先证明 entitlement（资格）成立，三类基础：补偿性、非补偿性、契约性。',
    question: '在安省，谁有资格主张配偶赡养费？什么情况下法院才会判付？',
    summary:
      '在安省，配偶赡养费<strong>不是离婚时自动产生的</strong>。主张方必须先证明 entitlement（资格）成立。加拿大最高法院在 <em>Bracklow v. Bracklow</em> [1999] 1 S.C.R. 420 中确立了三类资格基础：<strong>补偿性</strong>（一方在婚姻中为家庭放弃职业发展）、<strong>非补偿性</strong>（分居后基本生活困难）、<strong>契约性</strong>（婚前/分居协议约定）。如果 entitlement 不成立，无论 SSAG 计算结果多高，法院都不会判付。资格成立后，金额和年限的计算见<a href="/faq/anshen-peiou-shanyangfei-jine-nianxian/">安省配偶赡养费的金额和年限</a>；SSAG 性质见<a href="/faq/ssag-shi-shenme-anshen/">SSAG 是什么</a>；同居伴侣的特殊规则见<a href="/faq/tongju-banlv-peiou-fuyangfei-anshen/">同居伴侣能否主张配偶赡养费</a>。',
    answers: [
      '<strong>配偶赡养费不是自动的。</strong>主张方必须先过两道门槛：第一步是 entitlement（资格）是否成立；第二步才是金额和年限怎么定。如果第一步不成立，第二步根本不会展开——无论 SSAG 算出多高，法院都不会判付。',
      '加拿大最高法院在 <em>Bracklow v. Bracklow</em> 案中确立了三类 entitlement 基础：<strong>（1）补偿性请求</strong>——一方为家庭利益放弃职业发展；<strong>（2）非补偿性请求</strong>——分居后基本生活困难；<strong>（3）契约性请求</strong>——婚前协议、同居协议或分居协议中约定。这三类基础<strong>可以同时主张</strong>，叠加会显著影响金额和年限。',
    ],
    details: [
      '<strong>基础一：补偿性请求（Compensatory Support）</strong>。适用于一方在婚姻中为家庭利益放弃职业发展的情形。典型场景：全职妈妈/爸爸长期照顾子女离开职场；一方为支持配偶事业放弃工作随迁；一方资助配偶完成学业但未享受婚姻继续的回报；一方在家族企业中无偿付出多年。理论基础是"婚姻是经济伙伴关系，一方为共同利益的牺牲应得到补偿"。',
      '<strong>基础二：非补偿性（需求性）请求（Non-Compensatory）</strong>。适用于分居后一方基本生活困难需要支持的情形。典型场景：接收方收入显著低于支付方，无法维持婚姻期间生活水平；接收方因健康、年龄等原因就业能力受限；长期婚姻中两人经济深度依赖。理论基础是"婚姻产生经济相互依赖，分居后较短时间不能让一方陷入贫困"。',
      '<strong>基础三：契约性请求（Contractual）</strong>。基于双方在婚前协议、同居协议或分居协议中的约定。前提是协议本身有效——即满足程序要求（独立法律建议、充分披露、自愿签署）和实质要求（条款合理、不极端不公平）。如果协议中放弃配偶赡养费的条款不符合这些要求，可能被法院推翻。婚前协议详见<a href="/blog/prenuptial-agreement-ontario-2026/">安省婚前协议详解</a>。',
      '<strong>什么情况下 entitlement 不成立？</strong>三种典型：（1）短婚姻 + 双方都有独立收入（例如结婚 2 年，双方都全职工作，无补偿性也无生活困难）；（2）双方收入相当且经济独立无相互依赖；（3）婚前协议明确放弃且协议有效。但<strong>注意</strong>：即使表面上 entitlement 似乎弱，仔细分析也常能找到补偿性或非补偿性基础——这正是律师评估的价值。',
      '<strong>实务建议</strong>：（1）不要假设"短婚就没资格"——即使结婚 3 年，如果一方为搬家放弃职业，仍可能有补偿性请求；（2）不要假设"我收入高就一定要付"——如果对方收入也不低且无补偿性基础，entitlement 可能不成立；（3）协议条款要谨慎——婚前协议中放弃配偶赡养费的条款不一定有效。具体资格评估请致电 647-930-6688 预约何淼律师咨询。',
    ],
    keywords: 'entitlement,配偶赡养费资格,Bracklow v Bracklow,补偿性请求,非补偿性请求,契约性请求,婚前协议,何淼律师',
    featured: true,
    metaTitle: '在安省，谁有资格主张配偶赡养费？三类 entitlement 基础详解 | 何淼律师',
    metaDescription:
      '安省配偶赡养费不是离婚自动产生的，主张方必须先证明 entitlement 成立。三类基础：补偿性（职业牺牲）、非补偿性（生活困难）、契约性（协议约定）。',
    pairedFaqPath: '/en/faq/who-is-entitled-to-spousal-support-ontario/',
  },
  {
    slug: 'tongju-banlv-peiou-fuyangfei-anshen',
    capsule: '可以——只要满足同居 3 年以上，或同居期间有共同子女且关系稳定，就有权主张配偶赡养费。',
    question: '我们没有结婚，只是同居，分手时能要配偶赡养费吗？',
    summary:
      '可以——但有条件。在安省，<strong>未婚同居伴侣（common-law partner）</strong>只要满足以下任一条件，就有权主张配偶赡养费：（1）<strong>连续同居 3 年以上</strong>；或（2）<strong>同居期间有共同子女且关系具有相对稳定性</strong>。同居关系下的配偶赡养费计算方式与已婚夫妻基本相同，三类 entitlement 基础同样适用，SSAG 计算同样使用。但<strong>关键区别是同居伴侣没有自动的财产分割权利</strong>（不适用净家庭财产均等）。同居伴侣必须在分居后<strong>2 年内</strong>主张配偶赡养费，否则可能丧失权利。资格基础见<a href="/faq/peiou-fuyangfei-entitlement-zige-anshen/">谁有资格主张配偶赡养费</a>；金额年限见<a href="/faq/anshen-peiou-shanyangfei-jine-nianxian/">配偶赡养费金额和年限</a>；同居 vs 离婚详解见<a href="/blog/common-law-vs-divorce-ontario/">安省同居关系与婚姻的区别</a>。',
    answers: [
      '<strong>可以。</strong>这是华人客户最大的盲区——以为"没领证就两清"，实际上安省未婚同居伴侣分手时同样可能要支付配偶赡养费，金额和年限与已婚情况基本相同。这个盲区造成的实际损失常常巨大：一方放弃了本可主张的几年到十几年配偶赡养费，另一方在毫无心理准备下面对重大付款义务。',
      '根据安省 <em>Family Law Act</em> 第 29 条，"配偶"的定义除了已婚夫妻，还包括两类同居伴侣：（1）<strong>连续同居 3 年以上</strong>，处于"类似配偶关系（conjugal relationship）"；或（2）<strong>关系具有相对稳定性（some permanence）且有共同子女</strong>——此类不要求满 3 年。',
    ],
    details: [
      '<strong>什么算"类似配偶关系"？</strong>法院综合考量：共同居住地址；经济上的相互依赖（共同账户、共担房租房贷）；性关系；社交场合中以伴侣身份示人；共同度假、家庭活动；子女、亲属对关系的认知。不是单看是否登记同居协议，而是看实际关系状态。',
      '<strong>同居伴侣 vs 已婚夫妻的核心区别</strong>：（1）<strong>配偶赡养费</strong>——满足条件即可主张，标准与已婚相同；（2）<strong>三类 entitlement 基础</strong>——补偿性、非补偿性、契约性同样适用；（3）<strong>SSAG 计算</strong>——同样使用；（4）<strong>财产分割</strong>——<strong>这是关键区别！</strong>同居伴侣<strong>没有自动的净家庭财产（NFP）均等权利</strong>，财产纠纷需通过 constructive trust（推定信托）等普通法途径主张，难度更高。',
      '<strong>时效问题：分居后 2 年内必须主张</strong>。根据 <em>Family Law Act</em>，未婚同居伴侣必须在分居后 2 年内提出配偶赡养费主张，否则可能丧失权利。已婚夫妻虽也有时效，但同居伴侣的时效更严格、更容易被忽视——这是最常见的"权利沉睡"陷阱。',
      '<strong>典型情形示例</strong>：（1）同居 8 年未婚 + 2 名子女 + 女方分居前 5 年全职在家——满足"3 年"和"有共同子女"两个条件，强补偿性请求，可能每月数千加币 + 年限 4-8 年；（2）同居 2 年未婚 + 1 个子女——不满足 3 年但满足"关系稳定 + 共同子女"，仍可主张；（3）同居 4 年无子女 + 双方全职工作 + 收入相当——满足 3 年但 entitlement 较弱，是否判付要看具体补偿性/非补偿性基础。',
      '<strong>中国客户特别提醒</strong>：中国法律下"同居关系"基本不产生抚养费义务，从中国移民来的客户常误以为安省也是如此，这是危险的认知偏差。涉及中加跨境同居关系的常见复杂情形：两人在中国相识同居后移民安省；一方持工签随另一方来加但未能就业；一方有中国境内资产但未在加拿大披露。这类案件需要中加双重法律视角评估。何淼律师持中加双执照（中国 2009 / 安省 LSO #83315K），处理过多起涉及中加跨境同居关系的家庭法案件。咨询预约：647-930-6688。',
    ],
    keywords: '同居伴侣,common-law,common-law partner,配偶赡养费,Family Law Act 第29条,同居3年,Constructive Trust,共同子女,何淼律师',
    featured: true,
    metaTitle: '没结婚的同居伴侣能要配偶赡养费吗？安省 common-law 规则详解 | 何淼律师',
    metaDescription:
      '在安省，同居 3 年以上或有共同子女的未婚伴侣分手时同样可能主张配偶赡养费，标准与已婚相同。但同居伴侣没有自动的财产分割权利，且 2 年时效更严格。',
    pairedFaqPath: '/en/faq/common-law-spousal-support-ontario/',
  },
  {
    slug: 'peiou-cizhi-buzuo-zhifufang-shijiao',
    capsule: '作为支付方，如果对方分居后故意不工作来获取长期配偶赡养费，可以主张推定收入、申请有期限赡养命令或申请变更命令。',
    question: '我作为支付方，对方分居后故意辞职不工作来要更多赡养费，我怎么应对？',
    summary:
      '安省法院<strong>不允许接收方故意不工作</strong>来获取长期配偶赡养费。除非接收方有身体或精神原因导致无法工作，否则法院会期待其在合理时间内重新进入职场。作为支付方，你可以：（1）<strong>主张对接收方 impute income</strong>，按其合理就业能力推定收入降低应付金额；（2）<strong>申请有期限的过渡性赡养</strong>，明确截止期；（3）<strong>申请变更或终止赡养命令</strong>。判例支持详见 <em>Drygala v. Pauli</em>, 2002 CanLII 41868 (ON CA)。从接收方视角的同主题问答见<a href="/faq/quanzhizhifu-shinian-peioushanyangfei/">全职在家 10 年能拿多少赡养费</a>；推定收入实操见<a href="/faq/yincang-shouru/">对方隐瞒收入怎么办</a>；entitlement 基础见<a href="/faq/peiou-fuyangfei-entitlement-zige-anshen/">谁有资格主张配偶赡养费</a>。',
    answers: [
      '<strong>不要假设你必须一直养下去。</strong>加拿大《Divorce Act》明确规定双方都有"合理努力实现经济自给自足"的义务。这意味着接收方不能故意不工作来获取永久赡养——法院会评估接收方的<strong>就业能力</strong>，而不只是当前实际收入。',
      '作为支付方，你有三种主要应对策略：（1）<strong>主张推定收入（imputed income against recipient）</strong>——按对方合理就业能力推定一个收入数字，从而降低应付赡养费金额；（2）<strong>谈判或申请有期限的过渡性赡养</strong>——例如 3 年后自动终止；（3）<strong>已有命令的可申请变更或终止</strong>——如对方有能力工作但拒绝工作。',
    ],
    details: [
      '<strong>策略一：主张推定收入降低应付金额</strong>。安省判例 <em>Drygala v. Pauli</em>, 2002 CanLII 41868 (ON CA) 确立了在配偶刻意不工作时如何推定收入的标准。作为支付方需要举证：对方的工作经历、行业薪资水平、当前就业市场情况、对方的健康和年龄、是否需要照顾未成年子女、对方的求职努力（或缺乏努力）。如果证据成立，法院会按"对方应该能赚多少"而非"对方实际赚多少"计算 SSAG 输入数字，从而降低你的应付金额。',
      '<strong>策略二：争取有期限的过渡性赡养</strong>。在协议或法庭命令中，<strong>明确赡养费的截止日期或复审条件</strong>。常见结构：3-5 年的过渡期，到期自动终止；或设置中期复审日（例如 2 年后回看接收方就业进展）。对中等长度婚姻这是合理且常见的安排，避免承担"无限期付款"的风险。',
      '<strong>策略三：申请变更或终止已有命令</strong>。如果赡养命令已经存在但情况发生实质性变化（material change），可以申请变更（variation）或终止（termination）。可援引的事由：（1）对方有就业能力但拒绝寻找工作；（2）对方再婚或同居；（3）对方收入大幅上升；（4）你的收入显著下降。法院会要求你证明"实质性变化"——保留好相关证据是关键。',
      '<strong>哪些情形法院会支持接收方"暂时不工作"？</strong>并非所有"不工作"都被法院否定，以下情形法院可能支持较长期赡养：（1）有医学证明的身体或精神疾病无法工作；（2）20 年以上长期婚姻 + 一方为家庭长期放弃职业发展；（3）"65 法则"（婚姻年限 + 接收方分居时年龄 ≥ 65）；（4）接收方需照顾年幼或特殊需要的子女；（5）强补偿性请求（全职妈妈、随配偶工作搬迁等）。这些情形下推定收入主张往往不会被法院支持。',
      '<strong>实务建议</strong>：（1）不要假设"我必须一直养下去"——委托律师评估对方的真实就业能力；（2）<strong>收集对方有就业能力但不工作的证据</strong>（社交媒体、招聘网站浏览记录、行业内人脉证词等）；（3）在协议或命令中<strong>明确截止期</strong>或复审条件；（4）保留<strong>你自己收入变化</strong>的证据，为未来变更命令做准备。具体策略评估请致电 647-930-6688 预约何淼律师咨询。',
    ],
    keywords: '配偶赡养费支付方策略,推定收入接收方,有期限赡养命令,变更命令,Drygala v Pauli,自给自足义务,Divorce Act,何淼律师',
    featured: true,
    metaTitle: '配偶分居后故意不工作来要更多赡养费，支付方怎么应对？| 何淼律师',
    metaDescription:
      '安省法院不允许接收方故意不工作来获取长期赡养费。作为支付方可主张推定收入、申请有期限赡养、变更已有命令。Drygala v. Pauli 判例支持。',
    pairedFaqPath: '/en/faq/payor-spouse-refuses-to-work-ontario/',
  },
  {
    slug: 'best-chinese-family-lawyer-toronto',
    capsule: '多伦多离婚律师 · 熟悉多伦多 Superior Court',
    question: '在多伦多打家庭法官司，为什么律师熟悉多伦多法庭特别重要？',
    summary:
      '多伦多 Superior Court 与 Newmarket 等法院在法官分工与程序预期上差异很大。若案件在多伦多进行，需要的是<strong>在多伦多出庭积累的实战经验</strong>，而不只是一般的「安省家庭法经验」。',
    answers: [
      '多伦多采用通才制（General Division），家事法官并非只做家事案件；对动议、Case Conference、证据披露的预期与专职家事法庭不同。',
      '何淼律师自2009年起深耕家事法，常年在多伦多及其他主要法院出庭；完整说明见本站专题页。',
    ],
    details: [
      '本题详解请参阅：<a href="/best-chinese-family-lawyer-toronto/">多伦多华人家庭法专题 · 法庭经验说明</a>。',
      '预约咨询请致电 647-930-6688。',
    ],
    keywords:
      '多伦多华人离婚律师,多伦多 Superior Court,家庭法律师 Toronto,Miao He,何淼律师',
    featured: true,
    metaTitle: '多伦多家事法庭｜为何多伦多出庭经验很重要｜何淼律师',
    metaDescription:
      '多伦多家庭法院与其他城市运作方式不同；若律师仅在 Newmarket 积累经验，未必熟悉多伦多 Superior Court 的实际节奏。',
    pairedFaqPath: '/en/faq/best-chinese-family-lawyer-toronto/',
    detailPath: '/best-chinese-family-lawyer-toronto/',
  },
  {
    slug: 'uncontested-divorce-newmarket-court-markham',
    capsule: '万锦或列治文山可以办理无争议离婚吗？',
    question: '万锦或列治文山居民可以在Newmarket法庭办理无争议离婚吗？',
    featured: true,
    metaTitle:
      '万锦列治文山居民可以在Newmarket法庭办理无争议离婚吗？| 何淼律师 · 多伦多离婚律师 · 家庭法律师',
    metaDescription:
      '何淼律师提供安省无争议离婚全程办理服务，协助处理自行申请被退件、长期未获离婚令等常见问题，收费透明，无隐藏费用。预约咨询 647-930-6688。',
    summary:
      '可以。无争议离婚（Uncontested Divorce）是指双方就离婚本身及所有附属事项已达成一致，不需要法官裁决争议，费用远低于有争议诉讼。但<strong>无争议离婚并不只是提交申请书那么简单</strong>，法庭和法官的具体要求需要有经验的律师全程跟进。',
    answers: [
      '无争议离婚的申请书是在法庭<strong>公开档案</strong>中登记的，任何人都可以查阅。法庭和法官的具体要求远不止申请书本身，这些要求不会公开列明，AI也无法提供准确指引。',
      '在审理过程中，法官可以随时要求补充新文件，例如收入证明等。<strong>如果这些文件未能及时提交，离婚证就无法发出</strong>。更常见的情况是：申请提交后无人跟进，结果被搁置，客人等了两年还拿不到离婚证。',
    ],
    details: [
      '何淼律师处理过大量客人自行提交、或由其他律师提交后申请不下来的案例——原因往往是文件不符合法官要求，或提交后没有持续跟进。',
      '无争议离婚看似简单，实则有很多细节需要注意：申请书的格式和内容、附件文件的完整性、法庭对特定情况的额外要求（如涉及子女、跨境婚姻等），以及提交后与法庭的持续沟通跟进。',
      '何淼律师为客人办理无争议离婚采取<strong>Package收费</strong>：一次收费，全程跟进，无隐藏费用。中间需要为客人补充材料或与法庭沟通，均包含在内，不另行收费。欢迎致电<strong>647-930-6688</strong>了解具体流程和费用。',
    ],
    keywords:
      '无争议离婚万锦,无争议离婚列治文山,Newmarket法庭无争议离婚,Uncontested Divorce安省,离婚申请被退件,拿不到离婚证,华人离婚律师,何淼律师,Package收费离婚',
  },
  {
    slug: 'when-to-settle-divorce-ontario',
    capsule: '离婚案件什么时候应该提出和解？',
    question: '离婚案件什么时候应该提出和解？',
    featured: true,
    metaTitle: '离婚案件什么时候应该提出和解？| 何淼律师 · 多伦多离婚律师 · 家庭法律师',
    metaDescription:
      '家庭诉讼不仅是出庭，更是判断和解时机与整体布局。何淼律师每周处理家庭法案件，结合个案事实制定诉讼与谈判策略，全程亲自跟进。预约咨询 647-930-6688。',
    summary:
      '和解时机是家庭法诉讼中最关键也最难判断的问题之一。提出和解太早，筹码不足，可能接受不利条件；太晚，则白白增加诉讼成本。<strong>客人自己往往无法判断最佳时机</strong>，这正是有经验律师的价值所在。',
    answers: [
      '提出和解的时机取决于多个因素：<strong>案件的进展阶段、双方的谈判筹码、对方的态度、以及法庭程序节点</strong>。在某些程序节点前提出和解，效果往往优于其他时机。',
      '客人自己很难判断这些因素的综合权重。过早和解可能意味着在筹码不足时妥协；过晚和解则可能白白支出大量诉讼费用，而最终结果并不比早些谈判好多少。',
    ],
    details: [
      '诉讼不是目的，解决问题才是。但在家庭法案件中，「什么时候谈」往往比「谈什么」更重要。一个有经验的律师，会在整个诉讼过程中持续评估和解的时机，而不是被动等待对方提出。',
      '何淼律师在15年以上的诉讼实践中，处理过大量在不同阶段达成和解的案件。她会根据每个案件的具体情况，包括双方的财务状况、子女安排的争议程度、对方律师的策略风格，以及法庭程序的节点，<strong>主动为客人规划最佳的和解时机和策略</strong>。',
      '何律师的理念是：<strong>一个好的律师，不是跟着对方走，而是掌握主动权</strong>。从DRO Conference到Settlement Conference，每一个节点都是推动案件向有利方向发展的机会。欢迎致电<strong>647-930-6688</strong>预约咨询。',
    ],
    keywords:
      '离婚什么时候和解,家庭法和解时机,Settlement Offer安省,离婚谈判策略,万锦离婚律师,列治文山离婚律师,华人家庭法律师,何淼律师',
  },
  {
    slug: 'newmarket-court-regular-motion-vs-long-motion',
    capsule: '在Newmarket法庭，Regular Motion和Long Motion有什么区别？',
    question: '在Newmarket法庭，Regular Motion和Long Motion有什么区别？',
    featured: true,
    metaTitle:
      '在Newmarket法庭，Regular Motion和Long Motion有什么区别？| 何淼律师 · 多伦多离婚律师 · 家庭法律师',
    metaDescription:
      '何淼律师拥有 15 年以上诉讼经验，熟悉 Newmarket 法庭 Regular Motion、Long Motion 及紧急动议申请流程，为家庭法案件制定针对性策略。预约咨询 647-930-6688。',
    summary:
      'Newmarket法庭默认安排的是Regular Motion（一小时）。但复杂案件有时需要向法庭申请leave来安排Long Motion；紧急情况下则需要提起Urgent Motion。选错Motion类型，轻则浪费一次庭期，重则错失保护权益的最佳时机。',
    answers: [
      '<strong>Regular Motion</strong>：Newmarket法庭默认的Motion类型，庭期为一小时，适合争议相对集中、材料不复杂的申请。',
      '<strong>Long Motion</strong>：需要向法庭申请leave（许可）才能安排，适合争议复杂、需要更多时间陈述的案件。<strong>Urgent Motion</strong>：用于需要法庭紧急介入的情况，例如子女被带离安省、财产被转移等紧急事项。',
    ],
    details: [
      '在Newmarket法庭，选择正确的Motion类型是诉讼策略的重要组成部分。如果案件复杂但只申请了Regular Motion，一小时内无法充分陈述，可能导致法官无法完整了解案情，影响裁决结果。',
      '反之，如果不需要Long Motion却申请了，不仅浪费法庭资源，还可能拖延庭期安排。而在真正紧急的情况下，如果律师不知道如何提起Urgent Motion，客人的权益可能在等待中受损。',
      '何淼律师拥有超过15年家庭法诉讼经验，每周在Newmarket法庭出庭，能够根据案件的具体情况，判断<strong>什么时候提Regular Motion、什么时候申请Long Motion、什么时候需要Urgent Motion</strong>，为客人制定最有效的庭审策略。欢迎致电<strong>647-930-6688</strong>预约咨询。',
    ],
    keywords:
      'Regular Motion Newmarket,Long Motion安省,Urgent Motion家庭法,Newmarket法庭Motion,万锦离婚Motion,家庭法庭动议,华人离婚律师Newmarket,何淼律师',
  },
  {
    slug: 'newmarket-court-filing-deadline-requirements',
    capsule: 'Newmarket法庭的文件提交有什么特别要求？',
    question: 'Newmarket法庭的文件提交有什么特别要求？',
    featured: true,
    metaTitle:
      'Newmarket法庭的文件提交有什么特别要求？| 何淼律师 · 多伦多离婚律师 · 家庭法律师',
    metaDescription:
      'Newmarket 家庭法庭对文件提交时间要求严格。何淼律师熟悉 filing deadline 与程序规则，帮助客户避免因材料错误或逾期提交导致案件延误。预约咨询 647-930-6688。',
    summary:
      'Newmarket法庭对所有向法庭提交文件的时间有严格要求，必须在规定Deadline前完成。一旦错过，原本应该开的Conference可能无法如期进行，导致案件被拖延。这种拖延不仅浪费时间，更可能影响谈判筹码。',
    answers: [
      'Newmarket法庭对文件提交的<strong>Deadline要求严格，且与其他法庭不同</strong>。不熟悉该法庭程序的律师，可能因不了解具体要求而错过截止日期。',
      '一旦错过Deadline，原本应该开的Conference可能无法如期进行，案件被迫延期。这种延误<strong>不只是浪费时间</strong>，更可能在关键谈判节点上削弱客人的筹码。',
    ],
    details: [
      '在Newmarket法庭，每一次Conference和Motion之前都有对应的文件提交要求，包括材料的种类、格式和提交时间。这些要求有时与安省家庭法规则的一般性要求略有不同，体现了Newmarket法庭的本地惯例。',
      '何淼律师长期在Newmarket法庭出庭，对各类程序节点的文件要求了如指掌。她不仅帮助客人准备符合要求的文件，更会主动跟踪每一个Deadline，确保案件按计划推进，<strong>不因程序失误而造成不必要的延误</strong>。',
      '很多客人找到何律师时，都是因为之前的律师错过了Deadline，导致案件被拖延数月。选择一个熟悉Newmarket法庭程序的律师，是保护自己权益的第一步。欢迎致电<strong>647-930-6688</strong>预约咨询。',
    ],
    keywords:
      'Newmarket法庭文件要求,Newmarket filing deadline,万锦离婚文件提交,列治文山离婚程序,家庭法庭程序要求,华人离婚律师Newmarket,何淼律师',
  },
  {
    slug: 'newmarket-court-dro-conference-explained',
    capsule: 'Newmarket法庭的DRO Conference是什么？',
    question: 'Newmarket法庭的DRO Conference是什么？必须参加吗？',
    featured: true,
    metaTitle:
      'Newmarket法庭的DRO Conference是什么？必须参加吗？| 何淼律师 · 多伦多离婚律师 · 家庭法律师',
    metaDescription:
      '何淼律师长期处理 Newmarket 家庭法庭案件，熟悉 DRO Conference 流程、法官关注重点及协商策略，为万锦及列治文山华人客户提供直接法律代理。预约咨询 647-930-6688。',
    summary:
      'DRO Conference（Dispute Resolution Officer Conference）是Newmarket法庭特有的程序节点，通常建议在Case Conference之前进行。由双方律师协商时间安排，目的是在正式开庭前尝试解决争议、缩小分歧。熟悉Newmarket法庭的律师会充分利用DRO Conference的谈判机会，为后续程序争取有利位置。',
    answers: [
      'DRO Conference由<strong>双方律师直接协商时间安排</strong>，不需要等法庭排期，是Newmarket法庭在正式Case Conference之前的一个重要程序节点。',
      'DRO Conference结束后，根据案件进展，可以直接进入Case Conference，也可以进入Settlement Conference。<strong>不了解这个程序的律师可能直接跳过</strong>，导致案件走弯路，白白浪费时间和金钱。',
    ],
    details: [
      'Newmarket法庭的DRO Conference是该法庭独有的程序安排，在其他法庭（如多伦多、士嘉堡）并不常见。DRO（Dispute Resolution Officer）是由法庭指派的专业人员，协助双方在正式开庭前尝试解决争议。',
      '对于有经验的律师来说，DRO Conference不只是一个程序节点，更是一个重要的<strong>谈判窗口</strong>。如何在DRO Conference中展示己方立场、评估对方底线、为后续Conference做好铺垫，都需要丰富的实战经验。',
      '何淼律师每月在Newmarket法庭出席大量DRO Conference，对流程、法官关注重点及谈判策略均有深入了解，可以帮助客人在这个关键节点上争取最有利的位置。欢迎致电<strong>647-930-6688</strong>预约咨询。',
    ],
    keywords:
      'DRO Conference,Newmarket法庭DRO,Dispute Resolution Officer,万锦离婚程序,列治文山离婚程序,Newmarket家庭法程序,华人离婚律师Newmarket,何淼律师',
  },
  {
    slug: 'markham-richmond-hill-divorce-court-location',
    capsule: '住在万锦或列治文山，离婚案件在哪个法庭审理？',
    question: '住在万锦或列治文山，离婚案件在哪个法庭审理？',
    featured: true,
    metaTitle: '住在万锦或列治文山，离婚案件在哪个法庭审理？| 何淼律师 · 多伦多离婚律师 · 家庭法律师',
    metaDescription:
      '何淼律师专注安省家庭法，熟悉万锦及列治文山案件由 Newmarket 家庭法庭管辖的审理规则。普通话英语双语服务，帮助华人客户高效处理离婚与家庭纠纷。预约咨询 647-930-6688。',
    summary:
      '住在万锦（Markham）或列治文山（Richmond Hill）的居民，家庭法案件由<strong>Newmarket法庭</strong>管辖。Newmarket法庭是安省的专属家庭法法庭（Unified Family Court），所有法官专门审理家庭法案件，判决稳定、可预期。',
    answers: [
      'Newmarket法庭是安省少数几个<strong>专属家庭法法庭（Unified Family Court）</strong>之一，所有在此审理《离婚法》案件的法官都只处理家庭法案件，不兼审刑事或民事案件。',
      '这对当事人来说意味着：法官对财产分割、子女监护、赡养费等议题的判断标准比较一致，<strong>判决相对稳定、可预期</strong>。有经验的律师可以根据法庭倾向，提前为客人规划诉讼策略。',
    ],
    details: [
      '万锦和列治文山属于York Region，家庭法案件统一由Newmarket法庭管辖。很多客人在找律师时只看律师的口碑和价格，却忽略了一个关键问题：<strong>你的律师是否真正熟悉Newmarket法庭的程序？</strong>',
      'Newmarket法庭有自己独特的程序要求，包括DRO Conference的安排方式、文件提交的Deadline规定、Motion的申请类型等，这些都与多伦多、士嘉堡等其他法庭不同。不熟悉Newmarket法庭的律师，可能在程序上走弯路，白白浪费客人的时间和金钱。',
      '何淼律师长期在Newmarket法庭出庭，每月出席超过20小时的Conference和Motion，对该法庭的程序节奏、法官倾向、排期规律有深入的第一手了解。欢迎致电<strong>647-930-6688</strong>预约咨询。',
    ],
    keywords:
      '万锦离婚法庭,列治文山离婚法庭,Newmarket法庭,Newmarket家庭法庭,万锦离婚律师,列治文山离婚律师,华人离婚律师万锦,何淼律师',
  },
  {
    slug: 'case-conference-xuyao-zhunbei-shenme',
    capsule: '安省 Case Conference 需要准备什么？',
    question: '安省家庭法的 Case Conference 需要准备什么？',
    featured: true,
    pairedFaqPath: '/en/faq/how-to-prepare-case-conference-ontario/',
    metaTitle: '安省 Case Conference 需要准备什么？',
    metaDescription:
      '必备 Form 17A 案件会议简报和 Form 17F 确认书，涉及财产或支持的还需财务陈述。期限、页数限制、以及漏交 17F 会被取消开庭的后果。',
    summary:
      '必须准备两份文件：<strong>Form 17A</strong>（Case Conference Brief，案件会议简报）和 <strong>Form 17F</strong>（Confirmation of Conference，会议确认书）。涉及财产分割或抚养费、配偶支持的，还需要财务陈述。两份文件的提交期限不同，漏交 Form 17F 会导致开庭被取消。',
    answers: [
      '核心文件是 <strong>Form 17A</strong> 与 <strong>Form 17F</strong>；涉及财产或支持的，还要准备 Form 13/13.1 财务陈述和 Form 13A 披露证明书。',
      'Form 17A 通常有<strong>8 页上限</strong>（个别法院如 Barrie 为 12 页）；约到开庭日期的一方须在会议前 <strong>6 天</strong>送达并提交，另一方为 <strong>4 天</strong>（Rule 17(13.1)）。',
      'Form 17F 须<strong>先送达对方、再交给书记官</strong>，一般不迟于会议前 3 个工作日下午 2 点；漏交通常会被法院撤下排期。',
      '按 Rule 14(4)，在处理实质争议的会议完成前，多数动议不得送达或聆讯——Case Conference 准备充分与否，直接决定之后几个月的推进速度。',
    ],
    details: [
      '<strong>Case Conference 是什么。</strong>Case Conference（案件会议）是安省家庭法诉讼中，当事人第一次正式与法官见面的庭前程序，由法官主持。目的是让法官了解案件、缩小争议范围、推动双方和解，并安排下一步程序。它不是审判，也不是动议聆讯。法官通常不会对双方仍有争议的实质问题——孩子由谁抚养、财产如何分割、抚养费最终数额——作出最终裁决。依据：Ontario Family Law Rules, O. Reg. 114/99, Rules 14 与 17；Superior Court of Justice Consolidated Provincial Practice Direction for Family Proceedings。',
      '<strong>Form 17A：案件会议简报。</strong>Form 17A 是向法官介绍案件的核心文件，通常包括：双方的基本背景资料；婚姻或同居关系的发展过程；子女情况（如适用）；目前存在的争议事项；希望法官协助解决的问题；已经进行过的协商情况及和解建议。法官开庭前会阅读双方提交的简报，因此内容应当简洁、准确，并突出案件重点。',
      '<strong>页数限制。</strong>省高等法院的 Practice Direction 规定，Case Conference Brief 不得超过 <strong>8 页</strong>，其中包括作为附录或附表附上的额外事实与论点（规则允许的附件不计入）。个别法院另有规定，例如 Barrie 法院为 12 页。提交前应确认所在法院的具体要求。',
      '<strong>Form 17A 提交期限（Rule 17(13.1)）。</strong>申请会议、约到开庭日期的一方：会议前 <strong>6 天</strong>送达并提交；另一方：会议前 <strong>4 天</strong>。如果开庭日期是在 First Appearance 上排定的、双方都没有「申请」，则申请人（Applicant）被视为约到日期的一方，适用 6 天期限。',
      '<strong>财务文件。</strong>如果案件涉及财产分割、抚养费或配偶支持，还需要准备：Form 13 或 Form 13.1 财务陈述（Financial Statement）；Form 13A 财务披露证明书（Certificate of Financial Disclosure）。',
      '<strong>60 天规则。</strong>如果上一份财务陈述距今已超过 60 天，必须更新——财务状况有重大变化的，提交新的 Form 13；没有变化或仅有轻微变化的，提交 Form 14A 誓章说明。提交前，必须把财务陈述中的银行账户号码、按揭和贷款账号等个人识别信息涂黑。',
      '<strong>Form 17F：不要漏掉的确认书。</strong>除了简报之外，双方还必须在规定期限内提交 Confirmation（Form 17F）。它通知法院：会议是否仍需进行、预计需要多长时间、当天需要法官处理哪些最重要的问题，以及法官应当阅读哪些文件及其在 Continuing Record 中的卷号和标签号。',
      '<strong>漏交 17F 的后果。</strong>如果没有按时提交 Form 17F，法院通常会认为会议不再需要进行，并将已排定的 Case Conference 从排期表中撤下，除非法院另有命令。届时需要重新申请新的开庭日期，既浪费时间，也会拖慢整个案件的进度。',
      '<strong>Form 17F 期限和顺序。</strong>不迟于会议前 3 天下午 2 点交给法院书记官。规则条文写的是 “days”，但省高等法院的 Practice Direction 要求为 <strong>3 个工作日</strong>，按工作日计算更安全。顺序不能反：必须先把确认书副本给对方，然后才交给书记官（Rule 17(14)(b)）。交出后如果发现内容有误或情况有变，必须立即向对方和书记官提交更正后的确认书。',
      '<strong>会前必须沟通。</strong>双方（或律师）在会议前必须就争议事项进行口头或书面沟通，并在 Form 17F 中说明。未经沟通可能导致会议被押后，并被判承担成本。',
      '<strong>法官在会议上能作出什么命令。</strong>对双方仍有争议的实质问题，法官通常不会在这一步作出最终裁决。但按 Rule 17(8)，法官可以：作出文件披露命令（Rule 19）；命令进行 Questioning（Rule 20）；就专家意见证据作出命令；规定各项步骤的时间表、安排下一次开庭日期；就案件下一步给出指示。此外，只要双方就某事项达成一致，法官就可以据此签发 Consent Order（同意令）。实务上，很多实质问题确实是在这一阶段以同意令解决的，只是路径是协议，不是裁决。',
      '<strong>准备重点不只是介绍案情。</strong>更要想清楚：这次开庭要拿到哪几项程序命令。尤其是披露命令和时间表——拿不到，案件就会停在原地。',
      '<strong>会议上说的话会成为证据吗？</strong>一般不会。Case Conference 鼓励双方坦诚沟通，因此会议上发表的意见、提出的和解建议，以及为促成和解所作的讨论，通常不能在之后的审判或其他程序中被引用，用来证明某一方曾作出让步。Form 17A 不进入 Continuing Record，除非法官另有命令；即使法官命令归档，简报中涉及和解的部分必须删除（Rule 17(22.1)）。',
      '<strong>重要例外与 Endorsement。</strong>如果双方在会议上共同确认了某些事实，或达成了协议，而这些内容被记入法官的 Endorsement（批示）或 Consent Order，那么这些事实和命令日后是可以被引用和执行的。会议结束时务必认真核对 Endorsement 或 Consent Order，确认法院记录的事实、双方达成的协议以及程序安排是否准确。记错的内容会跟着案件走到最后。',
      '<strong>为什么这一步绕不过去。</strong>按 Rule 14(4)，在处理案件实质争议的会议完成之前，动议通知和支持证据都不得送达，动议也不得被聆讯。例外见 Rule 14(4.2)：法院认为存在紧急情况（urgency）、困难情形（hardship），或出于司法利益认为无需先开案件会议的，不受此限制。在拿到第一次会议之前，大部分临时救济都用不上；准备得充分与否，直接决定案件之后几个月的推进速度。',
      '如需结合您的案件排期、所在法院页数限制与披露现状做会前清单，欢迎致电 <strong>647-930-6688</strong> 预约何淼律师咨询。',
    ],
    keywords:
      'Case Conference准备,Form 17A,Form 17F,安省案件会议,家庭法Case Conference,财务陈述Form 13,安省家庭法规则17,何淼律师',
  },
  {
    slug: 'qinren-ansheng-qushi-zenmeban',
    capsule: '亲人在安省去世，怎么申请遗产认证？',
    question: '亲人在安省去世，有遗嘱是不是就不用去法院？',
    featured: true,
    detailPath: '/estate-trustee/',
    pairedFaqPath: '/en/estate-trustee/',
    metaTitle: '亲人在安省去世怎么办 | 遗产受托人证书 | 何淼律师',
    metaDescription:
      '有遗嘱也不等于不用去法院。银行和土地局通常要遗产受托人证书。何淼律师办理有遗嘱、无遗嘱及中国继承人申请。647-930-6688。',
    summary:
      '不一定。遗嘱指定了执行人，银行、土地局仍常常要求法院签发的遗产受托人证书。联名房产、指定受益人的注册账户有时可以不走这套。',
    answers: [
      '多数写在死者自己名下的房产和账户，机构要看到 Certificate of Appointment of Estate Trustee 才肯放行。',
      '何淼律师近期一件有遗嘱的申请大约四个月拿到证书；一件无遗嘱的申请已请求免除保证金。',
    ],
    details: [
      '详见<a href="/estate-trustee/">安省遗产认证</a>总览：有遗嘱、无遗嘱、继承人在中国、保证金和遗产管理税分开说明。',
      '预约请致电 647-930-6688。',
    ],
    keywords: '华人律师办理遗产继承,普通话律师协助遗产执行,万锦华人律师处理遗产继承,多伦多华人律师处理遗产,安省华人律师probate,亲人安省去世怎么办,何淼律师',
  },
  {
    slug: 'wu-yizhu-ansheng-jicheng',
    capsule: '安省无遗嘱，谁有权申请、财产怎么分？',
    question: '安省没有遗嘱，遗产怎么继承、谁去法院申请？',
    featured: true,
    detailPath: '/estate-trustee/without-a-will/',
    pairedFaqPath: '/en/estate-trustee/without-a-will/',
    metaTitle: '无遗嘱安省遗产继承 | 何淼律师',
    metaDescription:
      '无遗嘱按《继承法改革法》分配。非居民原则上不能当 administrator。何淼律师办理无遗嘱申请及保证金免除。647-930-6688。',
    summary:
      '财产分配和谁去法院申请是两件事。同居伴侣一般不按无遗嘱继承。人在中国通常不能直接当 administrator。',
    answers: [
      '已婚配偶有优先份额；有子女时再和子女分剩余部分。',
      '其他人同意后，可以由非优先顺位的人申请（Estates Act s. 29(2)）。',
    ],
    details: [
      '详见<a href="/estate-trustee/without-a-will/">无遗嘱申请</a>。',
      '预约请致电 647-930-6688。',
    ],
    keywords: '华人律师办理遗产继承,普通话律师协助遗产执行,万锦华人律师处理遗产继承,多伦多华人律师处理遗产,安省华人律师probate,无遗嘱遗产继承,何淼律师',
  },
  {
    slug: 'jichengren-zai-zhongguo-probate',
    capsule: '继承人在中国，安省遗产认证怎么做？',
    question: '人在中国，是唯一受益人，能不能自己当安省 estate trustee？',
    featured: true,
    detailPath: '/estate-trustee/china-heirs/',
    pairedFaqPath: '/en/estate-trustee/china-heirs/',
    metaTitle: '继承人在中国 | 安省遗产认证 | 何淼律师',
    metaDescription:
      '无遗嘱时非居民原则上不能获得 administration。有遗嘱的执行人通常要保证金。何淼律师处理中国继承人与国内证明文件。647-930-6688。',
    summary:
      '无遗嘱时原则上不能自己当 administrator；有遗嘱可以申请，但通常要保证金，除非法官减免。',
    answers: [
      '实务上常由安省居民申请，国内受益人配合同意书和亲属关系证明。',
      '国内公证、海牙认证和安省法院申请要放在同一套方案里。',
    ],
    details: [
      '详见<a href="/estate-trustee/china-heirs/">继承人在中国</a>。',
      '预约请致电 647-930-6688。',
    ],
    keywords: '华人律师办理遗产继承,普通话律师协助遗产执行,万锦华人律师处理遗产继承,多伦多华人律师处理遗产,安省华人律师probate,继承人在中国,何淼律师',
  },
  {
    slug: 'ansheng-yichan-baozhengjin',
    capsule: '安省遗产保证金是什么，可以免除吗？',
    question: '安省遗产认证的保证金（bond）一定要交吗？',
    featured: true,
    detailPath: '/estate-trustee/administration-bond/',
    pairedFaqPath: '/en/estate-trustee/administration-bond/',
    metaTitle: '安省遗产保证金免除 | 何淼律师',
    metaDescription:
      '符合条件可以用同意方式申请免除或减免保证金，不必一上来就动议。何淼律师近期无遗嘱案件已申请免除。647-930-6688。',
    summary:
      '无遗嘱、非居民执行人时最常被要求。全体有权分配的人同意且没有未成年受益人时，可以随申请请求免除。',
    answers: [
      'Rule 74.11(5) 允许同意申请，不必单独动议。',
      '何淼律师近期一件无遗嘱案件已申请免除保证金。',
    ],
    details: [
      '详见<a href="/estate-trustee/administration-bond/">保证金 Bond</a>。',
      '预约请致电 647-930-6688。',
    ],
    keywords: '华人律师办理遗产继承,普通话律师协助遗产执行,万锦华人律师处理遗产继承,多伦多华人律师处理遗产,安省华人律师probate,遗产保证金免除,何淼律师',
  },
  {
    slug: 'ansheng-yichan-guanlishui',
    capsule: '安省遗产管理税：5 万以下免，超出每千元 15 元',
    question: '安省遗产认证要交多少税？5 万以下是不是不用交？',
    featured: true,
    detailPath: '/estate-trustee/estate-administration-tax/',
    pairedFaqPath: '/en/estate-trustee/estate-administration-tax/',
    metaTitle: '安省遗产管理税怎么算 | 何淼律师',
    metaDescription:
      '2020年起申请：5万及以下税为0，超出部分每千元15元。这是交给财政部的税，不是律师费。647-930-6688。',
    summary:
      '5 万及以下税为 0；超出部分每 1,000 加元（不足一千按一千）15 加元。例如遗产 24 万，税 2,850 加元。',
    answers: [
      '这是交给安省财政部的税，不是律师费。',
      '出证后 180 天内通常还要提交 Estate Information Return。',
    ],
    details: [
      '详见<a href="/estate-trustee/estate-administration-tax/">遗产管理税</a>。',
      '预约请致电 647-930-6688。',
    ],
    keywords: '华人律师办理遗产继承,普通话律师协助遗产执行,万锦华人律师处理遗产继承,多伦多华人律师处理遗产,安省华人律师probate,安省遗产管理税,何淼律师',
  },
];

export function getFaqItemsForListing(): FaqItem[] {
  const pinned = faqItems.filter((item) => item.featured);
  const rest = faqItems.filter((item) => !item.featured);
  // Featured blocks are appended in chronological order; list newest first on /faq.
  return [...pinned].reverse().concat(rest);
}